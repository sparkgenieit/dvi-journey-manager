                            ...
                            <div class="col-md-12">
                                <div class="card p-0">
                                    <div class="card-header pb-3 d-flex justify-content-between">
                                        <h5 class="card-title mb-3">List of Vehicle Type</h5>
                                        <a href="javascript:void(0)" class="btn btn-label-primary waves-effect" onclick="showVEHICLETYPEMODAL(0);" data-bs-dismiss="modal">+ Add Vehicle Type</a>
                                    </div>
                                    <div class="card-body dataTable_select text-nowrap">
                                        <div class="text-nowrap table-responsive table-bordered">
                                            <table class="table table-hover " id="vehicle_type_LIST">
                                                <thead>
                                                    <tr>
                                                        <th scope="col">S.No</th>
                                                        <th scope="col">Action</th>
                                                        <th scope="col">Vehicle Type Title</th>
    ...
    <script>
        $(document).ready(function() {
            $('#vehicle_type_LIST').DataTable({
                ...
                ajax: {
                    "url": "engine/json/__JSONvehicletype.php",
                    "type": "GET"
                },
                columns: [{
                        data: "count"
        ...
        function showVEHICLETYPEMODAL(VEHICLE_TYPE_ID) {

            $('.receiving-vehicle-type-form-data').load('engine/ajax/__ajax_add_vehicle_type_form.php?type=show_form&VEHICLE_TYPE_ID=' + VEHICLE_TYPE_ID + '', function() {
                const container = document.getElementById("addVEHICLETYPEFORM");
                const modal = new bootstrap.Modal(container);
                modal.show();
                if (VEHICLE_TYPE_ID) {
                    $('#VEHICLETYPEFORMLabel').html('Update Vehicle Type');
                } else {
                    $('#VEHICLETYPEFORMLabel').html('Add Vehicle Type');
                }
            });
        }
        //SHOW DELETE POPUP
        function showDELETEVEHICLETYPEMODAL(ID) {
            $('.receiving-confirm-delete-form-data').load('engine/ajax/__ajax_manage_vehicle_type.php?type=delete&ID=' + ID, function() {
        ...
        function confirmVEHICLETYPEDELETE(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/__ajax_manage_vehicle_type.php?type=confirmdelete",
                data: {
                    _ID: ID
                },
                dataType: 'json',
        ...
        function togglestatusITEM(STATUS_ID, VEHICLE_TYPE_ID) {
            if (VEHICLE_TYPE_ID) {
                $.ajax({
                    type: "POST",
                    url: "engine/ajax/__ajax_manage_vehicle_type.php?type=updatestatus",
                    data: {
