      ...
      <div class="layout-page">
        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <span id="showvehicleLIST"></span>
            <span id="showvehicleFORMSTEP1"></span>
            <span id="showvehicleFORMSTEP2"></span>
                ...
                <div class="card p-0">
                  <div class="card-header pb-3 d-flex justify-content-between">
                    <div class="d-flex align-items-center">
                      <h5 class="m-0">Branch Name and Code - </h5>
                      <p class="ms-2 m-0 mt-1 fs-6 text-primary">Ekkattuthangal - Ekk215225</p>
                    </div>
                    <div>

                      <a href="vendor.php?route=add&formtype=branch_list&id=<?= $vendor_id; ?>" id="add_hotel" class="btn btn-label-github waves-effect ps-3"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-narrow-left me-1" width="20" height="20" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                          <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                          <path d="M5 12l14 0"></path>
                          <path d="M5 12l4 4"></path>
                          <path d="M5 12l4 -4"></path>
                        </svg>Back to Branches List</a>


                        <a href="vehicle_basic.php?branch_id=<?= $branch_id; ?>&vendor_id=<?= $vendor_id; ?>" id="add_hotel" class="btn btn-label-primary waves-effect me-2">+ Add Vehicle</a>
    ...
    <script>
      $(document).ready(function() {
        ...
        <?php elseif (($_GET['route'] == 'add' || $_GET['route'] == 'edit') && $_GET['formtype'] == 'basic_info') : ?>
          showvehicleFORMSTEP1('<?= $_GET['id']; ?>');
        <?php elseif ($_GET['route'] == 'add' && $_GET['formtype'] == 'image_upload') : ?>
          showvehicleFORMSTEP2('<?= $_GET['id']; ?>', '<?= $_GET['vendor_id']; ?>', '<?= $_GET['branch_id']; ?>');
        <?php elseif ($_GET['route'] == 'add' && $_GET['formtype'] == 'branch_list') : ?>
          showvendorFORMSTEP3('<?= $_GET['id']; ?>');
        <?php elseif ($_GET['route'] == 'add' && $_GET['formtype'] == 'permit_cost') : ?>
          showvendorFORMSTEP4('<?= $_GET['id']; ?>');
        <?php elseif ($_GET['route'] == 'preview' && $_GET['formtype'] == 'vendor_preview') : ?>
          show_VENDOR_PREVIEW('<?= $_GET['id']; ?>');
      ...
      function showvehicleLIST() {
        $.ajax({
          type: 'post',
          url: 'engine/ajax/__ajax_create_vehicle_list.php?type=show_form',
          data: {
            vendor_id: <?= $vendor_id ?>,
            branch_id: <?= $branch_id ?>
          },
          success: function(response) {
            $('#showvehicleLIST').html(response);
          }
        });
      }

      function showvehicleFORMSTEP1(ID, vendor_id, vehicle_id) {
        $.ajax({
          type: 'post',
          url: 'engine/ajax/__ajax_add_vehicle_form.php?type=basic_info&ID=' + ID + '&vendor_id=' + vendor_id + '&vehicle_id=' + vehicle_id,
          success: function(response) {
            $('#showvehicleLIST').html('');
            // $('#add_vendor').hide();
            $('#showvehicleFORMSTEP1').html(response);
          }
        });
      }


      function showvehicleFORMSTEP2(ID, vendor_id, branch_id) {

        $.ajax({
          type: 'post',
          url: 'engine/ajax/__ajax_add_vehicle_form.php?type=image_upload&ID=' + ID + '&vendor_id=' + vendor_id + '&branch_id=' + branch_id,
          success: function(response) {
            $('#showvehicleLIST').html('');
            $('#showvehicleFORMSTEP1').html('');

            // $('#add_vendor').hide();
            $('#showvehicleFORMSTEP2').html(response);
          }
        });
      }
