...
$current_page = 'newhotspot.php'; // Set the current page variable
...
<body>
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <span id="showHOTSPOTLIST"></span>
                        <span id="showHOTSPOTFORM"></span>
                        <span id="showHOTSPOTPREVIEW"></span>
                    </div>
                    <!-- / Content -->
                    <!-- Footer -->
    ...
    <script>
        <?php if ($_GET['route'] == '') : ?>
            ...
            show_HOTSPOT_LIST();
        <?php elseif (($_GET['route'] == 'add' || $_GET['route'] == 'edit') && $_GET['formtype'] == 'hotspot_info') : ?>
            show_HOTSPOT_FORM('<?= $_GET['route']; ?>', '<?= $_GET['id']; ?>');
        <?php elseif ($_GET['route'] == 'preview'  && $_GET['formtype'] == 'preview') : ?>
            show_HOTSPOT_PREVIEW('<?= $_GET['route']; ?>', '<?= $_GET['id']; ?>');
        <?php endif; ?>

        function show_HOTSPOT_LIST() {
            $.ajax({
                type: 'post',
                url: 'engine/ajax/__ajax_hotspot_list.php',
                success: function(response) {
                    $('#showHOTSPOTLIST').html(response);
                    $('#showHOTSPOTFORM').html('');
                    $('#showHOTSPOTPREVIEW').html('');
                }
            });
        }

        function show_HOTSPOT_FORM(TYPE, ID) {
            $.ajax({
                type: 'post',
                url: 'engine/ajax/__ajax_add_hotspot_form.php?type=hotspot_info',
                data: {
                    ID: ID,
                    TYPE: TYPE
                },
                success: function(response) {
                    $('#showHOTSPOTLIST').html('');
        ...
        function show_HOTSPOT_PREVIEW(TYPE, ID) {
            $.ajax({
                type: 'post',
                url: 'engine/ajax/__ajax_add_hotspot_form.php?type=preview',
                data: {
                    ID: ID,
                    TYPE: TYPE
                },
                success: function(response) {
                    $('#showHOTSPOTLIST').html('');
                    $('#showHOTSPOTFORM').html('');
                    $('#showHOTSPOTPREVIEW').html(response);
                }
            });
        }
    </script>
