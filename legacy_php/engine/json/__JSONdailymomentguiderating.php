...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    $select_hotel_list_query = sqlQUERY_LABEL("SELECT `guide_feedback_ID`, `itinerary_plan_ID`, `itinerary_route_ID`, `guide_rating`, `guide_description` FROM `dvi_confirmed_itinerary_guide_feedback` WHERE `deleted` = '0' AND `itinerary_plan_ID` = '$itinerary_plan_ID' ORDER BY `guide_feedback_ID` DESC") or die("#1-UNABLE_TO_COLLECT_HOTEL_LIST:" . sqlERROR_LABEL());
    while ($fetch_list_data = sqlFETCHARRAY_LABEL($select_hotel_list_query)) :
        $counter++;
        $guide_feedback_ID = $fetch_list_data['guide_feedback_ID'];
        ...
        $datas .= '"guide_rating": "' . $guide_rating . ' Star",';
        $datas .= '"guide_description": "' . $guide_description . '",';
        $datas .= '"modify": "' . $guide_feedback_ID . '"';
        $datas .= " },";
