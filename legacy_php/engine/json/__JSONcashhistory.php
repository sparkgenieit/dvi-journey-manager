...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    $select_list_query = sqlQUERY_LABEL("SELECT `transaction_date`, `transaction_amount`, `transaction_type`,`transaction_id`,  `remarks` FROM `dvi_cash_wallet` WHERE  `deleted` = '0'{$filter_by_agent_id} ORDER BY `cash_wallet_ID` DESC") or die("#1-UNABLE_TO_COLLECT_HOTEL_LIST:" . sqlERROR_LABEL());
