...
// echo '"data":[';
$hid = $_GET['hotel_id'];
$room_type = $_GET['roomTypeFilter'];
//$state_id = $_GET['state_id'];
//$city_id = $_GET['city_id'];
//$hotel_cat_id = $_GET['hotel_cat_id'];

if ($hid != '' && $hid != 'null') :
    $filter_by_hotel = " and `hotel_id`='$hid' ";
else :
    $filter_by_hotel = "";
endif;

if ($room_type != '' && $room_type != 'null') :
    $filter_room_type = " and `room_type_id` = '$room_type' ";
else :
    $filter_room_type = "";
endif;
...
if ($hid != '0' || $hid != '') :
    ...
    $select_price_list_data = sqlQUERY_LABEL("SELECT
                    ...
                    `year`,  `room_type_id` AS `room_type_id`,
                    `month`,
                ...
                WHERE
                    `deleted` = '0' and `status` = '1' {$filter_by_hotel} {$filter_room_type}
                ...
                WHERE
                    `deleted` = '0' and `status` = '1' {$filter_by_hotel} ") or die("#1-UNABLE_TO_COLLECT_PRICE_LIST:" . sqlERROR_LABEL());
...
while ($row = sqlFETCHARRAY_LABEL($select_price_list_data)) :
    ...
    if ($type == 'rooms') :
        ...
        $title_name = getROOM_DETAILS($room_id, 'room_title');
        $room_type_ID = $row['room_type_id'];
        $hotel_id =  $row['hotel_id'];
        $hotel_name = getHOTEL_DETAIL($hotel_id, '', 'label') . " , " . getHOTEL_DETAIL($hotel_id, '', 'hotel_state_city');
        $showROOMTYPE_TITLE = $hotel_name . " - " . getROOMTYPE_DETAILS($room_type_ID, 'room_type_title');
    elseif ($type == 'amenities') :
        ...
        $title_name = getAMENITYDETAILS($row['room_id_R_amenities_id'], 'amenities_title');
    endif;
    ...
    for ($dayNumber = 1; $dayNumber <= $total_days_in_month; $dayNumber++) {
        ...
        if ($row["day_" . $dayNumber]) :
            ...
            if ($room_type == '' || $room_type == 'null') :
                $events[] = [
                    'title' => $showROOMTYPE_TITLE . " - " . $title_name . " - " . $global_currency_format . ' ' . $price_rate,
                    'room_id' => $room_id,
                    'start' => $startDate,
                    'end' => $startDate,
                    'allDay' => true,
