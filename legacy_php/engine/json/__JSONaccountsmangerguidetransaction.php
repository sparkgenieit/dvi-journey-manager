...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    $dvi_accounts_itinerary_guide_details = $_GET['ID'];

    $select_accountsmanagerLIST_query = sqlQUERY_LABEL("SELECT  `transaction_amount`, `transaction_date`, `transaction_done_by`, `mode_of_pay`, `transaction_utr_no`, `transaction_attachment` FROM `dvi_accounts_itinerary_guide_transaction_history` WHERE `deleted` = '0' AND `accounts_itinerary_guide_details_ID` = '$dvi_accounts_itinerary_guide_details' ORDER BY `accounts_itinerary_guide_transaction_ID` DESC") or die("#1-UNABLE_TO_COLLECT_COURSE_LIST:" . sqlERROR_LABEL());
