...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
  ...
  while ($getstatus_fetch = sqlFETCHARRAY_LABEL($getstatus_query)) :
    ...
    $select_accountsmanagerLIST_query = sqlQUERY_LABEL("SELECT `accounts_itinerary_activity_details_ID`, `itinerary_plan_ID`, `itinerary_route_ID`, `hotspot_ID`, `activity_ID`, `activity_amount`, `total_payable`, `total_paid`, `total_balance` FROM `dvi_accounts_itinerary_activity_details` WHERE `deleted` = '0' AND `activity_amount` > 0 {$combined_filters}") or die("#1-UNABLE_TO_COLLECT_COURSE_LIST:" . sqlERROR_LABEL());
    while ($fetch_list_data = sqlFETCHARRAY_LABEL($select_accountsmanagerLIST_query)) :
        ...
        $datas .= '"acc_activity_detail_id": "' . $accounts_itinerary_activity_details_ID . '",';
