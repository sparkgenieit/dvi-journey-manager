...
// Query to fetch local price book details without filters
$local_pricebook_query = "
    ...
    WHERE vlpb.vendor_id = $vendor_ID AND vlpb.vehicle_price_book_id IS NOT NULL AND ((date(vlpb.createdon) = CURDATE()) OR (date(vlpb.updatedon) = CURDATE()))
    ORDER BY v.vendor_name, vb.vendor_branch_name, vt.vehicle_type_title 
";

// Query to fetch outstation price book details without filters
$outstation_pricebook_query = "
    ...
    WHERE vopb.vendor_id = $vendor_ID AND vopb.vehicle_outstation_price_book_id IS NOT NULL AND (date(vopb.createdon) = CURDATE() OR date(vopb.updatedon) = CURDATE())
    ORDER BY vopb.createdon DESC
";
