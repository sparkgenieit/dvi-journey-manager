...
*/
    ...
    if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) :
            ...
            $room_quantities=$_SESSION['global_itinerary_plan_ID'];
            $vendor_id=$_SESSION['global_vendor_id'];
            $vehicle_type=$_SESSION['global_vehicle_type'];
            $vehicle_quantity=$_SESSION['global_vehicle_quantity'];

             foreach ($vehicle_types as $index => $vehicle_type):
                        $room_id = explode('_', $vehicle_type)[0];
                        $vehicle_type_id = explode('_', $vehicle_type)[1];
                        $room_qty = $room_quantities[$index];
                        $vehicle_type_name = getROOMTYPE_DETAILS($vehicle_type_id,'vehicle_type_title');
                        $room_title = getROOM_DETAILS($room_id,'room_title');
                        $room_details .= $vehicle_type_name . ' - ' . $room_title . ': ' . $room_qty;
                        $room_details.="\n";
                       endforeach;
