...
?>
        ...
        <script>
            $(document).ready(function() {
                ...
                // Event listener for itinerary_type change
                itinerarytypeSelectize.on('change', function(value) {
                    //If default type is selected
                    if (value == 1) {
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.no_routes_found) {
                                        //TOAST_NOTIFICATION('warning', 'No routes Found', 'Warning !!!', '', '', '', '', '', '', '', '', '');
                                        ...
                                        // TOAST_NOTIFICATION('warning', 'Unable to find the matching Route Locations', 'Warning !!!', '', '', '', '', '', '', '', '', '');
                                        const container = document.getElementById("NOMATCHINGROUTESMODAL");
                                        const modal = new bootstrap.Modal(container);
                                        modal.show();
                                    } else {
                                        $("#show_default_route_details").html(response.route_details);
                                    }
                                }
                            });
            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '.addNextDayPlan', function() {
                    let lastRow = $("#route_details_LIST tbody tr:last-child");
                ...
                $(document).on('click', '.deleteRow', function() {
                    ...
                    $("#route_details_LIST tbody tr").each(function(index, element) {
                        $(this).find(".day").text(`DAY ${index + 1}`);
                    });

                    // Calculate total .day count
                    let totalDays = $("#route_details_LIST tbody tr").length;
                    ...
                    let inputFields = $("#route_details_LIST tbody tr").find(
                    ...
                    let rows = $("#route_details_LIST tbody tr");
            ...
            function initializeAutocompleteForAllDays(dayNumber) {
                let numDays = parseInt($("#route_details_LIST tbody tr").length); // Get the total number of days
    ...
    `;
                $("#route_details_LIST tbody").append(newRow);
            }
    ...
    `;
                ...
                let lastRow = $("#route_details_LIST tbody tr:last-child");
                if (dayNum <= tripDay) {
                    lastRow = $("#route_details_LIST tbody tr").eq(tripDay - 1);
                }
            ...
            function collectAndSendRouteDetails() {
                ...
                $('#route_details_tbody .route_details').each(function() {
                    let $row = $(this);
