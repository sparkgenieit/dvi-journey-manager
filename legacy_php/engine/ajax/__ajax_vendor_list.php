            ...
            <div class="col-md-12">
                <div class="card p-0">
                    <div class="card-header pb-3 d-flex justify-content-between">
                        <div class="col-md-8">
                            <?php if ($logged_vendor_id == 0): ?>
                                <h5 class="card-title mb-3 mt-2">List of Vendor</h5>
                            <?php else: ?>
                                <h5 class="card-title mb-3 mt-2">Vendor Details</h5>
                            <?php endif; ?>
                        </div>

                        <?php if ($logged_user_level == 1): ?>
                            <div class="col-md-4 text-end">
                                <a href="newvendor.php?route=add&formtype=basic_info" id="add_vendor" class="btn btn-label-primary waves-effect me-2">+ Add vendor</a>
                            </div>
                        <?php endif; ?>
        ...
        <script>
            $(document).ready(function() {
                $('#vendor_LIST').DataTable({
                    ...
                    ajax: {
                        "url": "engine/json/__JSONvendor.php",
                        "type": "GET"
                    },
                    columns: [{
                            data: "count"
            ...
            function togglestatusITEM(STATUS_ID, VENDOR_ID) {
                if (VENDOR_ID) {
                    $.ajax({
                        type: "POST",
                        url: "engine/ajax/__ajax_manage_vendor.php?type=updatestatus",
                        data: {
                            VENDOR_ID: VENDOR_ID,
                            oldstatus: STATUS_ID
                        },
                        ...
                        success: function(response) {
                            if (response.result_success == true) {
                                ...
                                TOAST_NOTIFICATION('success', 'Vendor Status Updated Successfully.', 'Success !!!', '', '', '', '', '', '', '', '', '');
                            } else {
                                TOAST_NOTIFICATION('success', 'Unable to update vendor status.', 'Success !!!', '', '', '', '', '', '', '', '', '');
                            }
                        }
                    });
                }
