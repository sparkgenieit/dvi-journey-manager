...
?>
        ...
        <div class="col-md-12">
            <div class="card p-0">
                <div class="card-header pb-3 d-flex justify-content-between">
                    <div class="col-md-8">
                        <h5 class="card-title mb-3 mt-2">List of Guide</h5>
                    </div>
                    <div class="col-md-4 text-end">
                        <a href="guide.php?route=add&formtype=basic_info" id="add_guide" class="btn btn-label-primary waves-effect me-2">+ Add Guide</a>
                    </div>
                </div>
                <div class="card-body dataTable_select text-nowrap">
                    <div class="text-nowrap overflow-hidden table-bordered">
                        <table id="guide_LIST" class="table table-hover">
                            <thead>
                                <tr>
                                    <th>S.No</th>
                                    <th>Action</th>
    ...
    <!-- Delete Guide modal -->
    <div class="modal fade" id="showGUIDEDELETEMODAL" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#guide_LIST').DataTable({
                ...
                ajax: {
                    "url": "engine/json/__JSONguide.php",
                    "type": "GET"
                },
                columns: [{
                        data: "count"
                    ...
                    {
                        data: "guide_primary_mobile_number"
                    }, //3
                    {
                        data: "guide_email"
                    }, //4
        ...
        function togglestatusITEM(STATUS_ID, GUIDE_ID) {

            if (GUIDE_ID) {
                $.ajax({
                    type: "POST",
                    url: "engine/ajax/__ajax_manage_guide.php?type=update_guide_status",
                    data: {
                        GUIDE_ID: GUIDE_ID,
                        STATUS_ID: STATUS_ID
                    },
        ...
        function showGUIDEDELETEMODAL(ID) {
            $('.receiving-delete-form-guide-data').load('engine/ajax/__ajax_manage_guide.php?type=guide_delete&ID=' + ID, function() {
                const container = document.getElementById("showGUIDEDELETEMODAL");
        ...
        function confirmGUIDEDELETE(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/__ajax_manage_guide.php?type=confirm_guide_delete",
                data: {
                    _ID: ID
                },
                dataType: 'json',
                success: function(response) {
                    if (response.result == true) {
                        $('#guide_LIST').DataTable().ajax.reload();
                        $('#showGUIDEDELETEMODAL').modal('hide');
