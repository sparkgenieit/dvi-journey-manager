...
*/
    ...
    if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) :
        ...
        $global_hidden_itinerary_route_date_val = $_SESSION['global_hidden_itinerary_route_date_val'];
        $global_hotel_id_val = $_SESSION['global_hotel_id_val'];
       ...
       /* $combined_meal_plan_details = "";
        ...
        for ($j = 0; $j < count($itinerary_plan_hotel_details_ID_array1); $j++):
            ...
            if ($j == 0) :
                $check_in_date_time = date('M d, Y', strtotime($hidden_itinerary_route_date_val)) . ' ' . $check_in_time;
            elseif ($j == (count($itinerary_plan_hotel_details_ID_array1) - 1)):
                $check_out_date_time = date('M d, Y', strtotime($hidden_itinerary_route_date_val . ' +1 day')) . ' ' . $check_out_time;
            endif;
...
*/
      ...
      // NEW: accumulators for segmented check-ins / check-outs
        ...
        // (Optional) If you still need the old multi-line HTML vars elsewhere, you can keep building them:
        // $check_in_date_time  = implode('<br>', array_map(fn($s) => $format_range($s['in_date'],$s['in_time']),  $stay_segments));
        // $check_out_date_time = implode('<br>', array_map(fn($s) => $format_range($s['out_date'],$s['out_time']), $stay_segments));
