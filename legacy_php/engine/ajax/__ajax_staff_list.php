...
if ($logged_user_level == '4') :
    ...
    $select_subscribed_query = sqlQUERY_LABEL("SELECT `subscription_plan_ID`, `staff_count`,`additional_staff_count` FROM `dvi_agent_subscribed_plans` WHERE `deleted` = '0'  AND `agent_ID` = '$logged_agent_id'") or die("#1-UNABLE_TO_COLLECT_SUBSCRIBED_PLAN:" . sqlERROR_LABEL());
    while ($fetch_data = sqlFETCHARRAY_LABEL($select_subscribed_query)) :
        $subscription_plan_ID = $fetch_data['subscription_plan_ID'];
        $plan_staff_count = $fetch_data['staff_count'];
        $additional_staff_count = $fetch_data['additional_staff_count'];
        $staff_count = $plan_staff_count + $additional_staff_count;
    endwhile;
    ...
    if ($current_staff_count >= $staff_count) :
        $add_onclick = "onclick='EXCEEDED_THE_STAFF_LIMIT(" . $subscription_plan_ID . "," . $logged_agent_id . ")'";
        $add_btn_label = "Request Additional Staff";
    else :
        $add_onclick = "onclick='show_staff_add_FORM()'";
        $add_btn_label = "Add Staff";
    endif;
else :
    $add_onclick = "onclick='show_staff_add_FORM()'";
    $add_btn_label = "Add Staff";
            ...
            <div class="col-md-12">
                <div class="card p-0">
                    <div class="card-header pb-3 d-flex justify-content-between">
                        <div class="col-md-8">
                            <h5 class="card-title mb-3 mt-2">List of Staff</h5>
                        </div>
                        <div class="col-md-4 text-end">

                            <button type="button" <?= $add_onclick; ?> id="add_staff" class="btn btn-label-primary waves-effect me-2">+ <?= $add_btn_label; ?></button>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#staff_LIST').DataTable({
                    ...
                    ajax: {
                        "url": "engine/json/__JSONstaff.php?agent_ID=<?= $logged_agent_id; ?>",
                        "type": "GET"
                    },
                    columns: [{
                            data: "count"
                    ...
                    columnDefs: [
                        ...
                        {
                            <?php if ($logged_user_level != '4') : ?> "targets": 6,
                            <?php else : ?> "targets": 5,
                            <?php endif; ?> "data": "status",
                        ...
                        {
                            <?php if ($logged_user_level != '4') : ?> "targets": 8,
                            <?php else : ?> "targets": 7,
                            <?php endif; ?> "visible": false,
                            "data": "status_label",
                            "render": function(data, type, row, full) {
                                if (data == 1) {
                                    return "Active";
            ...
            function show_staff_add_FORM() {
                location.assign('newstaff.php?route=add&formtype=basic_info');
            }


            function EXCEEDED_THE_STAFF_LIMIT(SUBSCRIPTION_PLAN_ID, AGENT_ID) {
                $('#add_staff').prop('disabled', true);
                $.ajax({
                    type: "POST",
                    url: "engine/ajax/__ajax_manage_staff.php?type=update_additional_staff_request",
                    data: {
                    ...
                    success: function(response) {
                        ...
                        if (response.result_success == true) {
                            TOAST_NOTIFICATION('success', 'Your request has been sent to the Travel Expert. <br/> You will be contacted shortly.', 'Success !!!', '', '', '', '', '', '', '', '', '');
                        } else {
                            TOAST_NOTIFICATION('error', 'There was an error sending your request to the Travel Expert. <br/> Please try again shortly.', 'Error !!!', '', '', '', '', '', '', '', '', '');
                        }
                    }
                });
            }
