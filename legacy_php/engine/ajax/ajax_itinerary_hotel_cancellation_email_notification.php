...
*/
    ...
    if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) :
        ...
        $room_id = $_SESSION['global_room_id'];
        $room_type_title = $_SESSION['global_room_type_title'];
        $room_qty = $_SESSION['global_room_qty'];
        $check_in_time = getROOM_DETAILS($room_id, 'check_in_time');
        $check_out_time = getROOM_DETAILS($room_id, 'check_out_time');
        $hotel_check_in = dateformat_datepicker($itinerary_route_date) . ' ' . date('h:i A', strtotime($check_in_time));
        $hotel_check_out = dateformat_datepicker($itinerary_route_date) . ' ' . date('h:i A', strtotime($check_out_time));

        $hotel_email = getHOTEL_DETAIL($hotel_id, '', 'hotel_email');
        // Convert the comma-separated email IDs into an array
        $global_hotel_email_array = explode(',', $hotel_email);
        ...
        if ($travel_expert_id) :
            ...
            $email_to = [$travel_expert_staff_email, $global_hotel_email_array, $global_default_hotel_email_array, $admin_emailid];
        else :
            $email_to = [$admin_emailid, $global_hotel_email_array, $global_default_hotel_email_array];
        endif;
        ...
        $current_YEAR = date('Y');
        $description = "This is an email to inform you that a hotel has been cancelled in a specific itinerary.";
...
<tr>
  ...
  ">
    ...
    <thead>
      ...
      <tr>
    <td colspan="2" style="padding:7px; font-size:14px; text-align:left;">
        <span style="background-color: yellow; padding: 2px 5px;">
            Note: The room and service associated with the hotel have also been cancelled.
        </span>
    </td>
</tr>
    </thead>
        ...
        $subject = "$site_title - Hotel Booking Has Been Successfully Cancelled #$confirmed_itinerary_quote_ID";
