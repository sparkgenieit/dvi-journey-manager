...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'show_form') :
        ...
        //get from cnf iti vendor vehicles

        $select_itineary_vehicle_details = sqlQUERY_LABEL("SELECT `vehicle_type_id`, SUM(`vehicle_count`) AS vehicle_count FROM `dvi_confirmed_itinerary_plan_vehicle_details`  WHERE `itinerary_plan_id` = '$itinerary_plan_ID' AND  `cancellation_status` = '0' AND  `added_via_amendment` = '1' AND  `status` = '1' and `deleted` = '0' GROUP BY `vehicle_type_id`") or die("#1-UNABLE_TO_COLLECT_DETAILS:" . sqlERROR_LABEL());
        ...
        if ($total_no_of_vehicle_count > 0) :
            while ($fetch_vehicle_data = sqlFETCHARRAY_LABEL($select_itineary_vehicle_details)) :
                $get_unique_vehicle_type[] = $fetch_vehicle_data['vehicle_type_id'];
...
?>
        ...
        <div class="card p-4 mt-3">
            <?php foreach ($get_unique_vehicle_type as $vehicle_type) :
                ...
                //get from cnf iti vendor vehicles
                $select_itineary_vehicle_list_query = sqlQUERY_LABEL("SELECT `vehicle_type_id` FROM `dvi_confirmed_itinerary_plan_vehicle_details` WHERE `itinerary_plan_id` = '$itinerary_plan_ID' and `vehicle_type_id` = '$vehicle_type' AND  `cancellation_status` = '0' AND  `added_via_amendment` = '1' AND `deleted` = '0' and `status` = '1' GROUP BY `vehicle_type_id`") or die("#1-UNABLE_TO_COLLECT_VEHICLE_DETAILS_LIST:" . sqlERROR_LABEL());
                        ...
                        <tbody class="table-borderless" id="itineary_vendor_LIST_<?= $vendor_count; ?>" width="100%">
                            ...
                            if ($select_itinerary_plan_vendor_count > 0) :
                                while ($fetch_eligible_vendor_data = sqlFETCHARRAY_LABEL($select_itinerary_plan_vendor_data)) :
                                    $vendor_counter++;
                                    $itinerary_plan_vendor_eligible_ID = $fetch_eligible_vendor_data['confirmed_itinerary_plan_vendor_eligible_ID'];
                            ...
                            ?>
                                    <tr class="cursor-pointer <?= $table_border_active; ?>" <?= $table_border_style; ?> data-vendor-counter="<?= $vendor_counter; ?>">
                                        ...
                                        <td colspan="2"><?= $total_vehicle_qty; ?> x <?= general_currency_symbol . ' ' . number_format($vehicle_grand_total, 2); ?></td>
                                    ...
                                    <?php if ($logged_user_level != 4) : ?>
                                        ...
                                        if ($select_itinerary_plan_vendor_vehicle_summary_count > 0) :
                                            ...
                                            while ($fetch_eligible_vendor_vehicle_data = sqlFETCHARRAY_LABEL($select_itinerary_plan_vendor_vehicle_summary_data)) :
                                                ...
                                                $itinerary_plan_vendor_vehicle_details_ID = $fetch_eligible_vendor_vehicle_data['confirmed_itinerary_plan_vendor_vehicle_details_ID'];
                                                $itinerary_plan_vendor_eligible_ID = $fetch_eligible_vendor_vehicle_data['confirmed_itinerary_plan_vendor_eligible_ID'];
                                                ...
                                                $get_total_outstation_trip = round(get_ASSIGNED_VEHICLE_FOR_CNFITINEARY_PLAN_DETAILS($itinerary_plan_vendor_eligible_ID, $itinerary_plan_ID, 'get_total_outstation_trip'));

                                                $get_total_local_trip = round(get_ASSIGNED_VEHICLE_FOR_CNFITINEARY_PLAN_DETAILS($itinerary_plan_vendor_eligible_ID, $itinerary_plan_ID, 'get_total_local_trip'));
                                        ...
                                        ?>
                                            ...
                                            <tr class="d-none vendor_details_<?= $vendor_counter; ?>">
                                                ...
                                                <td></td>
                                                <td class="text-end"><?= number_format(get_ASSIGNED_VEHICLE_FOR_CNFITINEARY_PLAN_DETAILS($itinerary_plan_vendor_eligible_ID, $itinerary_plan_ID, 'get_total_pickup_km'), 2); ?></td>
                                            </tr>
                                            <tr class="d-none vendor_details_<?= $vendor_counter; ?>">
                                                <td colspan="5" class="text-end">Total Pickup Duration</td>
                                                <td></td>
                                                <td class="text-end"><?= formatTimeDuration(get_ASSIGNED_VEHICLE_FOR_CNFITINEARY_PLAN_DETAILS($itinerary_plan_vendor_eligible_ID, $itinerary_plan_ID, 'get_total_pickup_duration')); ?></td>
                                            </tr>
                                            <tr class="d-none vendor_details_<?= $vendor_counter; ?>">
                                                <td colspan="5" class="text-end">Total Drop KM</td>
                                                <td></td>
                                                <td class="text-end"><?= number_format(get_ASSIGNED_VEHICLE_FOR_CNFITINEARY_PLAN_DETAILS($itinerary_plan_vendor_eligible_ID, $itinerary_plan_ID, 'get_total_drop_km'), 2); ?></td>
                                            </tr>
                                            <tr class="d-none vendor_details_<?= $vendor_counter; ?>">
                                                <td colspan="5" class="text-end">Total Drop Duration</td>
                                                <td></td>
                                                <td class="text-end"><?= formatTimeDuration(get_ASSIGNED_VEHICLE_FOR_CNFITINEARY_PLAN_DETAILS($itinerary_plan_vendor_eligible_ID, $itinerary_plan_ID, 'get_total_drop_duration')); ?></td>
                                            </tr>
                                            <tr class="d-none vendor_details_<?= $vendor_counter; ?>">
            ...
            <div class="row text-center mt-4">
                <div class="col-12">
                    <button id="vendor_vehicle_type_cancel_confirm" type="button" class="btn btn-outline-secondary me-2" data-bs-dismiss="modal">Close</button>
                    <button id="vendor_vehicle_type_submit_confirm" type="submit" class="btn btn-success" <?= $btn_attr; ?>>Confirm </button>
                </div>
            </div>
        ...
        <script>
            ...
            //             $.ajax({
            //                 url: 'engine/ajax/ajax_manage_confirmed_itinerary_cancellation_details.php?type=confirm_modify_itineary_plan_vehicle_vendor',
            //                 method: 'POST',
            //                 data: {
            ...
            //                     } else {
            //                         if(response.update_vehicle_vendor_success){
            ...
            $(document).ready(function() {
                ...
                // Disable the submit and close buttons by default
                $('#vendor_vehicle_type_submit_confirm').prop('disabled', true);
                $('#vendor_vehicle_type_cancel_confirm').prop('disabled', true);
                ...
                $('.vehicle-checkbox').change(function() {
                    ...
                    // Check if the total count for any vehicle type exceeds the required count
                    var allTypesHaveSelection = true;
                    $.each(vehicleTypeCounts, function(type, count) {
                        if (count == 0) {
                            allTypesHaveSelection = false;
                        }
                        if (count > parseInt($('.vehicle-checkbox[data-vehicle-type="' + type + '"]').data('required-count'))) {
                            TOAST_NOTIFICATION('error', 'Maximum allowed vehicles reached for ' + type + ' !!!', 'Error !!!', '', '', '', '', '', '', '', '', '');
                        }
                    ...
                    if (allTypesHaveSelection) {
                        $('#vendor_vehicle_type_submit_confirm').prop('disabled', false);
                        $('#vendor_vehicle_type_cancel_confirm').prop('disabled', false);
                    } else {
                        $('#vendor_vehicle_type_submit_confirm').prop('disabled', true);
                        $('#vendor_vehicle_type_cancel_confirm').prop('disabled', true);
                    }
                ...
                $('#vendor_vehicle_type_submit_confirm').click(function() {
                    $.ajax({
                        url: 'engine/ajax/ajax_manage_confirmed_itinerary_cancellation_details.php?type=delete_itinerary_plan_vendor_vehicle',
                        method: 'POST',
                        data: {
                            itinerary_plan_id: '<?= $itinerary_plan_ID; ?>',
                        },
                        ...
                        success: function(response) {
                            if (!response.update_vehicle_vendor_success) {
                                TOAST_NOTIFICATION('error', 'Unable to Delete the Vehicle Vendor !!!', 'Error !!!', '', '', '', '', '', '', '', '', '');
                            } else {
                                if (response.update_vehicle_vendor_success) {
                                    $('#showVENDORADDFORMDATA').modal('hide');
                                    TOAST_NOTIFICATION('success', 'Successfully updated the Vendor !!!', 'Success !!!', '', '', '', '', '', '', '', '', '');
                                    setTimeout(() => {
                                        location.reload();
                                    }, 2000);
                                }
            ...
            //     $('.receiving-vehicle-modal-info-form-data').load('engine/ajax/ajax_confirmed_itineary_vehicle_amedment_details.php?type=show_form&itinerary_plan_vendor_vehicle_details_ID=' + itinerary_plan_vendor_vehicle_details_ID + '&itinerary_plan_vendor_eligible_ID=' + itinerary_plan_vendor_eligible_ID + '&itinerary_plan_ID=' + itinerary_plan_ID, function() {
