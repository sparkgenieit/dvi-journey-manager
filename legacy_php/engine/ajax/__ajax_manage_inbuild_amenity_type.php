...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'add') :
        ...
        if (empty($_POST['inbuilt_amenity_title'])) :
            $errors['inbuilt_amenity_title_required'] = '<div class="alert alert-left alert-warning  mt-3" role="alert"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.com/svgjs" width="24" height="24" x="0" y="0" viewBox="0 0 128 128" style="enable-background:new 0 0 512 512" xml:space="preserve" class="mx-2"><g><path fill="#f16a1b" d="M57.362 26.54 20.1 91.075a7.666 7.666 0 0 0 6.639 11.5h74.518a7.666 7.666 0 0 0 6.639-11.5L70.638 26.54a7.665 7.665 0 0 0-13.276 0z" data-original="#ffb400" opacity="1"></path><g fill="#fcf4d9"><rect width="9.638" height="29.377" x="59.181" y="46.444" rx="4.333" fill="#fcf4d9" data-original="#fcf4d9"></rect><circle cx="64" cy="87.428" r="4.819" fill="#fcf4d9" data-original="#fcf4d9"></circle></g></g></svg>Please Enter inbuild amenity title !!!</div>';

        // elseif (empty($_POST['availability'])) :
        //     $errors['amenity_availability_required'] = '<div class="alert alert-left alert-warning  mt-3" role="alert"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.com/svgjs" width="24" height="24" x="0" y="0" viewBox="0 0 128 128" style="enable-background:new 0 0 512 512" xml:space="preserve" class="mx-2"><g><path fill="#f16a1b" d="M57.362 26.54 20.1 91.075a7.666 7.666 0 0 0 6.639 11.5h74.518a7.666 7.666 0 0 0 6.639-11.5L70.638 26.54a7.665 7.665 0 0 0-13.276 0z" data-original="#ffb400" opacity="1"></path><g fill="#fcf4d9"><rect width="9.638" height="29.377" x="59.181" y="46.444" rx="4.333" fill="#fcf4d9" data-original="#fcf4d9"></rect><circle cx="64" cy="87.428" r="4.819" fill="#fcf4d9" data-original="#fcf4d9"></circle></g></g></svg>Please Enter amenity availability !!!</div>';
        endif;
        //SANITIZE
        $sanitize_inbuilt_amenity_title = $validation_globalclass->sanitize($_POST['inbuilt_amenity_title']);
        // $sanitize_inbuilt_amenity_availability = $validation_globalclass->sanitize($_POST['inbuilt_amenity_availability']);
        $hiddenAMENITY_TYPE_ID = $validation_globalclass->sanitize($_POST['hiddeninbuilt_amenity_type_id']);
        ...
        if (!empty($errors)) :
            ...
            $arrFields = array('`inbuilt_amenity_type_id`','`inbuilt_amenity_title`', '`createdby`', '`status`');
            $arrValues = array("$hiddenAMENITY_TYPE_ID","$sanitize_inbuilt_amenity_title", "$logged_user_id", "1");
       
            if ($hiddenAMENITY_TYPE_ID != '' && $hiddenAMENITY_TYPE_ID != 0 && (!empty($hiddenAMENITY_TYPE_ID))) :
                $sqlwhere = " `inbuilt_amenity_type_id` = '$hiddenAMENITY_TYPE_ID' ";

                //UPDATE HOTEL CATEGORY INFO
                if (sqlACTIONS("UPDATE", "dvi_inbuilt_amenities", $arrFields, $arrValues, $sqlwhere)) :
                    //SUCCESS
                ...
                if (sqlACTIONS("INSERT", "dvi_inbuilt_amenities", $arrFields, $arrValues, '')) :
                    //SUCCESS
    ...
    elseif ($_GET['type'] == 'updatestatus') :
        ...
        $AMENITY_TYPE_ID = $_POST['AMENITY_TYPE_ID'];
        $STATUS_ID = $_POST['STATUS_ID'];

        //SANITIZE
        $AMENITY_TYPE_ID = $validation_globalclass->sanitize($AMENITY_TYPE_ID);
        ...
        $sqlwhere = " `inbuilt_amenity_type_id` = '$AMENITY_TYPE_ID' ";
    ...
    elseif ($_GET['type'] == 'delete') :
        ...
        // $select_hotel_id_already_used = sqlQUERY_LABEL("SELECT COUNT(`hotel_id`) AS TOTAL_USED_COUNT FROM `dvi_hotel` WHERE `status` = '1' and `hotel_category` = '$ID' and `deleted` = '0'") or die("#1-UNABLE_TO_COLLECT_DETAILS:" . sqlERROR_LABEL());
        // while ($fetch_hotel_used_data = sqlFETCHARRAY_LABEL($select_hotel_id_already_used)) :
        ...
        <div class="modal-body">
            <div class="row">
                ...
                <div class="text-center pb-0">
                    ...
                    <button type="submit" onclick="confirmINBUILDAMENITYTYPEDELETE('<?= $ID; ?>');" class="btn btn-danger">Delete</button>
                </div>
...
<?php
    elseif ($_GET['type'] == 'confirmdelete') :
        ...
        $delete_inbuildamenity = sqlQUERY_LABEL("UPDATE `dvi_inbuilt_amenities` SET `deleted` = '1', `updatedon` = '" . date('Y-m-d H:i:s') . "' WHERE `inbuilt_amenity_type_id` = '$_ID'") or die("#1-UNABLE_TO_DELETE_GST:" . sqlERROR_LABEL());

        if ($delete_inbuildamenity) :
