...
?>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card p-0">
                <div class="card-header pb-3 d-flex justify-content-between align-items-center flex-wrap">

                    <div class="col-md-3">
                        <h5 class="card-title mb-3 mt-2">List of Location</h5>
                    </div>
                    <div class="col-md-9 text-end">
                        <a href="javascript:void(0)" class="btn btn-label-primary waves-effect" onclick="showLOCATIONMODAL();" data-bs-dismiss="modal">+ Add Locations</a>
                        <a href="javascript:void(0)" class="btn btn-label-primary waves-effect" onclick="updateLOCATIONNAMEMODAL();" data-bs-dismiss="modal"><i class="ti ti-pencil ti-xs me-1"></i> Modify Location Name</a>
                        <a href="javascript:void(0)" class="btn btn-label-primary waves-effect" onclick="deleteLOCATIONNAMEMODAL();" data-bs-dismiss="modal"><i class="ti ti-trash ti-xs me-1"></i> Delete Location Name</a>
                        <div class="btn-group" id="dropdown-icon-demo">
                            <a href="location_toll_charge_pricebook.php?route=import" class="btn btn-label-primary waves-effect" style="padding-top: 7px;padding-bottom: 7px;"><i class="ti ti-upload ti-xs me-1"></i> Toll Charges</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body dataTable_select text-nowrap">
                    <div class="text-nowrap table-responsive table-bordered">
                        <table class="table table-hover " id="location_LIST">
                            <thead>
                                <tr>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#source_location').change(function() {
                ...
                location_LIST.ajax.url('engine/json/__JSONlocation.php?source_location=' + source_location +
                    '&destination_location=' + destination_location).load();
            ...
            var location_LIST = $('#location_LIST').DataTable({
                ...
                processing: true, // Enable processing indication
                ajax: {
                    "url": "engine/json/__JSONlocation.php",
                    "type": "GET",
                    "data": function(d) {
                ...
                columnDefs: [{
                    ...
                    "render": function(data, type, full) {
                        return '<div class="flex align-items-center text-center list-user-action">' +
                            '<a class="btn btn-sm btn-icon text-primary flex-end" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Preview" href="locations.php?route=preview&formtype=preview&id=' +
                            data + '" style="margin-right: 10px;">' +
                            '<span class="btn-inner"><svg style="width:26px; height:26px;color:#888686;" fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">' +
                            ...
                            '<a class="btn btn-sm btn-icon text-primary flex-end" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit" onclick="showLOCATIONMODAL(' +
                            data + ');" href="#" style="margin-right: 10px;">' +
                            '<span class="btn-inner"><svg style="width: 22px; height: 22px;" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                            ...
                            '<path d="M15.1655 4.60254L19.7315 9.16854" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></a>' +
                            '<a class="btn btn-sm btn-icon text-danger flex-end" href="javascript:void(0);" onclick="showDELETELOCATIONMODAL(' +
                            data + ');" aria-label="Delete" data-bs-original-title="Delete">' +
        ...
        //ADD & EDIT MODAL
        function showLOCATIONMODAL(LOCATION_ID = "") {
            $('.receiving-location-form-data').load('engine/ajax/__ajax_add_location.php?type=show_form&LOCATION_ID=' +
                LOCATION_ID + '',
                function() {
                    const container = document.getElementById("addLOCATIONFORM");
                    const modal = new bootstrap.Modal(container);
                    modal.show();
                    if (LOCATION_ID != "") {
                        $('#LOCATIONFORMLabel').html('Update Location');
                    } else {
                        $('#LOCATIONFORMLabel').html('Add Location');
                    }
                });
        }

        function updateLOCATIONNAMEMODAL(LOCATION_ID = "") {
            $('.receiving-location-form-data').load('engine/ajax/__ajax_modify_location_name.php?type=show_form',
                function() {
                    const container = document.getElementById("addLOCATIONFORM");
                    const modal = new bootstrap.Modal(container);
                    modal.show();
                    $('#LOCATIONFORMLabel').html('Modify Location Name');
                });
        }

        function deleteLOCATIONNAMEMODAL(LOCATION_ID = "") {
            $('.receiving-delete-location-form-data').load('engine/ajax/__ajax_delete_location_name.php?type=show_form',
                function() {
                    const container = document.getElementById("deleteLOCATIONFORM");
                    const modal = new bootstrap.Modal(container);
                    modal.show();
                    $('#DELETELOCATIONFORMLabel').html('Delete Location Name');
                });
        }

        //SHOW DELETE POPUP
        function showDELETELOCATIONMODAL(ID) {
            $('.receiving-confirm-delete-location-form-data').load(
                'engine/ajax/__ajax_manage_location.php?type=delete_location&ID=' + ID,
                function() {
                    const container = document.getElementById("confirmDELETELOCATIONINFODATA");
                    const modal = new bootstrap.Modal(container);
                    modal.show();
                });
        }
        ...
        function confirmLOCATIONDELETE(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/__ajax_manage_location.php?type=confirm_delete_location",
                data: {
                    _ID: ID
                },
                dataType: 'json',
                success: function(response) {
                    if (response.result == true) {
                        ...
                        $('#confirmDELETELOCATIONINFODATA').modal('hide');
        ...
        // Setup for source location autocomplete
        /* var source_location = {
             url: function(phrase) {
                 return "engine/json/__JSONsearchsourcelocation.php?phrase=" + encodeURIComponent(phrase) +
         ...
         $("#source_location").easyAutocomplete(source_location);*/

        // Function to setup destination location autocomplete based on selected source location
        /* function get_destination_location_details() {
             var source_location = $("#source_location").val();
             var destination_location = {
                 url: function(phrase) {
                     return "engine/json/__JSONsearchsourcelocation.php?phrase=" + encodeURIComponent(phrase) +
                         "&format=json&type=destination&source_location=" + encodeURIComponent(source_location);
                 },
             ...
             $("#destination_location").easyAutocomplete(destination_location);
         }*/
    </script>
<?php
endif;
