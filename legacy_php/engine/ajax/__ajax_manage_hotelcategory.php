...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'add') :
        ...
        if (!empty($errors)) :
            ...
            $arrFields = array('`hotel_category_code`', '`hotel_category_title`', '`createdby`', '`status`');
            $arrValues = array("$sanitize_hotel_category_code", "$sanitize_hotel_category_title", "$logged_user_id", "1");

            if ($hiddenHOTEL_CATEGORY_ID != '' && $hiddenHOTEL_CATEGORY_ID != 0 && (!empty($hiddenHOTEL_CATEGORY_ID))) :
                $sqlwhere = " `hotel_category_id` = '$hiddenHOTEL_CATEGORY_ID' ";

                //UPDATE HOTEL CATEGORY INFO
                if (sqlACTIONS("UPDATE", "dvi_hotel_category", $arrFields, $arrValues, $sqlwhere)) :
                    //SUCCESS
                ...
                if (sqlACTIONS("INSERT", "dvi_hotel_category", $arrFields, $arrValues, '')) :
                    //SUCCESS
                    $response['result'] = true;
    ...
    elseif ($_GET['type'] == 'updatestatus') :
        ...
        //SANITIZE
        $HOTEL_CATEGORY_ID = $validation_globalclass->sanitize($HOTEL_CATEGORY_ID);
        ...
        $sqlwhere = " `hotel_category_id` = '$HOTEL_CATEGORY_ID' ";
...
<?php
    elseif ($_GET['type'] == 'confirmdelete') :
        ...
        $delete_HOTEL = sqlQUERY_LABEL("UPDATE `dvi_hotel_category` SET `deleted` = '1', `updatedon` = '" . date('Y-m-d H:i:s') . "' WHERE `hotel_category_id` = '$_ID'") or die("#1-UNABLE_TO_DELETE_HOTEL:" . sqlERROR_LABEL());
        if ($delete_HOTEL) :
