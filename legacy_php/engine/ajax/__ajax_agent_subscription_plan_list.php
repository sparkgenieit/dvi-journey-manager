...
?>
        ...
        <div class="col-md-12">
            <div class="card p-0">
                <div class="card-header pb-3 d-flex justify-content-between">
                    <h5 class="card-title mb-3">List of Agent Subscription Plan</h5>
                    <a href="agent_subscription_plan.php?route=add&formtype=basic_info" id="add_guide" class="btn btn-label-primary waves-effect me-2">+ Add Subscription Plan</a>
                </div>
                <div class="card-body dataTable_select text-nowrap">
                    <div class="text-nowrap table-responsive table-bordered">
                        <table class="table table-hover" id="agent_subscription_plan_LIST">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">Itinerary count</th>
                                    <th scope="col">Cost (₹)</th>
                                    <th scope="col">Joining Bonus (₹)</th>
                                    <th scope="col">Itinerary Cost (₹)</th>
    ...
    <!-- Delete Guide modal -->
    <div class="modal fade" id="showAGENT_SUBSCRIBE_PLAN_DELETEMODAL" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content receiving-delete-form-guide-data">
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            $('#agent_subscription_plan_LIST').DataTable({
                dom: 'Blfrtip',
                "bFilter": true,
                buttons: [{
                        extend: 'copy',
                ...
                ajax: {
                    "url": "engine/json/__JSONagent_subscription_plan.php",
                    "type": "GET"
                },
                columns: [{
                        data: "count"
                    ...
                    {
                        data: "agent_subscription_plan_title"
                    }, //3
                    {
                        data: "itinerary_allowed"
                    }, //4
                    {
                ...
                columnDefs: [{
                    ...
                    },
                    {
                        ...
                        "render": function(data, type, row, full) {
                            switch (data) {
                                case '1':
                                    return '<div class="media-body text-start switch-sm"><label class="switch mt-0"><input type="checkbox" class="switch-input" checked onChange="recommendedstatusITEM(' + data + ',' + row.modify + ');"><span class="switch-toggle-slider"><span class="switch-on"></span></span></label></div>';
                                    break;
                                case '0':
                                    return '<div class="media-body text-start switch-sm"><label class="switch mt-0"><input type="checkbox" class="switch-input"  onChange="recommendedstatusITEM(' + data + ',' + row.modify + ');"><span class="switch-toggle-slider"><span class="switch-on"></span></span></label></div>';
                                    break;
                            }
                        }
                    }, {
        ...
        function togglestatusITEM(STATUS_ID, AGENT_SUBSCRIPTION_ID) {

            if (AGENT_SUBSCRIPTION_ID) {
                $.ajax({
                    type: "POST",
                    url: "engine/ajax/__ajax_manage_agent_subscription_plan.php?type=update_status",
                    data: {
                    ...
                    success: function(response) {
                        if (response.result_success == true) {
                            $('#agent_subscription_plan_LIST').DataTable().ajax.reload();
        ...
        function recommendedstatusITEM(RECOMMENDED_STATUS_ID, AGENT_SUBSCRIPTION_ID) {

            if (AGENT_SUBSCRIPTION_ID) {
                $.ajax({
                    type: "POST",
                    url: "engine/ajax/__ajax_manage_agent_subscription_plan.php?type=recommended_status",
                    data: {
                        AGENT_SUBSCRIPTION_ID: AGENT_SUBSCRIPTION_ID,
                        RECOMMENDED_STATUS_ID: RECOMMENDED_STATUS_ID
                    },
                    dataType: 'json',
                    success: function(response) {
                        if (response.result_success == true) {
                            $('#agent_subscription_plan_LIST').DataTable().ajax.reload();
                            // Status updated successfully
                            TOAST_NOTIFICATION('success', 'Recommended Status updated Successfully', 'Success !!!', '', '', '', '', '', '', '', '', '');
                        } else {
                            TOAST_NOTIFICATION('error', 'Unable to update status', 'Error !!!', '', '', '', '', '', '', '', '', '');
                        }
                    }
        ...
        //SHOW DELETE GUIDE POPUP
        function showAGENT_SUBSCRIBE_PLAN_DELETEMODAL(ID) {
            $('.receiving-delete-form-guide-data').load('engine/ajax/__ajax_manage_agent_subscription_plan.php?type=update_delete&ID=' + ID, function() {
                const container = document.getElementById("showAGENT_SUBSCRIBE_PLAN_DELETEMODAL");
        ...
        function confirmAGENT_SUBSCRIBE_PLAN(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/__ajax_manage_agent_subscription_plan.php?type=confirm_update_delete",
                data: {
                    _ID: ID
                },
                dataType: 'json',
                success: function(response) {
                    if (response.result == true) {
                        $('#agent_subscription_plan_LIST').DataTable().ajax.reload();
                        $('#showAGENT_SUBSCRIBE_PLAN_DELETEMODAL').modal('hide');
                        TOAST_NOTIFICATION('success', 'Deleted Successfully', 'Success !!!', '', '', '', '', '', '', '', '', '');
                    } else {
                        TOAST_NOTIFICATION('error', 'Unable to delete the Subscribed Plan', 'Error !!!', '', '', '', '', '', '', '', '', '');
                    }
                }
            });
        }
