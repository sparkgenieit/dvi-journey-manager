...
?>
        <form id="ajax_add_cash_wallet_details_form" class="row g-3" action="" method="post" data-parsley-validate>
            ...
            <div class="col-md-12">
                <label class="form-label" for="cash_remarks">Remarks<span class=" text-danger"> *</span></label>
                <div class="form-group">
                    <textarea rows="3" id="cash_remarks" name="cash_remarks" placeholder="Enter the Remarks" class="form-control required-field" required data-parsley-trigger="keyup" data-parsley-whitespace="trim"></textarea>
                </div>
            </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $("#ajax_add_cash_wallet_details_form").submit(function(event) {
                    ...
                    $.ajax({
                        type: "post",
                        url: 'engine/ajax/__ajax_manage_agent_cash_wallet.php?type=add',
                        data: data,
                        processData: false,
                        contentType: false,
                        cache: false,
                        ...
                        if (!response.success) {
                            ...
                            if (response.errors.cash_amount_required) {
                                ...
                                $('#cash_amount').focus();
                            } else if (response.errors.cash_remarks_required) {
                                TOAST_NOTIFICATION('error', 'Cash Remarks Required !!!', 'Error !!!', '', '', '', '', '', '', '', '', '');
                                $('#cash_remarks').focus();
                            }
                        } else {
                            //SUCCESS RESPOSNE
