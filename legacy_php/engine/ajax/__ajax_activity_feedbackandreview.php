...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
  if ($_GET['type'] == 'show_form') :
    ...
    if ($TYPE != "" && $TYPE == "1") :
      $select_review = sqlQUERY_LABEL("SELECT `activity_review_id`, `activity_id`, `activity_rating`, `activity_description`, `createdby`, `createdon`, `updatedon`, `status`, `deleted` FROM `dvi_activity_review_details` WHERE `deleted`='0' AND `activity_review_id`='$REVIEW_ID' ") or die("#1-UNABLE_TO_COLLECT_HOTEL_LIST:" . sqlERROR_LABEL());
      while ($fetch_review_data = sqlFETCHARRAY_LABEL($select_review)) :
        $activity_rating = $fetch_review_data['activity_rating'];
        $activity_description = $fetch_review_data['activity_description'];
...
?>
    ...
    <div class="row mt-3">
      <div class="col-md-4">
        <div class="card p-4">
          <form id="activity_feedback_form" data-parsley-validate method="POST">
            <div class="row g-3">
              <div class="col-12 row g-2" id="ajax_form_review">
                <div class="col-12">
                  ...
                  <div class="form-group">
                    <select class="form-control form-select" name="activity_rating" id="activity_rating" required>
                      <?= getSTARRATINGCOUNT($activity_rating, 'select'); ?></select>
                ...
                <div class="col-12">
                  ...
                  <div class="form-group">
                    <textarea class="form-control" id="review_description" name="review_description" rows="3" required><?= $activity_description ?></textarea>
                  </div>
                </div>
                <div class="col-12 text-end pt-4">
                  <div>
                    <button type="submit" id="submit_driver_rating_btn" class="btn btn-primary btn-md"><?= $btn_label ?></button>
                  </div>
                </div>
              </div>
            </div>
            <input type="hidden" name="hidden_activity_review_id" id="hidden_activity_review_id" value="<?= $REVIEW_ID; ?>" />
        ...
        <div class="card">
          ...
          <div class="card-body dataTable_select text-nowrap">
            <div class="text-nowrap table-responsive table-bordered">
              <table id="activity_feedback_list" class="table table-hover">
      ...
      <div class="col-12 d-flex justify-content-between text-center pt-4">
        <div>
          <a href="<?= $activity_pricebook_url; ?>" class="btn btn-secondary">Back</a>
        </div>
        <div>
          <a href="<?= $activity_preview_url; ?>" class="btn btn-primary btn-md">Update & Continue</a>
        </div>
      </div>
    </div>
    <!-- / Content -->
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#activity_feedback_list').DataTable({
          dom: 'Blfrtip',
          "bFilter": true,
          buttons: [{
              extend: 'copy',
              ...
              exportOptions: {
                columns: [0, 1, 2, 3], // Only name, email and role
              }
            },
            {
              extend: 'excel',
          ...
          ajax: {
            "url": "engine/json/__JSONactivityreview.php?ACTIVITY_ID=" + ACTIVITY_ID,
            "type": "GET"
          },
          columns: [{
              data: "counter"
        ...
        //AJAX FORM SUBMIT
        $("#activity_feedback_form").submit(function(event) {
          var form = $('#activity_feedback_form')[0];
          var data = new FormData(form);
          $(this).find("button[id='submit_itinerary_basic_info_btn']").prop('disabled', true);
          $.ajax({
            type: "post",
            url: 'engine/ajax/__ajax_manage_activity.php?type=activity_feedback',
            data: data,
            processData: false,
            contentType: false,
            cache: false,
            ...
            if (!response.success) {
              ...
              if (response.errors.activity_rating_required) {
                TOAST_NOTIFICATION('error', 'Rating is Required', 'Error !!!', '', '', '', '', '', '', '', '', '');
              } else if (response.errors.review_description_required) {
              ...
              if (response.i_result == true) {
                ...
                show_ACTIVITY_FEEDBACK_AND_REVIEW(response.activity_id);
      ...
      function showRATINGDELETEMODAL(ID) {
        $('.receiving-delete-form-data').load('engine/ajax/__ajax_manage_activity.php?type=activity_feedback_delete&ID=' + ID, function() {
      ...
      function confirmRATINGDELETE(ID) {
        $.ajax({
          type: "POST",
          url: "engine/ajax/__ajax_manage_activity.php?type=confirm_activity_feedback_delete",
          data: {
            _ID: ID
          },
          dataType: 'json',
          success: function(response) {
            if (response.result == true) {
              $('#activity_feedback_list').DataTable().ajax.reload();
      ...
      function addRATING() {
        var activity_rating = $('#activity_rating').val();
        activity_rating++;
        $('#activity_rating').val(activity_rating);
      }
      ...
      function show_ACTIVITY_FEEDBACK_AND_REVIEW(ACTIVITY_ID = "", TYPE = "", REVIEW_ID = "") {
