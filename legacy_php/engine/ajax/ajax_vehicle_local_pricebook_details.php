...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'show_form') :
        ...
        // 1. Build the unique vehicle types and time limits for all selected months
        $vehicle_types = [];
        foreach ($months as $monthYear) {
            list($monthName, $year) = explode('-', $monthYear);
            $query = "
                SELECT VENDOR_VEHICLE_TYPE.vendor_vehicle_type_ID, VEHICLE_TYPE.vehicle_type_title, TIME_LIMIT.time_limit_title, PRICE_BOOK.time_limit_id
                FROM dvi_vehicle_local_pricebook PRICE_BOOK
                INNER JOIN dvi_vendor_vehicle_types VENDOR_VEHICLE_TYPE ON VENDOR_VEHICLE_TYPE.vendor_vehicle_type_ID = PRICE_BOOK.vehicle_type_id
                    AND PRICE_BOOK.vendor_id = VENDOR_VEHICLE_TYPE.vendor_id
                INNER JOIN dvi_vehicle_type VEHICLE_TYPE ON VEHICLE_TYPE.vehicle_type_id = VENDOR_VEHICLE_TYPE.vehicle_type_id
                LEFT JOIN dvi_time_limit TIME_LIMIT ON TIME_LIMIT.time_limit_id = PRICE_BOOK.time_limit_id
                WHERE PRICE_BOOK.vendor_id = '$vendor_ID'
                AND PRICE_BOOK.year = '$year'
                AND PRICE_BOOK.month = '$monthName'
                GROUP BY VENDOR_VEHICLE_TYPE.vendor_vehicle_type_ID, PRICE_BOOK.time_limit_id
                ORDER BY VENDOR_VEHICLE_TYPE.vendor_vehicle_type_ID ASC
            ";
            $result = sqlQUERY_LABEL($query);
            while ($vrow = sqlFETCHARRAY_LABEL($result)) {
                $key = $vrow['vendor_vehicle_type_ID'] . '-' . $vrow['time_limit_id'];
                if (!isset($vehicle_types[$key])) {
                    $vehicle_types[$key] = [
                        'vehicle_type_title' => $vrow['vehicle_type_title'],
                        'time_limit_title' => $vrow['time_limit_title'],
                        'vendor_vehicle_type_ID' => $vrow['vendor_vehicle_type_ID'],
                        'time_limit_id' => $vrow['time_limit_id']
                    ];
                }
            }
        }

        // 2. Prepare HTML & CSS (no UI changes, as requested)
        $html = <<<HTML
<style>
...
HTML;
        ...
        // 3. LEFT SIDE: Vehicle Type + KM Limit Table
        ...
        foreach ($vehicle_types as $vrow) {
            $html .= '<tr>
                ...
                <td class="text-start">' . htmlspecialchars($vrow['time_limit_title']) . '</td>
            </tr>';
        }
        ...
        // 4. RIGHT SIDE: Scrollable Price Table (rows = vehicle_types, columns = days in months)
                ...
                foreach ($days_in_month as $day) {
                    ...
                    $price_query = "
                        ...
                        WHERE vendor_id = '$vendor_ID'
                          ...
                          AND time_limit_id = '$time_limit_id'
                        LIMIT 1
                    ";
