...
*/
  ...
  if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) :
    ...
    $agent_id = $_SESSION['global_g_agent_ID'];
    $guide_id = $_SESSION['global_g_guide_ID'];
    $guide_type_id = $_SESSION['global_g_guide_type'];
    $guide_type = getITINEARY_TOTAL_GUIDE_CHARGES_DETAILS($guide_type_id, '', '', 'guide_type_label');

    $guide_language_id = $_SESSION['global_g_guide_language'];
    $guide_language = getGUIDE_LANGUAGE_DETAILS($guide_language_id, 'label');
    $guide_slot_id = $_SESSION['global_g_guide_slot'];
    $guide_slot = getSLOTTYPE($guide_slot_id, 'label');
    $guide_cost = $_SESSION['global_g_guide_cost'];
    $guide_cost = number_format($guide_cost, 2); // Format with two decimal places
    $guide_cost = general_currency_symbol . ' ' . $guide_cost;
    ...
    $itinerary_route_id = $_SESSION['global_g_itinerary_route_ID'];
    $itinerary_plan_id = $_SESSION['global_g_itinerary_plan_ID'];
    $location_name = getITINEARYROUTE_DETAILS($itinerary_plan_id, $itinerary_route_id, 'location_name');
    if ($travel_expert_email) :
      $email_to = [$travel_expert_email, $admin_emailid];
    else :
      $email_to = [$admin_emailid];
    endif;

    $title = 'You are assigned to a specific Itinerary!';
    $site_title = getGLOBALSETTING('site_title');
    $description = " This is a notification to confirm that you have been successfully assigned to a specific Itinerary.";
