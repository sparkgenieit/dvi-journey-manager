...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'show_form_hotspot') :
        ...
        $hotspot_id_filter = $_POST['hotspot_name'];
        ...
        $accounts_itinerary_details_ID_hotspot = getACCOUNTSfilter_MANAGER_DETAILS('', $hotspot_id_filter, 'hotspot_id_accounts');
        ...
        } elseif (!empty($hotspot_id_filter)) {
...
?>
        ...
        <div class="card p-3 mb-4 px-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-2">List of Hotspot ledger</h4>
                <?php
                $select_accountsmanagermain_query = sqlQUERY_LABEL("SELECT `transaction_amount` FROM `dvi_accounts_itinerary_hotspot_transaction_history` WHERE `deleted` = '0' {$filterbyaccounts_date} {$filterbyaccountsquoteid} {$filterbyaccountshotspot}") or die("#1-UNABLE_TO_COLLECT_COURSE_LIST:" . sqlERROR_LABEL());
                if (sqlNUMOFROW_LABEL($select_accountsmanagermain_query) && $hotspot_id_filter != 0 && !empty($hotspot_id_filter)): ?>
                    <button id="export-accounts-btn-hotspot" class="btn btn-sm btn-label-success"><i class="ti ti-download me-2"></i>Export</button>
                <?php endif; ?>
                   ...
                   ") or die("#get_hotspot_data_query: " . sqlERROR_LABEL());
                        ...
                        if (sqlNUMOFROW_LABEL($get_hotspot_data_query)):
                            while ($row = sqlFETCHARRAY_LABEL($get_hotspot_data_query)) :
                                ...
                                echo "<td>" . general_currency_symbol . ' ' . number_format(round($hotspot_amount), 2) . "</td>";
        ...
        <script>
            $(document).ready(function() {
                $('#export-accounts-btn-hotspot').click(function() {
                    window.location.href = 'excel_export_accounts_ledger_hotspot.php?quote_id=<?= $quote_id; ?>&from_date=<?= $from_date; ?>&to_date=<?= $to_date; ?>&hotspot_id=<?= $hotspot_id_filter; ?>';
                });
            });
