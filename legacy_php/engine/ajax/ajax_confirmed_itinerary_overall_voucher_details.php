...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'show_form') :
        ...
        $existing_vendor_record_count = get_CONFIRMED_ITINERARY_VOUCHER_DETAILS($itinerary_plan_ID, 'vendor_voucher_created_count');
?>
        <div class="row">
            <div class="col-md-12">
                <div class="card p-4">
                    <div class="row">
                        <div class="d-flex justify-content-between align-items-center  mb-3">
                            <h5 class="text-primary mb-0">Voucher Details</h5>

                            <div class="d-flex gap-2 ms-auto">
                                <!--<a id="btn_cancel_itinerary" href="latestconfirmeditinerary_cancellation.php?cip_id=<?= $itinerary_plan_ID ?>" target="_blank" class="btn btn-label-danger ps-3">-->
                                <a id="btn_cancel_itinerary" href="javascript:void(0);" onclick="showCANCEL_ITINERARY_MODAL('<?= $itinerary_plan_ID ?>');" class="btn btn-label-danger ps-3">
                                    <i class="ti ti-x pe-1"></i> Cancel Itinerary
                                </a>

                                <a href="latestconfirmeditinerary.php?route=add&formtype=generate_itinerary&id=<?= $itinerary_plan_ID; ?>" class="btn btn-sm btn-label-github waves-effect ps-3">
                                    <i class="tf-icons ti ti-arrow-left ti-xs me-1"></i> Back
                                </a>
                            </div>
                        </div>

                        <div class="col-md-3 mb-3">
                            <label class="text-light">Booking ID</label>
                    ...
                    <div class="card p-0">
                        <form action="" method="post" id="confirmed_itineary_vendor_voucher_form">
                            <div class="card-header mb-2 d-flex align-items-center justify-content-between">
                                ...
                                <div>
                                    <?php if ($existing_vendor_record_count > 0) : if ($logged_user_level != 4): ?>
                                            <a href="agent_vouchervehiclepdf.php?id=<?= $itinerary_plan_ID; ?>" type="button" class="btn btn-secondary btn-label-secondary">Download Agent Vehicle Voucher</a>
                                    <?php endif;
                                    endif; ?>
                                    <?php if ($existing_vendor_record_count > 0) : ?>
                                        <a href="vouchervehiclepdf.php?id=<?= $itinerary_plan_ID; ?>" type="button" class="btn btn-label-success">Download Vehicle Voucher</a>
                                    <?php endif; ?>
                                    <?php if ($logged_user_level == 1 || $logged_user_level == 3): ?>
                                        <button id="createVehicleVoucherButton" type="submit" class="btn btn-label-primary d-none">+<?= ($existing_vendor_record_count > 0) ? " Update " : " Create " ?>Vehicle Voucher</button>
                                    <?php endif; ?>
                                </div>
                            </div>
                                        ...
                                        <tbody class="border-top-0" id="itineary_hotel_LIST">
                                            ...
                                            foreach ($get_unique_vehicle_type as $vehicle_type) :
                                                ...
                                                if ($select_itinerary_plan_vendor_count > 0) :
                                                    while ($fetch_eligible_vendor_data = sqlFETCHARRAY_LABEL($select_itinerary_plan_vendor_data)) :
                                                        ...
                                                        $get_vehicle_booking_status = get_ITINERARY_VEHICLE_VOUCHER_DETAILS($confirmed_itinerary_plan_vendor_eligible_ID, 'vehicle_booking_status');
                                            ...
                                            ?>
                                                        <tr class="cursor-pointer">
                                                            ...
                                                            <td>
                                                                <input type="checkbox" name="hidden_confirmed_itinerary_plan_vendor_eligible_ID[]" value="<?= $confirmed_itinerary_plan_vendor_eligible_ID; ?>"  style="display:none;">

                                                                <input class="form-check-input vehicle-checkbox" type="checkbox" value="<?= $itinerary_plan_vendor_eligible_ID; ?>" name="itinerary_plan_vendor_eligible_ID[]" id="hotel_check_<?= $itinerary_plan_vendor_eligible_ID; ?>"    onclick="this.closest('td').querySelector('input[name=\'hidden_confirmed_itinerary_plan_vendor_eligible_ID[]\']').checked = this.checked;">
                                                            ...
                                                            <td>
                                                                <?= $total_vehicle_qty; ?> x <?= general_currency_symbol . ' ' . number_format(round($vehicle_grand_total), 2); ?>
                                                            </td>
                                                ...
                                                <div class="d-flex align-items-center justify-content-between my-3">
                                                    ...
                                                    <p class="mb-0"><?= general_currency_symbol . ' ' . number_format(round($vehicle_total_amount), 2); ?></p>
                                                </div>                                                 
                                                <div class="d-flex align-items-center justify-content-between my-3">
                                                    <p class="mb-0">GST <?= $vehicle_gst_percentage . '%'; ?></p>
                                                    <p class="mb-0"><?= general_currency_symbol . ' ' . number_format(round($vehicle_gst_amount), 2); ?></p>
                                                </div>
                                                <div class="d-flex align-items-center justify-content-between my-3">
                                                    <p class="mb-0">Vendor Margin (<?= $vendor_margin_percentage . '%'; ?>)</p>
                                                    <p class="mb-0"><?= general_currency_symbol . ' ' . number_format(round($vendor_margin_amount), 2); ?></p>
                                                </div>     
                                                <div class="d-flex align-items-center justify-content-between my-3">
                                                    <p class="mb-0">Margin Service Tax <?= $vendor_margin_gst_percentage . '%'; ?></p>
                                                    <p class="mb-0"><?= general_currency_symbol . ' ' . number_format(round($vendor_margin_gst_amount), 2); ?></p>
                                                </div> 
                                                <hr class="my-2">   
                                                <div class="d-flex align-items-center justify-content-between my-3 grand_total_section">
                                                    <p class="mb-0"><b>Grand Total <?= $total_vehicle_qty; ?> x <?= general_currency_symbol . ' ' . number_format(round($vehicle_grand_total), 2); ?></b></p>
                                                    <p class="mb-0"><b><?= general_currency_symbol . ' ' . number_format(round(($total_vehicle_qty * $vehicle_grand_total)), 2); ?></b></p>
                                                </div>
                                            </div>'><b><?= general_currency_symbol . ' ' . number_format(round(($total_vehicle_qty * $vehicle_grand_total)), 2); ?></b></span>
                                                                <?php else : ?>
        ...
        if (($itinerary_preference == 1 || $itinerary_preference == 3)):
            $hotel_voucher_confirmed_count = get_CONFIRMED_ITINERARY_VOUCHER_DETAILS($itinerary_plan_ID, 'hotel_voucher_confirmed_count');
        ?>
                            ...
                            <div class="card-header mb-2 d-flex align-items-center justify-content-between">
                                ...
                                <div>
                                    ...
                                    ?>
                                            ...
                                            <?php else:                                         if ($logged_user_level != 4) :
                                            ?>

                                                <a id="downloadAgentHotelVoucherButton" class="btn btn-secondary btn-label-secondary d-none">
                                                    <i class="ti ti-download me-1"></i> Download Agent Hotel Voucher
                                                </a>
                                            <?php endif; ?>
                                            <a id="downloadHotelVoucherButton" class="btn btn-label-success d-none">
                                                <i class="ti ti-download me-1"></i> Download Hotel Voucher
                                            </a>
                                    <?php endif;
                                    endif; ?>
                                    <?php if ($logged_user_level == 1 || $logged_user_level == 3): ?>
                                        <button id="createHotelVoucherButton" type="submit" class="btn btn-label-primary d-none">+<?= ($existing_hotel_record_count > 0) ? " Update " : " Create " ?>Hotel Voucher</button>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="card-body dataTable_select text-nowrap">
                                            ...
                                            ?>
                                                    <tr class="cursor-pointer">
                                                        ...
                                                        <td>
                                                            <?php
                                                            if (($hotel_required == 1)) :
                                                                // if ($previous_hotel_id != $selected_hotel_id):
                                                            ?>
                                                                <input type="checkbox" class="hotel-checkbox-confirmed" name="hidden_confirmed_itinerary_plan_hotel_details_ID[]" id="hotel_check_<?= $confirmed_itinerary_plan_hotel_details_ID; ?>" value="<?= $confirmed_itinerary_plan_hotel_details_ID; ?>" hidden>

                                                                <input class="form-check-input hotel-checkbox" type="checkbox" value="<?= $confirmed_itinerary_plan_hotel_details_ID; ?>" name="itinerary_plan_hotel_details_ID[]" id="hotel_check_<?= $confirmed_itinerary_plan_hotel_details_ID; ?>" data-id="<?= $selected_hotel_id ?>" data-booking-status="<?= $get_hotel_booking_status ?>">
                                                            <?php
                                                            // endif;
                                                            else : ?>
                                                                <span>--</span>
                                                ...
                                                <?php
                                                    $previous_hotel_id = $selected_hotel_id;
                                                endwhile;
                                            else: ?>
                                                <tr>
                                                    <td class="text-center" colspan='7'>No Hotel Available</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
        ...
        <!-- Modal -->
        <div class="modal fade" id="showVENDORCANCELLATIONPOLICYFORMDATA" tabindex="-1" aria-labelledby="showVENDORCANCELLATIONPOLICYFORMDATALabel" aria-hidden="true">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#allhotelcustomCheck').change(function() {
                    ...
                    $('.hotel-checkbox-confirmed').prop('checked', isChecked);
                    toggleCreateHotelVoucherButton();
                    toggleDownloadHotelVoucherButton();
                    toggleDownloadAgentHotelVoucherButton();
                });
                ...
                $('#allvehiclecustomCheck').change(function() {
                    ...
                    toggleCreateVehicleVoucherButton();
                    toggleDownloadHotelVoucherButton();
                    toggleDownloadAgentHotelVoucherButton();
                });

                //                 $(document).on("change", ".hotel-checkbox", function() {
                //     let checkboxID = $(this).attr("id"); // Get the ID of the clicked checkbox

                //     // Find all confirmed checkboxes with the same ID and set their checked state
                //     $(".hotel-checkbox-confirmed#" + checkboxID).prop("checked", this.checked);
                // });
                ...
                $('.hotel-checkbox').change(function() {
                    ...
                    var checkboxID = $(this).attr('id'); // Get the ID of the current checkbox

                    // Check/uncheck the corresponding "hotel-checkbox-confirmed" input with the same ID
                    $('#' + checkboxID + '.hotel-checkbox-confirmed').prop('checked', isChecked);
                    ...
                    for (var i = currentIndex + 1; i < $('.hotel-checkbox').length; i++) {
                        if ($('.hotel-checkbox').eq(i).data('id') == selectedHotelID) {
                            $('.hotel-checkbox').eq(i).prop('checked', isChecked);
                            $('#' + $('.hotel-checkbox').eq(i).attr('id') + '.hotel-checkbox-confirmed').prop('checked', isChecked);
                        } else {
                ...
                function toggleDownloadHotelVoucherButton() {
                    // Are there _any_ hotel rows at all?
                    const anyCheckboxes = $('.hotel-checkbox').length > 0;
                    ...
                    // show the button whenever there is at least one voucher row
                    $('#downloadHotelVoucherButton')
                        .attr('href', '#')
                        .removeClass('d-none')
                        .off('click')
                        .on('click', function(event) {
                            ...
                            // get all checked, or if none checked, get _all_ checkboxes                         
                            let boxes = $('.hotel-checkbox:checked');
                            if (boxes.length === 0) {
                                boxes = $('.hotel-checkbox');
                            }

                            // group by hotelId
                            const hotelDetailsById = {};
                            boxes.each(function() {
                                const $cb = $(this);
                                const id = $cb.data('id');
                                const status = $cb.data('booking-status');
                                const val = $cb.val();

                                if (!hotelDetailsById[id]) {
                                    hotelDetailsById[id] = {
                                        values: [],
                                        bookingStatus: status
                                    };
                                }
                                hotelDetailsById[id].values.push(val);
                            });

                            // trigger a download per hotel group
                            Object.entries(hotelDetailsById).forEach(([hotelId, details]) => {
                                if (!details.bookingStatus) {
                                    console.log(`Skipping Hotel ${hotelId} (no booking-status)`);
                                    return;
                                }

                                const list = encodeURIComponent(details.values.join(','));
                                const url = `voucherpdf.php` +
                                    `?itinerary_plan_ID=<?= $itinerary_plan_ID ?>` +
                                    `&confirmid=<?= $confirmed_itinerary_plan_ID ?>` +
                                    `&selectedHotel=${list}`;

                                console.log(`Downloading Hotel ${hotelId}: ${url}`);
                ...
                // whenever your table changes, re-run this to refresh button visibility
                toggleDownloadHotelVoucherButton();


                function toggleDownloadAgentHotelVoucherButton() {
                    // Show button if there’s at least one agent‐voucher row on the page
                    const anyAgentBoxes = $('.hotel-checkbox').length > 0;
                    if (!anyAgentBoxes) {
                        // nothing to download
                        $('#downloadHotelVoucherButton').addClass('d-none');
                        return;
                    ...
                    $('#downloadAgentHotelVoucherButton')
                        .attr('href', '#')
                        .removeClass('d-none')
                        .off('click')
                        .on('click', function(event) {
                            ...
                            Object.entries(hotelDetailsById).forEach(([hotelId, details]) => {
                                ...
                                const list = encodeURIComponent(details.values.join(','));
                                const url = `agent_voucherpdf.php` +
                                    `?itinerary_plan_ID=<?= $itinerary_plan_ID ?>` +
                                    `&confirmid=<?= $confirmed_itinerary_plan_ID ?>` +
                                    `&selectedHotel=${list}`;
                ...
                // call it on page load / whenever checkboxes change
                toggleDownloadAgentHotelVoucherButton();
            ...
            function toggleCreateVehicleVoucherButton() {
                if ($('.vehicle-checkbox:checked').length > 0) {
                    $('#createVehicleVoucherButton').removeClass('d-none');
                } else {
                    $('#createVehicleVoucherButton').addClass('d-none');
                }
            }
