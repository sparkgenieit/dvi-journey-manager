...
?>
        ...
        <div class="col-md-12">
            <div class="card p-0">
                <div class="card-header pb-3 d-flex justify-content-between">
                    ...
                    <div class="col-md-7">
                        <h5 class="card-title mb-3 mt-2">List of Hotspot</h5>
                    </div>

                    <div class="col-md-5 text-end">
                        <a href="newhotspot.php?route=add&formtype=hotspot_info" class="btn btn-label-primary waves-effect">+ Add Hotspot</a>
                        <div class="btn-group" id="dropdown-icon-demo">
                            <a href="hotspot_parking_charge_pricebook.php?route=import" class="btn btn-label-primary waves-effect"><i class="ti ti-upload ti-xs me-1"></i> Parking charges</a>
                        </div>
                    </div>
                        ...
                        <table class="table table-hover" id="hotspot_LIST">
                            <thead class="table-head">
                                <tr>
                                    ...
                                    <th scope="col">Hotspot Priority</th>
                                    <th scope="col">Hotspot Place</th>
                                    <th scope="col">Local Person</th>
                                    <th scope="col">Foreign Person</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
    ...
    <div class="modal fade" id="showHOTSPOTDELETEMODAL" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content receiving-delete-form-hotspot-data">
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#hotspot_LIST').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: "hotspot_priority",
                        render: function(data, type, row) {
                            return '<input type="text" class="form-control hotspot-priority-input" autocomplete="off" data-parsley-type="number" data-parsley-trigger="keyup" required value="' + data + '" data-id="' + row.modify + '" />';
                        }
                    }, //4
                    {
                        data: "hotspot_locations"
                    }, //5
                    {
                        data: "local_members"
                    }, //6
                    {
                        data: "foreign_members"
                    } //7
                ],
                initComplete: function(settings, json) {
                    // Initialize Parsley on the new input fields after DataTable rendering
                    $('.hotspot-priority-input').parsley();
        ...
        $(document).on('keyup', '.hotspot-priority-input', function() {
            var $this = $(this);

            // Initialize Parsley validation for this specific input
            $this.parsley();
            ...
            if ($this.parsley().isValid()) {
                ...
                // Perform the AJAX request to update the priority
                $.ajax({
                    type: "POST",
                    url: "engine/ajax/__ajax_manage_hotspot.php?type=hotspot_priority_update",
                    data: {
                        id: hotspotId,
                    ...
                    success: function(response) {
                        if (response.success) {
                            $('#hotspot_LIST').DataTable().ajax.reload();
                            TOAST_NOTIFICATION(
                                'success',
                                'Hotspot Priority Updated Successfully.',
                                'Success !!!',
                                '', '', '', '', '', '', '', '', ''
                            );
                        } else {
                            TOAST_NOTIFICATION(
                                'error',
                                'Failed to Update Hotspot Priority.',
                                'Error !!!',
                                '', '', '', '', '', '', '', '', ''
                            );
                        }
        ...
        function showHOTSPOTADDDETAILMODAL(result_count, duplicate_count) {
            $('.receiving-hotspot-add-form-data').load('engine/ajax/__ajax_add_hotspot_form.php?type=show_details&RESULT_COUNT=' + result_count + '&DUPLICATE_COUNT=' + duplicate_count + '', function() {
                $('#addHOTSPOTADDFORM').modal('hide');
                const container = document.getElementById("addHOTSPOTADDFORM");
        ...
        function showHOTSPOTDELETEMODAL(ID) {
            $('.receiving-delete-form-hotspot-data').load('engine/ajax/__ajax_manage_hotspot.php?type=hotspot_delete&ID=' + ID, function() {
                const container = document.getElementById("showHOTSPOTDELETEMODAL");
                const modal = new bootstrap.Modal(container);
                modal.show();
            });
        }
        //CONFIRM DELETE POPUP
        function confirmHOTSPOTDELETE(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/__ajax_manage_hotspot.php?type=confirm_hotspot_delete",
                data: {
                    _ID: ID
                },
                dataType: 'json',
                success: function(response) {
                    if (response.result == true) {
                        $('#hotspot_LIST').DataTable().ajax.reload();
                        $('#showHOTSPOTDELETEMODAL').modal('hide');
