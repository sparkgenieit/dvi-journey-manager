...
?>
                ...
                <form id="form_itinerary_basicinfo" action="" method="post" data-parsley-validate>
                    <div class="card p-4">
                        <div class="row g-3">
                            ...
                            <?php
                            if ($logged_user_level == 1 || $logged_user_level == 6) :
                                //SUPER ADMIN
                            ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <select name="agent" id="agent" class="form-select form-control location" required>
                                                <?= getAGENT_details($agent_id, '', 'agent_with_company') ?>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-12 <?= $room_type_container_add_class; ?>" id="add_room_container">
                                <?php if ($itinerary_preference != 2) : ?>
                                    ...
                                    <div class="card shadow-none bg-transparent border border-primary border-dashed p-4 pt-1 ps-3">
                                        ...
                                        <?php
                                        if (!empty($itinerary_plan_ID) && $total_itinerary_traveller_details_count > 0):
                                            $add_func = 'Onclick="addNewRooms()"';
                                        endif;
                                        ?>
                                        <div class="d-flex align-items-center gap-2 mt-2">
                                            <span class="text-muted small">Total</span>
                                            <input type="number" id="roomsTotal" class="form-control form-control-sm"
                                                value="<?= $total_itinerary_traveller_details_count; ?>" min="1" style="width:60px">
                                            <button type="button" id="applyRoomsBtn" <?= $add_func; ?>
                                                class="btn btn-link rounded-pill waves-effect p-0 text-primary">
                    ...
                    <?php if ($itinerary_plan_ID != 0 && $itinerary_plan_ID != '') : ?>
                                        ...
                                        <div class="nav-align-top nav-tabs-shadow mb-4">
                                            ...
                                            <div class="tab-content">
                                                <div class="tab-pane fade show active" id="route-one" role="tabpanel">
                                                    <div id="show_default_route_details">
                                                            ...
                                                            <tbody id="route_details_tbody">
                                                                ...
                                                                ?>
                                                                        <tr id="route_details_<?= $itinerary_route_ID; ?>" class="route_details" data-itinerary_route_ID="<?= $itinerary_route_ID; ?>" data-day-no="<?= $route_day_counter; ?>">
                                                                            ...
                                                                            <td>
                                                                                <input type="text" name="source_location[]" id="source_location_<?= $route_day_counter; ?>" class="bg-body form-select form-control location" value="<?= $location_name; ?>" <?= $location_name ? 'readonly' : ''; ?>>
        ...
        <script>
            <?php if (empty($itinerary_plan_ID)) : ?>
                ...
                function appendAndSelectItineraryType(val, label) {

                    var itinerarytypeSelectize = $('#itinerary_type').selectize({
                        plugins: ['select_on_focus']
                    })[0].selectize;

                    // 1) Add the option if it doesn’t already exist
                    if (!itinerarytypeSelectize.options[val]) {
                        itinerarytypeSelectize.addOption({
                            value: val,
                            text: label
                        });
                        // 2) Force Selectize to rebuild its option dropdown
                        itinerarytypeSelectize.refreshOptions(false);
                    }

                    // 3) Clear any previous selection (for single-select controls)
                    itinerarytypeSelectize.clear();

                    // 4) Insert the new item into the selected-items list
                    itinerarytypeSelectize.addItem(val);
                }
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.no_routes_found) {
                                    ...
                                    modal.show();
                                    $("#no_route_mgs1").html(response.no_routes_message);

                                    appendAndSelectItineraryType(2, '2');

                                } else if (response.no_matching_routes_found) {
                                    const container = document.getElementById("NOMATCHINGROUTESMODAL");
                                    const modal = new bootstrap.Modal(container);
                                    modal.show();
                                    $("#no_route_mgs").html(response.no_routes_message);
                                    appendAndSelectItineraryType(2, '2');
                                } else {
                ...
                function showCustomRoute() {
                    if ($('#NOROUTESUGGESTIONSMODAL').hasClass('show')) {
                        $('#NOROUTESUGGESTIONSMODAL').modal('hide');
                    }
                    $('#itinerary_type')[0].selectize.setValue(2);
                ...
                <table id="custom_route_details_LIST" class="table table-borderless" style="width:100%">
                    ...
                    <tbody id="custom_route_details_tbody">
            `;
                            if (customRouteDetails.length === 0) {
                                customRouteContent += `
                    <tr>
                        <td colspan="7">Please Select the Trip Start and End Date First</td>
                    </tr>
            ...
            `;
                ...
                function handleDayNightCalcChangeds(selectedDates, dateStr, instance) {
                    ...
                    if (startDateTrip && endDateTrip) {
                        ...
                        $('#custom_route_details_LIST tbody').html('');
            ...
            $(document).ready(function() {
                ...
                flatpickr(tripStartTimeInput, {
                    ...
                    dateFormat: "h:i K",
                    time_24hr: false,
                    defaultDate: tripStartTimeInput.value || "12:00 PM",
                ...
                function handleEndDateChange() {
                    ...
                    var selected_itinerary_type = $('#itinerary_type').val();
                    setupItineraryTypeUI(selected_itinerary_type); 
                }
                ...
                <?php if (!in_array($logged_user_level, [4, 8])): ?>
                    //get_destination_location_details();
                    function initAgentSelect(defaultVal) {
                        if ($('#agent')[0].selectize) $('#agent')[0].selectize.destroy();
                        var s = $('#agent').selectize({
                            plugins: ['select_on_focus']
                        })[0].selectize;
                        if (defaultVal) s.setValue(String(defaultVal));
                    }

                    // preserve value across refresh/reinit
                    var current = $('#agent')[0].selectize ? $('#agent')[0].selectize.getValue() : $('#agent').val();
                    initAgentSelect(current);
                <?php endif; ?>
            ...
            function addNewRooms() {
                const inp = document.getElementById('roomsTotal');
                inp.value = parseInt(inp.value);
                setRoomsTo(inp.value);
            }

            function getRoomNodes() {
                const c = document.getElementById('room_container');
                let nodes = c.querySelectorAll(ROOM_SELECTOR);
                if (!nodes.length) nodes = c.children; // fallback
                return Array.from(nodes);
            }

            function renumberRooms() {
                getRoomNodes().forEach((node, i) => {
                    const title = node.querySelector('.room-title'); // optional: update "#Room X"
                    if (title) title.textContent = `#Room ${i + 1}`;
                });
            }

            function setRoomsTo(target) {
                const inp = document.getElementById('roomsTotal');
                const min = parseInt(inp.min || '1', 10);
                const want = Math.max(parseInt(target ?? inp.value, 10) || 1, min);

                const container = document.getElementById('room_container');
                let current = getRoomNodes().length;

                // add missing rooms
                for (let i = current; i < want; i++) addmoreRoom(2, 0, 0);

                // remove extras
                while (getRoomNodes().length > want) container.lastElementChild?.remove();

                renumberRooms();
            }

            // ---------- helpers (adjust ROOM_SELECTOR if your room blocks have a class) ----------
            const ROOM_SELECTOR = '.room-card'; // if you don't have this class, we'll fall back to children
            ...
            document.querySelectorAll('input[name="itinerary_prefrence"]').forEach(function(radio) {
                radio.addEventListener('change', function() {
                    ...
                    if (selectedPreference === '2') {
                        ...
                        // UI template — input placed beside the Add Rooms button
                        const add_room_container_details = `
                        <div class="card shadow-none bg-transparent border border-primary border-dashed p-4 pt-1 ps-3">
                        <div class="row mt-2" id="room_container"></div>

                        <div class="d-flex align-items-center gap-2 mt-2">
                            <span class="text-muted small">Total</span>
                            <input type="number" id="roomsTotal" class="form-control form-control-sm" value="1" min="1" style="width:60px">

                            <button id="applyRoomsBtn" type="button"
                            class="btn btn-link rounded-pill waves-effect p-0 text-primary">
                            <span class="tf-icons ti ti-circle-plus ti-xs me-1"></span> Add Rooms
                            </button>
                        </div>
                        ...
                        // wire up
                        document.getElementById('applyRoomsBtn').addEventListener('click', () => setRoomsTo());
                        document.getElementById('roomsTotal').addEventListener('keydown', e => {
                            if (e.key === 'Enter') setRoomsTo();
                        });

                        // keep your existing initial room if needed
                        addRoom(2, 0, 0);
            ...
            function initializeAutocomplete(dayNum) {
                ...
                // Only set value if prevVisitingLocation is not empty
                if (prevVisitingLocation) {
    ...
    `;
            ...
            function addDayToRoute(tabIndex) {
                let lastRow = $("#custom_route_details_LIST_" + tabIndex + " tbody tr:last-child");

                if (lastRow.length === 0) {
                    console.error("Error: No rows found in the table for tabIndex:", tabIndex);
                    return;
                }
                ...
                // Use a regular expression to extract the day number
                let dayMatch = dayText.match(/DAY\s*(\d+)/i);
                let lastDayNum = dayMatch ? parseInt(dayMatch[1], 10) : NaN;

                // Check if lastDayNum is a valid number
                if (isNaN(lastDayNum)) {
                    console.error("Error: Unable to parse day number. Extracted text:", dayText);
                    return;
                }

                console.log("Last Day Number:", lastDayNum);
                ...
                // Update UI
                $(`#source_location_${tabIndex}_1`).attr('readonly', true);
                $(`#next_visiting_location_${tabIndex}_${lastDayNum}`).removeAttr('readonly');
                $(`#next_visiting_location_${tabIndex}_${nextDayNum}`).attr('readonly', true);
                addDefaultNewDayRow(tabIndex, nextDayNum, lastDayNum, formattedDate); // Pass formattedDate instead of nextDayNum
                initializeAutocomplete(nextDayNum);
            }

            function addDefaultNewDayRow(tabIndex, dayNum, tripDay, getDay) {
                let newRow = `
        <tr>
            <td class="day text-start" width="8%">DAY ${dayNum}</td>
            <td class="date" id="route_date_${tabIndex}_${dayNum}">${getDay}</td>
            <td>
                <input type="text" readonly required class="bg-body form-control" id="source_location_${tabIndex}_${dayNum}" name="source_location_${tabIndex}[]" style="cursor:not-allowed;" placeholder="Source Location" aria-describedby="defaultFormControlHelp">
            </td>
            <td>
                <select name="next_visiting_location_${tabIndex}[]" id="next_visiting_location_${tabIndex}_${dayNum}" class="next_visiting_location text-start bg-body form-select form-control location" tabindex="${dayNum}" required>
                </select>
            </td>
            <td>
                <button type="button" class="btn btn-outline-primary btn-sm add_via_route" onclick="addDEFAULTVIAROUTE(${dayNum},'','',${tabIndex}"><i class="ti ti-route ti-tada-hover"></i></button>
            </td>
            <td>
                <label class="switch switch-sm"><input type="checkbox" id="direct_destination_visit_${tabIndex}_${dayNum}" name="direct_destination_visit_${tabIndex}[${dayNum}][]" class="switch-input"><span class="switch-toggle-slider"><span class="switch-on"><i class="ti ti-check"></i></span><span class="switch-off"><i class="ti ti-x"></i></span></span></label>
            </td>
            <td>${dayNum > tripDay ? `<i class="ti ti-x ti-danger ti-tada-hover ti-md deleteDefaultRow" 
    data-tabindex="${tabIndex}" style="color: #F32013; cursor: pointer;"></i>` : ''}
</td>
    ...
    `;
                ...
                if (dayNum <= tripDay) {
                    lastRow = $("#custom_route_details_LIST_" + tabIndex + " tbody tr").eq(tripDay - 1);
                }
                ...
                // Reorder the day count and IDs
                $("#custom_route_details_LIST_" + tabIndex + " tbody tr").each(function(index, element) {
                    let currentDayNum = index + 1; // Index starts from 0, so +1 for day number

                    $(this).find(".day").text(`DAY ${currentDayNum}`);
                    $(this).find(".date").attr("id", `route_date_${tabIndex}_${currentDayNum}`);
                    $(this).find(`select[name='itinerary_route_date_${tabIndex}[]']`).attr("id", `next_visiting_location_${tabIndex}_${currentDayNum}`);
                    $(this).find(`input[name='source_location_${tabIndex}[]']`).attr("id", `source_location_${tabIndex}_${currentDayNum}`);
                    $(this).find(`select[name='next_visiting_location_${tabIndex}[]']`).attr("id", `next_visiting_location_${tabIndex}_${currentDayNum}`);
                    $(this).find(`input[name^='direct_destination_visit_${tabIndex}']`).attr("id", `direct_destination_visit_${tabIndex}_${currentDayNum}`).attr("name", `direct_destination_visit_${tabIndex}[${currentDayNum}][]`);
                });

                // Update total .day count
                let totalDays = $("#custom_route_details_LIST_" + tabIndex + " tbody tr").length;
                ...
                // Update VIA_ROUTE_COUNT for each add_via_route button
                VIA_ROUTE_COUNT = 0;
                $(`.add_via_route`).each(function() {
                    VIA_ROUTE_COUNT++;
                    $(this).attr(
                        "onclick",
                        `addDEFAULTVIAROUTE(${VIA_ROUTE_COUNT}, '', '', ${tabIndex})`
                    );
                });

                initializeDefaultAutocomplete(tabIndex, tripDay);
                ...
                $(`#source_location_${tabIndex}_1`).val($("#arrival_location").val());
                initializeDefaultEasyAutocomplete(1, tabIndex, dayNum, $(`#source_location_${tabIndex}_${dayNum}`).val(), $(`#next_visiting_location_${tabIndex}_${dayNum}`));
                $(`#source_location_${tabIndex}_${tripDay}`).val(
                    $(`#next_visiting_location_${tabIndex}_${tripDay - 1}`).val()
                );
            }

            /* // Delete button event listener
            $(document).on('click', '.deleteDefaultRow', function() {
                let tabIndex = $(this).data("tabindex");
                $(this).closest('tr').remove();

                // Select only rows related to the current tabIndex
                rows = $(`#custom_route_details_LIST_${tabIndex} tbody tr`);
                ...
                // Parse the start date with the desired format
                const startDateStr = $(`#trip_start_date_and_time_${tabIndex}`).val();
                const startDateTrip = flatpickr.parseDate(startDateStr, "d/m/Y");
                ...
                var dayNumber = dateId.match(/\d+/)[0]; // Extracts the number from the string
                // initializeAutocomplete(totalDays);
                // initializeDefaultAutocomplete(tabIndex, dayNumber);
            }); */
            ...
            $(document).on('click', '.deleteDefaultRow', function() {
                ...
                // Select only rows related to the current tabIndex
                let rows = $(`#custom_route_details_LIST_${tabIndex} tbody tr`);

                // Re-calculate day numbers, dates, and update IDs for remaining rows
                $(rows).each(function(index, row) {
                    let rowNumber = index + 1; // Ensure numbering starts from 1

                    // Update day number
                    $(this).find(".day").text(`DAY ${rowNumber}`);

                    // Update date ID
                    $(this).find(".date").attr("id", `route_date_${tabIndex}_${rowNumber}`);

                    // Update source and next visiting location IDs and Names
                    $(this).find("input[name='source_location[]']")
                        .attr("id", `source_location_${tabIndex}_${rowNumber}`)
                        .attr("name", `source_location_[${tabIndex}][]`);

                    $(this).find("input[name='next_visiting_location[]']")
                        .attr("id", `next_visiting_location_${tabIndex}_${rowNumber}`)
                        .attr("name", `next_visiting_location_[${tabIndex}][]`);

                    // Update direct_destination_visit[] ID and Name properly
                    $(this).find("input[name^='direct_destination_visit']").each(function() {
                        $(this).attr("id", `direct_destination_visit_${tabIndex}_${rowNumber}`);
                        $(this).attr("name", `direct_destination_visit_[${tabIndex}][${rowNumber - 1}][]`);
                    });
                });
                ...
                if (!startDateTrip) {
                    console.error(`Invalid start date for tab index: ${tabIndex}`);
                    return;
                }
                ...
                rows.each(function(index, row) {
                    ...
                    $(dateCell).attr("id", `route_date_${tabIndex}_${rowNumber}`);
                });
                ...
                // Update total days and nights
                $(`#no_of_days_${tabIndex}`).val(totalDays);
                $(`#no_of_nights_${tabIndex}`).val(totalDays - 1);

                // Update trip end date
                let tripEndDate = new Date(startDate);
                tripEndDate.setDate(tripEndDate.getDate() + totalDays - 1);
                let formattedEndDate = ("0" + tripEndDate.getDate()).slice(-2) + '/' +
                    ("0" + (tripEndDate.getMonth() + 1)).slice(-2) + '/' +
                    tripEndDate.getFullYear();

                let flatpickrInstance = $(`#trip_end_date_and_time_${tabIndex}`)[0]?._flatpickr;
                if (flatpickrInstance) {
                    flatpickrInstance.setDate(formattedEndDate);
                } else {
                    console.error(`Flatpickr instance not found for tab index: ${tabIndex}`);
                }

                let dateId = $(this).closest('tr').find('td.date').attr('id');
                if (dateId) {
                    let dayNumber = dateId.match(/\d+/)[0];
                    initializeAutocompleteForDefaultAllDays(dayNumber);
                }
            });
            ...
            function initializeDefaultSelectize(selectizeInput, optionsData, tabIndex) {
                ...
                if (selectizeInput[0].selectize) {
                    ...
                    updateDefaultNextDaySourceLocation(tabIndex, day_no, '');
            ...
            function updateDefaultNextDaySourceLocation(tabIndex, current_day_no, value) {
                ...
                if (next_day_no <= total_no_of_days) {
                    var next_source_input = $('#source_location_' + tabIndex + '_' + next_day_no);
                    if (next_source_input.length > 0) {
                        next_source_input.val(value).trigger('change');
                    } else {
                        next_source_input.val('').trigger('change');
                    }
                    var next_visiting_input = $('#next_visiting_location_' + tabIndex + '_' + next_day_no);
                    console.log('nextSourceInputId', next_source_input);
                    console.log('nextVisitingInputId', next_visiting_input);
            ...
            function addDEFAULTVIAROUTE(DAY_NO, itinerary_route_ID, itinerary_plan_ID, tabIndex) {
                // Encode and trim the values specific to the tab
                var itinerary_route_date = encodeURIComponent($('#itinerary_route_date_' + tabIndex + '_' + DAY_NO).val()).trim();
                var selected_source_location = encodeURIComponent($('#source_location_' + tabIndex + '_' + DAY_NO).val()).trim();
                var selected_next_visiting_location = encodeURIComponent($('#next_visiting_location_' + tabIndex + '_' + DAY_NO).val()).trim();

                if (selected_source_location && selected_next_visiting_location) {
                    $('.receiving-modal-info-form-data').load(
                        'engine/ajax/ajax_latest_itineary_via_route_form.php?type=show_form&DAY_NO=' + DAY_NO +
                        '&selected_source_location=' + selected_source_location +
                        '&selected_next_visiting_location=' + selected_next_visiting_location +
            ...
            // async function getdefaultVEHICLEPLANDETAILS(redirect_URL, itinerary_plan_ID) {
            //     toggleLoader(true);
            //     try {
            //         const response = await $.ajax({
            //             type: 'post',
            ...
            //             dataType: 'json'
            //         });
            //         handleDefaultVehiclePlanResponse(response, redirect_URL);
            //     } catch (error) {
            //         console.error(error);
            //     } finally {
            //         toggleLoader(false);
            ...
            //     // Count the total number of route tabs
            //     let totalRouteTabs = $(".tab-pane.fade").length;

            //     // Debug: Log the total number of route tabs
            ...
            //     // Determine the URL based on the OPTIMIZE_ROUTE flag
            ...
            //     // Debug: Log the URL being used for the AJAX request
            //     // console.log('AJAX URL:', _url);
            ...
            //             contentType: false,
            //             cache: false,
            //             timeout: 800000, // 800 seconds (13.3 minutes)
            //             dataType: 'json'
            //         });

            //         // Debug: Log the response from the server
            //         // console.log('Response:', response);
            //         if (response.result_success && response.i_result && Array.isArray(response.inserted_itineries)) {
            //             if (Array.isArray(response.inserted_itineries)) {
            //                 response.inserted_itineries.forEach(itinerary_plan_ID => {
            //                     // Create a response object for each itinerary
            //                     const itineraryResponse = {
            //                         i_result: true,
            //                         success: true,
            //                         redirect_URL: `latestitinerary.php?route=edit&formtype=generate_itinerary&id=${itinerary_plan_ID}&selected_group_type=1`,
            //                         itinerary_plan_ID: itinerary_plan_ID,
            //                         itinerary_prefrence: response.itinerary_prefrence,
            //                         result_success: true
            //                     };

            //                     // Debug: Log the itinerary response
            //                     // console.log('Handling response for itinerary:', itineraryResponse);

            //                     // Handle the response for each itinerary
            //                     handleDefaultResponse(itineraryResponse, itineraryResponse.redirect_URL);
            //                 });
            //             }
            //         } else {
            ...
            // 1) Keep this loader only at the outer boundary
            async function submitForm(OPTIMIZE_ROUTE) {
                let totalRouteTabs = $(".tab-pane.fade").length;
                ...
                try {
                    ...
                    // Gate check: be tolerant to different success flags
                    const itineraries = response?.inserted_itineries;
                    const ok = response?.success || response?.result_success || response?.i_result;

                    if (ok && Array.isArray(itineraries) && itineraries.length > 0) {
                        TOAST_NOTIFICATION('success', 'Itinerary Basic Details Added', 'Success !!!');

                        // 2) Pre-create vehicle plans for ALL itineraries *before* redirect
                        const results = await createVehiclePlansForAll(itineraries);

                        // Optional: surface any failures
                        const failures = results.filter(r => !r.success);
                        if (failures.length) {
                            failures.forEach(f => {
                                TOAST_NOTIFICATION('error', 'Vehicle Plan', `Failed for itinerary ${f.id}: ${f.message || 'Unknown error'}`);
                            });
                        }

                        // 3) Redirect *once* to the first itinerary (single tab)
                        const firstId = itineraries[0];
                        const redirect_URL = `latestitinerary.php?route=add&formtype=generate_itinerary&id=${firstId}&selected_group_type=1`;
                        window.location.href = redirect_URL;

                    } else {
                        // Fall back to your existing handler
                        handleResponse(response, response?.redirect_URL);
                    }

                } catch (error) {
                    console.error('Error submitting form:', error);
                    handleErrors(['Network or server error during submission']);
                } finally {
                    toggleLoader(false);
                }
            }
            ...
            /**
             * Sequentially create vehicle plans for every itinerary.
             * Sequential = safer for your server and guarantees order.
             */
            async function createVehiclePlansForAll(itineraryIds) {
                const out = [];
                for (const id of itineraryIds) {
                    try {
                        const resp = await getdefaultVEHICLEPLANDETAILS(id);
                        if (resp?.success) {
                            out.push({
                                id,
                                success: true
                            ...
                            out.push({
                                id,
                                success: false,
                                message: (resp && resp.errors && resp.errors[0]) || 'Unknown error'
                            });
                        }
                    } catch (e) {
                        out.push({
            ...
            // 3) Keep this function focused. No loader toggles here (outer handles it).
            async function getdefaultVEHICLEPLANDETAILS(itinerary_plan_ID) {
                try {
                    return await $.ajax({
                        type: 'post',
                        url: 'engine/ajax/ajax_latest_itineary_manage_vehicle_details.php?type=add_vehicle_plan',
                    ...
                    console.error('Error fetching vehicle plan details:', error);
                    return {
                        success: false,
            ...
            // 4) Your existing helpers are fine; keep them.
            function handleErrors(errors) {
                console.error('Errors:', errors);
                if (Array.isArray(errors) && errors.length > 0) {
                    errors.forEach(error => {
                        TOAST_NOTIFICATION('error', 'Error', error);
                    });
                } else {
                    TOAST_NOTIFICATION('error', 'Error', 'An unknown error occurred.');
                }
            }

            async function handleDefaultResponse(
                response,
                redirect_URL,
                index = 0,
                totalCount = 1,
                ...
                if (!response?.success) {
                    console.error('Response was not successful:', response?.errors);
                    handleErrors(response?.errors || ['Unknown error']);
                    return;
                }

                const selected_itinerary_preference = response.itinerary_prefrence;
                TOAST_NOTIFICATION('success', 'Itinerary Basic Details Added', 'Success !!!');

                try {
                    const delay = Number.isFinite(index) ? index * 300 : 0;

                    const vehicleResponse = await new Promise((resolve) => {
                        setTimeout(async () => {
                            const vResp = await getdefaultVEHICLEPLANDETAILS(redirect_URL, response.itinerary_plan_ID);

                            if (vResp && vResp.success) {
                                // Open logic:
                                if (index === 0) {
                                    // ✅ First route → current tab
                                    window.location.href = redirect_URL; // or: window.open(redirect_URL, '_self')
                                } else {
                                    // ✅ Others → new tabs
                                    window.open(redirect_URL, '_blank', 'noopener,noreferrer');
                                }
                                if (windowOpenCounter) windowOpenCounter.count++;
                            } else {
                                console.error('Error in vehicle details response:', vResp ? vResp.errors : 'Unknown error');
                                handleErrors(vResp ? vResp.errors : ['Unknown error occurred']);
                            }
            ...
            /* async function getdefaultVEHICLEPLANDETAILS(redirect_URL, itinerary_plan_ID) {
                ...
                try {
                    ...
                    // console.log('Vehicle Plan Response:', response); // Log the response
            ...
            function handleDefaultVehiclePlanResponse(response, redirect_URL) {
                if (response && response.success) {
                    window.open(redirect_URL);
                } else {
                    console.error('Invalid response or error in vehicle details:', response);
                    handleErrors(response ? response.errors : ['Unknown error occurred']);
                }
            }

            /* function handleErrors(errors) {
                console.error('Errors:', errors);
                // Display errors to the user, e.g., using a notification or alert
                if (Array.isArray(errors) && errors.length > 0) {
                    errors.forEach(error => {
                        TOAST_NOTIFICATION('error', 'Error', error);
                    });
            ...
            function initializeEasyAutocomplete(add_row, day_no) {
                ...
                fetchDestinationLocations(day_no, function(response) {
                    // console.log('initializeSelectize:', nextVisitingInput, response);
            ...
            function initializeDefaultAutocomplete(tabIndex, dayNum) {
                ...
                let prevVisitingLocationId = `#next_visiting_location_${tabIndex}_${prevDayNum}`;
                ...
                let sourceLocation = $(prevVisitingLocation);
