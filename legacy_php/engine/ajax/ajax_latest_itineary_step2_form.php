...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'show_form') :
        ...
        if ($agent_margin_value != '' && $agent_margin_value != '0' && ($logged_agent_id != '' && $logged_agent_id != '0')):
            $total_discount_amount = $total_net_amount - $total_margin_discount + $agent_margin_value;
        else:
...
?>
            ...
            <div class="col-md-12">
                <div class="card itinerary-header-title-sticky p-3 py-2">
                    <div class=" d-flex justify-content-between align-items-center">
                        ...
                        <a href="latestitinerary.php?route=edit&formtype=basic_info&id=<?= $itinerary_plan_ID; ?>" type="button" class="btn btn-sm btn-label-github waves-effect ps-3"><i class="tf-icons ti ti-arrow-left ti-xs me-1"></i> Back to Route List</a>
                    </div>
                </div>
                <?php
                ...
                <?php
                if ($itinerary_type !=2):
                    $multiple_itinerary_quote_ID = explode('-', $itinerary_quote_ID);
                    $main_quote_ID = $multiple_itinerary_quote_ID[0];
                    $select_itinerary_multiple_route_suggestion_details = sqlQUERY_LABEL("SELECT `itinerary_plan_ID`, `itinerary_quote_ID` FROM `dvi_itinerary_plan_details` WHERE `itinerary_quote_ID` LIKE '%$main_quote_ID%' AND `itinerary_type` = '$itinerary_type'") or die("#1-UNABLE_TO_COLLECT_MULTIPLE_ROUTE_LIST:" . sqlERROR_LABEL());
                    $total_no_of_route_suggestion = sqlNUMOFROW_LABEL($select_itinerary_multiple_route_suggestion_details);
                    if ($total_no_of_route_suggestion > 0):
                ?>
                        <div class="row mt-3 itinerary-header-title-sticky bg-transparent">
                            <?php
                            while ($fetch_route_suggestion_data = sqlFETCHARRAY_LABEL($select_itinerary_multiple_route_suggestion_details)) :
                                $route_count++;
                                $route_suggestion_itinerary_plan_ID = $fetch_route_suggestion_data['itinerary_plan_ID'];
                                $itinerary_quote_ID = $fetch_route_suggestion_data['itinerary_quote_ID'];

                                if ($route_suggestion_itinerary_plan_ID == $_POST['_ID']):
                                    $bg_color = 'bg-success';
                                    $text_color = 'text-white';
                                else:
                                    $bg_color = '';
                                    $text_color = 'text-blue-color';
                                endif;
                            ?>
                                <div class="col-auto">
                                    <div class="card card-body <?= $bg_color; ?> p-2">
                                        <a href="latestitinerary.php?route=add&formtype=generate_itinerary&id=<?= $route_suggestion_itinerary_plan_ID; ?>">
                                            <h6 class="m-0 <?= $text_color; ?> text-center">Route #<?= $route_count; ?></h6>
                                        </a>
                                    </div>
                                </div>
                            <?php
                ...
                <div class="itinerary-header-sticky-element card-header sticky-element bg-label-primary p-3 mt-3">
                    ...
                    <div class="d-flex align-items-center justify-content-between mt-2">
                        ...
                        <h5 class="card-title mb-sm-0">Overall Trip Cost : <b class="text-primary fs-4"><span id="overall_trip_cost"><?= general_currency_symbol . ' ' . number_format(round($total_discount_amount), 2); ?></span></b>
                        </h5>
                    </div>
                </div>
                                                ...
                                                <div id="accordionIcon" class="accordion accordion-without-arrow">
                                                    ...
                                                    if ($total_itinerary_plan_route_details_count > 0) :
                                                        ...
                                                        while ($fetch_itinerary_plan_route_data = sqlFETCHARRAY_LABEL($select_itinerary_plan_route_details_query)) :
                                                            ...
                                                            if (trim($departure_location) == trim($next_visiting_location) && $no_of_days == $itineary_route_count) :
        ...
        <!-- Full-page centered loader (hidden by default) -->
...
">
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                <?php if (in_array($itinerary_preference, array(1, 2, 3))) : ?>
            ...
            function showHOTSPOTLIST(itinerary_plan_ID, itinerary_route_ID, itinerary_route_date) {
                console.log("Showing hotspot list for itinerary plan ID: " + itinerary_plan_ID + ", route ID: " + itinerary_route_ID + ", route date: " + itinerary_route_date);

                var group_type = '<?= $selected_group_type; ?>';

                // ✅ Show loader only when this function is called
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        // ✅ Hide loader before displaying modal
                    ...
                    error: function(xhr, status, error) {
                        document.getElementById('pageLoader').style.display = 'none';
                        alert('Error loading hotspot list. Please try again.');
                        console.error(error);
                    }
                });
            }
            ...
            function showOVERALLCOSTAMOUNT(itinerary_plan_ID, group_type) {
                var agent_margin_input_data = $('.agent-margin-input').val();
                $.ajax({
                    type: "POST",
                    ...
                    data: {
                        _itinerary_plan_ID: itinerary_plan_ID,
                        _groupTYPE: group_type,
                        _agent_margin_input_data: agent_margin_input_data
                    },
                    success: function(response) {
