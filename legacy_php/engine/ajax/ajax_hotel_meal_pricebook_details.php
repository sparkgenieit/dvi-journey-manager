...
HTML;
    ...
    <table>
        ...
        foreach ($months as $monthYear) {
            ...
            if ($monthIndex > 0) {
                $html .= '<tr><th style="border-top-left-radius:0">Meal Type</th></tr>';
            }
            foreach ($mealTypes as $mtId => $mtTitle) {
                $html .= '<tr>
            <td class="text-start">' . htmlspecialchars($mtTitle) . '</td>
        </tr>';
            }
            $monthIndex++;
        }
        ...
        // ---------- RIGHT: scrollable (one table per month; columns = days; rows = meal types in same order) ----------
        ...
        foreach ($months as $monthYear) {
            ...
            // Fetch all meal rows for the month at once; index by meal_type
            $price_sql = "
        SELECT meal_type, $selectCols
    ...
    ";
            ...
            $price_idx = []; // key = meal_type
            while ($pr = sqlFETCHARRAY_LABEL($price_res)) {
                $price_idx[(int)$pr['meal_type']] = $pr;
            }
