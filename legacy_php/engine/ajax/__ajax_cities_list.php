...
?>
        ...
        <div class="col-md-12">
            <div class="card p-0">
                <div class="card-header pb-3 d-flex justify-content-between">
                    <h5 class="card-title mb-3">List of Cities</h5>
                    <a href="javascript:void(0)" class="btn btn-label-primary waves-effect"
                        onclick="showCITIESMODAL(0);" data-bs-dismiss="modal">+ Add City</a>
                </div>
                <div class="card-body dataTable_select text-nowrap">
                    <div class="text-nowrap table-responsive table-bordered">
                        <table class="table table-hover " id="cities_LIST">
                            <thead>
                                <tr>
    ...
    <!--ADD UPDATE CITY MODEL -->
    <div class="modal fade" id="addCITYFORM" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-simple modal-enable-otp modal-dialog-centered">
            <div class="modal-content p-3 p-md-5">
                <div class="receiving-update-cities-form-data">
                </div>
            </div>
        </div>
    </div>

    <!--DELETE UPDATE CITY MODEL -->
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#cities_LIST').DataTable({
                ...
                buttons: [{
                        ...
                        exportOptions: {
                            columns: [0, 2, 3], // Only name, email and role
                        }
                    },
                    {
                        extend: 'excel',
                ...
                ajax: {
                    "url": "engine/json/__JSONcities.php",
                    "type": "GET"
                },
                columns: [{
                        data: "count"
        ...
        //ADD & DELETE MODAL
        function showCITIESMODAL(CITY_ID) {
            $('.receiving-update-cities-form-data').load(
                'engine/ajax/__ajax_add_cities.php?type=show_form&CITY_ID=' + CITY_ID + '',
                function() {
                    const container = document.getElementById("addCITYFORM");
                    const modal = new bootstrap.Modal(container);
                    modal.show();
                    if (CITY_ID) {
                        $('#CITYFORMLabel').html('Update City');
                    } else {
                        $('#CITYFORMLabel').html('Add City');
                    }
                });
        }
        ...
        function showDELETECITYMODAL(ID) {
            $('.receiving-confirm-delete-form-data').load('engine/ajax/__ajax_manage_cities.php?type=delete&ID=' +
                ID,
                function() {
        ...
        function confirmCITYDELETE(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/__ajax_manage_cities.php?type=confirmdelete",
                data: {
                    _ID: ID
                },
                dataType: 'json',
