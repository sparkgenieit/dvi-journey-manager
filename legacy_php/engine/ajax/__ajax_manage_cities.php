...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'add') :
        ...
        $hiddenCITY_ID = $validation_globalclass->sanitize($_POST['hiddenCITY_ID']);
        ...
        if (!empty($errors)) :
            ...
            if ($hiddenCITY_ID != '' && $hiddenCITY_ID != 0 && (!empty($hiddenCITY_ID))) :
                $sqlwhere = " `id` = '$hiddenCITY_ID' ";

                //UPDATE CITY INFO
                if (sqlACTIONS("UPDATE", "dvi_cities", $arrFields, $arrValues, $sqlwhere)) :
                    //SUCCESS
    ...
    elseif ($_GET['type'] == 'delete') :
        ...
        $select_city_id_already_used = sqlQUERY_LABEL("SELECT COUNT(`hotel_city`) AS TOTAL_USED_COUNT FROM `dvi_hotel` WHERE `status` = '1' and `hotel_category` = '$ID' and `deleted` = '0'") or die("#1-UNABLE_TO_COLLECT_DETAILS:" . sqlERROR_LABEL());
        while ($fetch_hotel_used_data = sqlFETCHARRAY_LABEL($select_city_id_already_used)) :
            ...
            <div class="row">
                <?php if ($TOTAL_USED_COUNT == 0) : ?>
                    ...
                    <div class="text-center pb-0">
                        ...
                        <button type="submit" onclick="confirmCITYDELETE('<?= $ID; ?>');" class="btn btn-danger">Delete</button>
                    </div>
                <?php else : ?>
                    <div class="text-center">
...
<?php
    elseif ($_GET['type'] == 'confirmdelete') :
        ...
        $delete_CITY = sqlQUERY_LABEL("UPDATE `dvi_cities` SET `deleted` = '1', `updatedon` = '" . date('Y-m-d H:i:s') . "' WHERE `id` = '$_ID'") or die("#1-UNABLE_TO_DELETE_delete_CITY:" . sqlERROR_LABEL());
        if ($delete_CITY) :
