...
<body>
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <span id="showTRAVELEXPERTMOMENTLIST"></span>
                        <span id="showTRAVELEXPERTMOMENTDAY"></span>
                    </div>
                    <!-- / Content -->
                    <!-- Footer -->
    ...
    <script>
         $(document).ready(function() {
            ...
            <?php if ($_GET['formtype'] == '') : ?>
                show_MOMENT_LIST();
            <?php elseif ($_GET['formtype'] == 'day_list'): ?>
                show_MOMENT_DAYLIST('<?= $_GET['id']; ?>');
            <?php elseif ($_GET['formtype'] == 'day_list_guide'): ?>
                show_MOMENT_GUIDE_DAYLIST('<?= $_GET['id']; ?>');
            <?php endif; ?>
        });

        function show_MOMENT_LIST() {
            $.ajax({
                type: 'post',
                url: 'engine/ajax/__ajax_travelexpert_moment.php?type=show_form',
                success: function(response) { 
                    $('#showTRAVELEXPERTMOMENTDAY').html('');
                    $('#showTRAVELEXPERTMOMENTLIST').html(response);
                }
            });
        }

        function show_MOMENT_DAYLIST(ID) {
            $.ajax({
                type: 'post',
                url: 'engine/ajax/__ajax_travelexpert_moment.php?type=day_list',
                data: {
                    ID: ID,
                },
                success: function(response) {
                    $('#showTRAVELEXPERTMOMENTLIST').html('');
                    $('#showTRAVELEXPERTMOMENTDAY').html(response);
                }
            });
        }

        function show_MOMENT_GUIDE_DAYLIST(ID) {
            $.ajax({
                type: 'post',
                url: 'engine/ajax/__ajax_travelexpert_guidemoment.php?type=day_list_guide',
                data: {
                    ID: ID,
                },
                success: function(response) {
