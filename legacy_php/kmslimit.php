...
$current_page = 'kmslimit.php';
admin_reguser_protect(); // Set the current page variable

if ($logged_vendor_id != '' && $logged_vendor_id != 0) :
    $select_kmlimit_details = sqlQUERY_LABEL("SELECT `kms_limit_id` FROM `dvi_kms_limit` WHERE `deleted` = '0' AND `vendor_id` = '$logged_vendor_id'") or die("#1-UNABLE_TO_COLLECT_KMS_LIMIT_DETAILS:" . sqlERROR_LABEL());
    $kmlimit_exist = sqlNUMOFROW_LABEL($select_kmlimit_details);
                            ...
                            <div class="col-md-12">
                                <div class="card p-0">
                                    <div class="card-header pb-3 d-flex justify-content-between">
                                        <h5 class="card-title mb-3">List of Kilometer Limit</h5>
                                        <a href="javascript:void(0)" class="btn btn-label-primary waves-effect" onclick="showKMSLIMITMODAL(0);" data-bs-dismiss="modal">+ Add Kilometer Limit</a>
                                    </div>
                                    <div class="card-body dataTable_select text-nowrap">
                                        <div class="text-nowrap table-responsive table-bordered">
                                            <table class="table table-hover " id="kms_limit_LIST">
                                                <thead>
                                                    <tr>
                                                        <th scope="col">S.No</th>
                                                        <th scope="col">Kilometer Limit Title</th>
                            ...
                            <div class="col-md-12">
                                <div class="card p-0">
                                    <div class="card-header pb-3 d-flex justify-content-between">
                                        ...
                                        ?>
                                        <a id="btn_add_kmlimit" href="javascript:void(0)" class="btn btn-label-primary waves-effect" onclick="showKMSLIMITMODAL(0);" data-bs-dismiss="modal">+ Add Outstation KM Limit</a>
                                        <?php //endif; 
                                        ?>
                                    </div>
                                    <div class="card-body dataTable_select text-nowrap">
    ...
    <script>
        $(document).ready(function() {
            $('#kms_limit_LIST').DataTable({
                ...
                ajax: {
                    "url": "engine/json/__JSONkmslimit.php",
                    "type": "GET"
                },
                columns: [{
                        data: "count"
        ...
        //ADD & DELETE MODAL
        function showKMSLIMITMODAL(KMS_LIMIT_ID) {
            $('.receiving-kms-limit-form-data').load('engine/ajax/__ajax_add_kmslimit.php?type=show_form&KMS_LIMIT_ID=' + KMS_LIMIT_ID + '', function() {
        ...
        //SHOW DELETE POPUP
        function showDELETEKMSLIMITMODAL(ID) {
            $('.receiving-confirm-delete-form-data').load('engine/ajax/__ajax_manage_kmslimit.php?type=delete&ID=' + ID, function() {
        ...
        function confirmKMSLIMITDELETE(ID) {
            $.ajax({
                type: "POST",
