                     ...
                     FROM 
                       ...
                       WHERE 
                           activity_details.`deleted` = '0'
                            AND activity_details.`activity_ID` = $activity_id
...
$get_activity_data_query = sqlQUERY_LABEL("
   SELECT 
                       ...
                       WHERE 
                           ...
                           {$filterbyaccountsquoteid_activity}
                           {$filterbyaccountsactivity_join}
                           {$filterbyaccounts_date} GROUP BY activity_details.accounts_itinerary_activity_details_ID
...
if (sqlNUMOFROW_LABEL($get_activity_data_query)):
    ...
    while ($fetch_data = sqlFETCHARRAY_LABEL($get_activity_data_query)) :
        ...
        $accounts_itinerary_activity_details_ID = $fetch_data['accounts_itinerary_activity_details_ID'];
        ...
        $activity_amount = $fetch_data['activity_amount'];
        $total_activity_purchase = 0;
        $total_activity_purchase += $fetch_data['activity_amount'];
        ...
        $hotspotHeaders = [
            ...
            'Total Purchase Amount' => $activity_amount
        ...
        $get_activitypaid_data_query = sqlQUERY_LABEL("
 SELECT 
                       ...
                       WHERE 
                           ...
                           AND activity_details.`activity_ID` = $activity_id
                           AND transaction_history.`deleted` = '0'
                           AND activity_details.`accounts_itinerary_activity_details_ID` = $accounts_itinerary_activity_details_ID
                           {$filterbyaccountsquoteid_activity}
                           {$filterbyaccountsactivity_join}
                           {$filterbyaccounts_date}
    ") or die("#get_activitypaid_data_query: " . sqlERROR_LABEL());

        if (sqlNUMOFROW_LABEL($get_activitypaid_data_query)):
            $total_transaction_amount = 0;
            $payment_counter = 0;
            while ($fetch_data = sqlFETCHARRAY_LABEL($get_activitypaid_data_query)) :
                $payment_counter++;
    ...
    if ($mode_of_pay ==  1) {
        ...
        $mode_of_pay_label = "Net Banking";
            ...
            $total_balance = $total_activity_purchase - $total_transaction_amount;
...
header('Content-Disposition: attachment; filename="ITINERARY-' . $activity_name . '.xlsx"');
