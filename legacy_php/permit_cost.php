...
$current_page = 'permit_cost.php'; // Set the current page variable
?>
<!DOCTYPE html>
...
<body>
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <?php if (empty($_GET['route']) && $_GET['route'] == '') : ?>
                            ...
                            <!-- Permit Cost List Table -->
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="card p-0">
                                        <div class="card-header pb-3 d-flex justify-content-between">
                                            <h5 class="card-title mb-3">List of Permit Cost</h5>
                                            <a href="permit_cost.php?route=add" class="btn btn-label-primary waves-effect">+ Add Permit Cost</a>
                                        </div>
                                        <div class="card-body dataTable_select text-nowrap">
                                            <table class="table table-flush-spacing border table-bordered table-responsive" id="permit_cost_LIST">
                                                <thead class="table-head">
                                                    <tr>
                                                        <th scope="col">S.No</th><!-- 1 -->
                                                        <th scope="col">Source State</th><!-- 2 -->
                                                        <th scope="col">Destination State</th><!-- 3 -->
                                                        <th scope="col">Cost</th><!-- 4 -->
                                                        <th scope="col">Status</th><!-- 5 -->
                                                        <th scope="col">Action</th><!-- 6 -->
                                                    </tr>
                                                </thead>
                                            </table>
                                        </div>
                                ...
                                <div class="col-md-12">
                                    <div class="card p-0">
                                        <!-- <div class="card-header pb-3 d-flex justify-content-center">
                                            <h4 class="card-title mb-0">Add Permit Cost</h4>
                                        </div> -->
                                        <div class="card-body dataTable_select text-nowrap">
                                            <div class="col-md-4 mb-3">
                                                <label class="form-label" for="selected_state">State <span class="text-danger">*</span></label>
                                                <select class="form-select" name="selected_state" id="hotel_country" onchange="toggleStateInputs();" required>
                                                    <option value="">Select Any One</option>
                                                </select>
                                            </div>
                                            <div class="row" id="stateInputContainer">
                                                <!-- State-specific input fields will be dynamically generated here -->
                                            </div>
                                            <div class=" text-center mt-3">
                                                <button type="submit" class="btn btn-primary float-end ms-2" id="rolepermission_form_submit">Save</button>
                                                <a class="btn btn-light float-start" href="permit_cost.php" data-bs-dismiss=" modal">Back</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="modal fade" id="addPERMITCOSTFORM" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-simple modal-enable-otp modal-dialog-centered">
            <div class="modal-content p-3 p-md-5">
                <div class="receiving-permitcost-form-data">
    ...
    <script>
        ...
        //ADD & DELETE MODAL
        function showPERMITCOSTMODAL(PERMIT_COST_ID) {
            $('.receiving-permitcost-form-data').load('engine/ajax/__ajax_add_permitcost.php?type=show_form&PERMIT_COST_ID=' + PERMIT_COST_ID + '', function() {
                const container = document.getElementById("addPERMITCOSTFORM");
                const modal = new bootstrap.Modal(container);
                modal.show();
                if (PERMIT_COST_ID) {
                    $('#PERMITCOSTFormLabel').html('Update Permit Cost');
                } else {
                    $('#PERMITCOSTFormLabel').html('Add Permit Cost');
                }
            });
        }
        //SHOW DELETE POPUP
        function showDELETEPERMITCOSTMODAL(ID) {
            $('.receiving-confirm-delete-form-data').load('engine/ajax/__ajax_manage_permitcost.php?type=delete&ID=' + ID, function() {
        ...
        function confirmPERMITCOSTDELETE(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/__ajax_manage_permitcost.php?type=confirmdelete",
                data: {
                    _ID: ID
                },
                dataType: 'json',
        ...
        //STATUS UPDATE
        function togglestatusITEM(STATUS_ID, PERMIT_COST_ID) {
            if (PERMIT_COST_ID) {
                $.ajax({
                    type: "POST",
                    url: "engine/ajax/__ajax_manage_permitcost.php?type=updatestatus",
                    data: {
                        PERMIT_COST_ID: PERMIT_COST_ID,
                        STATUS_ID: STATUS_ID
                    },
                    dataType: 'json',
                    success: function(response) {
        ...
        var stateInputMap = {
            ...
            "35": "Uttarakhand",
            "36": "West Bengal"
            // Add mappings for other states here
        };

        function populateDropdown() {
            // Get the select element
            var select = document.getElementById("hotel_country");
        ...
        function toggleStateInputs() {
            ...
            var selectedState = document.getElementById("hotel_country").value;
            ...
            if (selectedState !== "") {
                ...
                for (var stateValue in stateInputMap) {
                    if (stateValue !== selectedState) {
                        ...
                        stateInput.name = stateName.toLowerCase().replace(" ", "_") + "_cost";
