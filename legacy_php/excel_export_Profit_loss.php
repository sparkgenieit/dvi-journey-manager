...
while ($fetch_itinerary_plan_data = sqlFETCHARRAY_LABEL($select_itinerary_plan_details_query)) :
  ...
  $itinerary_total_coupon_discount_amount = round($fetch_itinerary_plan_data['itinerary_total_coupon_discount_amount']);
  $customer_name = get_CONFIRMED_ITINEARY_CUSTOMER_DETAILS($itinerary_plan_ID, 'primary_customer_name');
  $total_guide_charge = getITINEARY_CONFIRMED_TOTAL_GUIDE_CHARGES_DETAILS($itinerary_plan_ID, '', 'TOTAL_ITINEARY_CONFIRMED_GUIDE_CHARGES');
  $total_hotspot_charge = getITINEARY_CONFIRMED_COST_DETAILS($itinerary_plan_ID, '', 'total_hotspot_amount');
  $total_activity_charge = getITINEARY_CONFIRMED_COST_DETAILS($itinerary_plan_ID, '', 'total_activity_amout');
  $total_hotel_charge = getITINEARYCONFIRMED_MARGIN_COST_DETAILS($itinerary_plan_ID, 'total_payable_cost_hotel');
  $total_hotel_withoutmargin_charge = round(getITINEARYCONFIRMED_WITHOUTMARGIN_COST_DETAILS($itinerary_plan_ID, '', 'total_payable_cost_hotel'));
  $total_vehicle_charge = getITINEARYCONFIRMED_MARGIN_COST_DETAILS($itinerary_plan_ID, 'total_payable_cost_vehicle');
  $total_vehicle_withoutmargin_charge = round(getITINEARYCONFIRMED_WITHOUTMARGIN_COST_DETAILS($itinerary_plan_ID, '', 'total_payable_cost_vehicle'));
  $agent_margin_charges = get_ITINEARY_CONFIRMED_PLAN_DETAILS($itinerary_plan_ID, 'agent_margin_charges');


  $total_sale = $total_guide_charge + $total_hotspot_charge + $total_activity_charge + $total_hotel_charge + $total_vehicle_charge + $agent_margin_charges;
  $total_purchase = $total_guide_charge + $total_hotspot_charge + $total_activity_charge + $total_hotel_withoutmargin_charge + $total_vehicle_withoutmargin_charge;


  $total_margin_hotel_charge = round(getITINEARYCONFIRMED_GST_COST_DETAILS($itinerary_plan_ID, 'total_gst_cost_hotel'));
  $total_margin_vehicle_charge = round(getITINEARYCONFIRMED_GST_COST_DETAILS($itinerary_plan_ID, 'total_gst_cost_vehicle'));
  $agent_margin_gst_charges = round(get_ITINEARY_CONFIRMED_PLAN_DETAILS($itinerary_plan_ID, 'agent_margin_gst_charges'));
  ...
  $total_margingst_cost_vehicle = getITINEARYCONFIRMED_MARGINGST_COST_DETAILS($itinerary_plan_ID, 'total_margingst_withgst_cost_vehicle');

  $total_GST_sale = $total_margin_hotel_charge + $total_margin_vehicle_charge + $agent_margin_gst_charges;
  $total_GST_profit = round($total_margingst_cost_service) + round($total_margingst_cost_hotel) + round($total_margingst_cost_vehicle);

  $total_billed_amount = round($total_sale) +  $total_GST_sale - $itinerary_total_coupon_discount_amount;

  $total_profit_amount = $total_billed_amount - $total_purchase;
  $total_profit_amount_without_GST = $total_profit_amount - $total_GST_profit;
...
$headers = [
  ...
  'Total Sale (Excluding GST)' => round($total_sale),
  'Gst on Sales' => $total_GST_sale,
  'Coupon Discount' => $itinerary_total_coupon_discount_amount,
  'Total Bill Value' => $total_billed_amount,
  'Total Purchase (Including GST)' => $total_purchase,
];
...
foreach ($headers as $header => $value) :
  ...
  if (in_array($header, [
    'Total Sale (Excluding GST)',
    'Gst on Sales',
    'Coupon Discount',
    'Total Bill Value',
    'Total Purchase (Including GST)',
    'Net Profit'
  ])) :
...
$sheet->setCellValue('A' . $row, 'Net Profit (Including GST)');
$sheet->setCellValue('B' . $row, round($total_profit_amount));
$sheet->getStyle('A' . $row)->applyFromArray($lightGreenStyle);
$sheet->getStyle('B' . $row)->applyFromArray($lightGreenStyle);
$sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
$row++;
$sheet->setCellValue('A' . $row, 'GST on Net Profit');
$sheet->setCellValue('B' . $row, round($total_GST_profit));
$sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
$sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
$sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
$row++;
$sheet->setCellValue('A' . $row, 'Net Profit (Excluding GST)');
$sheet->setCellValue('B' . $row, round($total_profit_amount_without_GST));
$sheet->getStyle('A' . $row)->applyFromArray($lightGreenStyle);
$sheet->getStyle('B' . $row)->applyFromArray($lightGreenStyle);
$sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
$row++;
...
if (sqlNUMOFROW_LABEL($getstatus_query_guide)):
  ...
  while ($fetch_data = sqlFETCHARRAY_LABEL($getstatus_query_guide)) :
    ...
    $coupon_discount_amount_guide = round(get_ITINEARY_CONFIRMED_PLAN_DETAILS($itinerary_plan_ID, 'itinerary_total_coupon_discount_amount') / $discount_count);
    ...
    $total_profit =  number_format(round($total_guide_amount - $total_guide_incidental - $coupon_discount_amount_guide), 2);
  ...
  $sheet->setCellValue('A' . $row, 'Total Tax Amount');
  $sheet->setCellValue('B' . $row,  $total_guide_tax_amount);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;

  $sheet->setCellValue('A' . $row, 'Total Service Amount');
  $sheet->setCellValue('B' . $row, $total_guide_amount);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('A' . $row, "Incidental Expenses");
  $sheet->setCellValue('B' . $row, $total_guide_incidental_format);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('B' . $row, $coupon_discount_amount_guide);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;

  $sheet->setCellValue('A' . $row, "Total Guide Profit");
  $sheet->setCellValue('B' . $row, $total_profit);
  $sheet->getStyle('A' . $row)->applyFromArray($lightGreenStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($lightGreenStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
...
if (sqlNUMOFROW_LABEL($get_hotspot_data_query)):
  ...
  while ($fetch_data = sqlFETCHARRAY_LABEL($get_hotspot_data_query)) :
    ...
    $incident_hotspot_count = $getguide + $gethotspot + $getactivity;
    ...
    $discount_count = $preference_value + $incident_hotspot_count;

    $coupon_discount_amount_hotspot = round(get_ITINEARY_CONFIRMED_PLAN_DETAILS($itinerary_plan_ID, 'itinerary_total_coupon_discount_amount') / $discount_count);
    ...
    $total_profit =  round($total_hotspot_amount - $total_hotspot_incidental - $coupon_discount_amount_hotspot);
  ...
  // Add Total Hotel Cost and Total Hotel Tax Amount after amenities
  $sheet->setCellValue('A' . $row, 'Total Tax Amount');
  $sheet->setCellValue('B' . $row,  $total_hotspot_tax_amount_format);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('A' . $row, 'Total Service Amount');
  $sheet->setCellValue('B' . $row, $total_hotspot_amount_format);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  // Add Day X Total Cost
  $sheet->setCellValue('A' . $row, "Incidental Expenses");
  $sheet->setCellValue('B' . $row, $total_hotspot_incidental_format);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('A' . $row, "Coupon Discount");
  $sheet->setCellValue('B' . $row, $coupon_discount_amount_hotspot);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;

  $sheet->setCellValue('A' . $row, "Total Hotspot Profit");
  $sheet->setCellValue('B' . $row, $total_profit);
  $sheet->getStyle('A' . $row)->applyFromArray($lightGreenStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($lightGreenStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
...
if (sqlNUMOFROW_LABEL($get_activity_data_query)):
  ...
  while ($fetch_data = sqlFETCHARRAY_LABEL($get_activity_data_query)) :
    ...
    $incident_activity_count = $getguide + $gethotspot + $getactivity;
    ...
    $discount_count = $preference_value + $incident_activity_count;

    $coupon_discount_amount_activity = round(get_ITINEARY_CONFIRMED_PLAN_DETAILS($itinerary_plan_ID, 'itinerary_total_coupon_discount_amount') / $discount_count);
    ...
    $total_profit =  round($total_activity_amount - $total_activity_incidental - $coupon_discount_amount_activity);
  ...
  // Add Total Hotel Cost and Total Hotel Tax Amount after amenities
  $sheet->setCellValue('A' . $row, 'Total Tax Amount');
  $sheet->setCellValue('B' . $row,  $total_activity_tax_amount_format);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('A' . $row, 'Total Service Amount');
  $sheet->setCellValue('B' . $row, $total_activity_amount_format);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  // Add Day X Total Cost
  $sheet->setCellValue('A' . $row, "Incidental Expenses");
  $sheet->setCellValue('B' . $row, $total_activity_incidental_format);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('A' . $row, "Coupon Discount");
  $sheet->setCellValue('B' . $row, $coupon_discount_amount_activity);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;

  $sheet->setCellValue('A' . $row, "Total Activity Profit");
  $sheet->setCellValue('B' . $row, $total_profit);
  $sheet->getStyle('A' . $row)->applyFromArray($lightGreenStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($lightGreenStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
...
if (sqlNUMOFROW_LABEL($get_hotel_data_query)):
  ...
  while ($fetch_data = sqlFETCHARRAY_LABEL($get_hotel_data_query)) :
    ...
    $incident_hotel_count = $getguide + $gethotspot + $getactivity;
    ...
    $discount_count = $preference_value + $incident_hotel_count;

    $coupon_discount_amount_hotel = round(get_ITINEARY_CONFIRMED_PLAN_DETAILS($itinerary_plan_ID, 'itinerary_total_coupon_discount_amount') / $discount_count);
    ...
    $total_hotel_tax_amount += $fetch_data['hotel_margin_rate_tax_amt'];
    ...
    $total_profit_amount =  $total_hotel_margin_rate - $total_hotel_incidental - $coupon_discount_amount_hotel;
    ...
    $hotelyHeaders = [
      ...
      'Hotel GST Amount' => $gst_cost_hotel,
      'Hotel Purchase' => $total_hotel_purchase,
      'Hotel Margin' => $hotel_margin_rate,
      'Hotel Margin Tax' => $hotel_margin_rate_tax_amt
    ];
  ...
  // Add Total Hotel Cost and Total Hotel Tax Amount after amenities
  $sheet->setCellValue('A' . $row, 'Total Tax Amount');
  $sheet->setCellValue('B' . $row,  $total_hotel_tax_amount);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;

  $sheet->setCellValue('A' . $row, 'Total Margin Amount');
  $sheet->setCellValue('B' . $row, $total_hotel_margin_rate);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  // Add Day X Total Cost
  $sheet->setCellValue('A' . $row, "Incidental Expenses");
  $sheet->setCellValue('B' . $row, $total_hotel_incidental);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('A' . $row, "Coupon Discount");
  $sheet->setCellValue('B' . $row, $coupon_discount_amount_hotel);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
...
if (sqlNUMOFROW_LABEL($get_vendor_data_query)):
  ...
  while ($fetch_data = sqlFETCHARRAY_LABEL($get_vendor_data_query)) :
    ...
    $incident_vehicle_count = $getguide + $gethotspot + $getactivity;
    ...
    $discount_count = $preference_value + $incident_vehicle_count;

    $coupon_discount_amount_vehicle = round(get_ITINEARY_CONFIRMED_PLAN_DETAILS($itinerary_plan_ID, 'itinerary_total_coupon_discount_amount') / $discount_count);
    ...
    $total_profit_amount =  $total_vendor_margin_amount - $total_vendor_incidental - $coupon_discount_amount_vehicle ;
    ...
    $vendorHeaders = [
      ...
      'purchase GST' => $fetch_data['vehicle_gst_amount'],
      'Total Purchase Amount' => $total_purchase,
      'Vendor Margin' => $vendor_margin_amount,
      'Vendor Margin Tax' => $vendor_margin_gst_amount
    ];
  ...
  // Add Total Hotel Cost and Total Hotel Tax Amount after amenities
  $sheet->setCellValue('A' . $row, 'Total Tax Amount');
  $sheet->setCellValue('B' . $row,  $total_vendor_margin_gst_amount);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('A' . $row, 'Total Margin Amount');
  $sheet->setCellValue('B' . $row, $total_vendor_margin_amount);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  // Add Day X Total Cost
  $sheet->setCellValue('A' . $row, "Incidental Expenses");
  $sheet->setCellValue('B' . $row, $total_vendor_incidental);
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('A' . $row, "Coupon Discount");
  $sheet->setCellValue('B' . $row, $coupon_discount_amount_vehicle );
  $sheet->getStyle('A' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($overallCostStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
  ...
  $sheet->setCellValue('B' . $row, $total_profit_amount);
  $sheet->getStyle('A' . $row)->applyFromArray($lightGreenStyle);
  $sheet->getStyle('B' . $row)->applyFromArray($lightGreenStyle);
  $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
  $row++;
