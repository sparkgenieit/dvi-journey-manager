...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'show_form_guide') :
        ...
        while ($fetch_list_data = sqlFETCHARRAY_LABEL($select_accountsmanagerLIST)) :
            ...
            $select_accountsmanagerGUIDE_query = sqlQUERY_LABEL("SELECT `itinerary_plan_ID`, SUM(`total_paid`) AS `paid_amount`, SUM(`total_balance`) AS `balance_amount` FROM `dvi_accounts_itinerary_guide_details` WHERE `deleted` = '0' {$filterbyaccounts_date} {$filterbyaccountsmanager} {$acc_itinerary_details_ID} GROUP BY `itinerary_plan_ID`") or die("#1-UNABLE_TO_COLLECT_COURSE_LIST:" . sqlERROR_LABEL());
...
?>
                    ...
                    <div class="card-body p-3">
                        <div class="d-flex align-items-start justify-content-center">
                            <div class="content-left">
                                ...
                                <div class="text-center mt-2">
                                    <h4 class="mb-0 me-2 <?= $profit_label ?>"><?= general_currency_symbol ?> <?= number_format(round($total_profit_guide - $coupon_discount_amount), 2); ?></h4>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card p-3 mb-4 px-4">
            <div class="d-flex justify-content-between mb-3">
                <h4 class="mb-0">List of Guide Detail</h4>
                <div class="d-flex align-items-center">
                    <input type="text" id="searchGuide" class="form-control me-3" placeholder="Search...">
                    <?php if($billed_amount != 0):?>
                    <button id="export-accounts-guide-btn" class="btn btn-sm btn-label-success"><i class="ti ti-download me-2"></i>Export</button>
                    <?php endif; ?>
                </div>
            </div>
            <div class="text-nowrap  table-responsive  table-bordered">
                <table class="table table-hover" id="guide_accountsmanager_list">
                        ...
                        echo '<tbody>';
                            while ($fetch_data = sqlFETCHARRAY_LABEL($getstatus_query_guide)) :
                                ...
                                if ($ID == 2 || $total_balance_withoutformat == 0):
                                    ...
                                    $paynow_button = '<button type="button" class="btn btn-label-primary pay-now-guide-btn" data-row-id="' . $guide_id . '" data-bs-toggle="modal" data-total-inhandeguide-paynow="' . $inhand_amount_withoutformat . '" data-acc-guide-detail-id="' . $accounts_itinerary_guide_details_ID . '" data-total-balanceguide-paynow="' . $total_balance_withoutformat . '" data-itinerary-plan-id="' . $itinerary_plan_ID . '" data-bs-target=".accountmanageraddpaymentguidemodalsection">Pay Now</button>';
                                endif;
                                ...
                                $total_profit =  general_currency_symbol . ' ' . number_format(round($total_guide_amount - $total_guide_incidental - $coupon_discount_amount), 2);
        ...
        <?php endif;?>
        <div class="modal fade accountmanageraddpaymentguidemodalsection" id="accountmanageraddpaymentguidemodalsection" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-simple modal-enable-otp modal-dialog-centered">
                <div class="modal-content p-3 p-md-5">
                    <div class="modal-body">
                        ...
                        <form id="paynowguideForm" class="row g-3" action="" method="post" data-parsley-validate>
                            <div class="col-12">
                                <div>
                                    ...
                                    <input type="hidden" name="hidden_itinerary_ID" id="hidden_itinerary_ID" hidden>
                                </div>
                            </div>
                            <div class="col-12">
                                <div>
                                    <label for="payment_amount_guide" class="form-label">Payment Amount <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" required id="payment_amount_guide" name="payment_amount_guide" autocomplete="off" placeholder="Enter Payment Amount" />
                                    <input type="hidden" name="hidden_guide_id" id="hidden_guide_id">
                                    <input type="hidden" name="hidden_acc_guide_detail_id" id="hidden_acc_guide_detail_id">
        ...
        <script>
            $(document).ready(function() {
                $("#searchGuide").on("keyup", function() {
                    ...
                    $("#guide_accountsmanager_list tbody tr").filter(function() {
                ...
                $('#export-accounts-guide-btn').click(function() {
                    window.location.href = 'excel_export_accounts_manager_guide.php?id=<?= $ID ?>&from_date=<?= $from_date ?>&to_date=<?= $to_date ?>&agent_name=<?= $agent_name ?>&quote_id=<?= $quote_id ?>';
                });

                // Handle Pay Now button click
                $(document).on('click', '.pay-now-guide-btn', function() {
                    const generalCurrencySymbol = '₹';
                    const guideId = $(this).data('row-id');
                    const totalBalance = $(this).data('total-balanceguide-paynow');
                    const totalInhand = $(this).data('total-inhandeguide-paynow');
                    const accguidedetailid = $(this).data('acc-guide-detail-id');
                    const itineraryid = $(this).data('itinerary-plan-id');

                    console.log('Guide ID:', guideId);
                    console.log('Total Balance:', totalBalance);
                    console.log('Total Inhand:', totalInhand);
                    console.log('Total Inhand:', accguidedetailid);
                    console.log('Total Inhand:', itineraryid);

                    $('#paynowguideForm')[0].reset();
                    $('#hidden_guide_id').val(guideId);
                    $('#hidden_acc_guide_detail_id').val(accguidedetailid);
                    $('#hidden_itinerary_ID').val(itineraryid);
                    $('#payment_amount_guide').data('total-balanceguide-paynow', totalBalance).data('total-inhandeguide-paynow', totalInhand);
                ...
                $('#payment_amount_guide').on('input', function() {
                    var paymentAmount = parseFloat($(this).val()) || 0;
                    var totalInhand = parseFloat($(this).data('total-inhandeguide-paynow')) || 0;
                    var totalBalance = parseFloat($(this).data('total-balanceguide-paynow')) || 0;
                    ...
                    if (paymentAmount > totalInhand) {
                        ...
                        $('#paynowguideForm button[type="submit"]').attr('disabled', true);
                        ...
                        $('#paynowguideForm button[type="submit"]').attr('disabled', false);
                    }
                });

                // Clear modal on close
                $('.accountmanageraddpaymentguidemodalsection').on('hidden.bs.modal', function() {
                    $('#paynowguideForm')[0].reset();
                    $('#hidden_guide_id').val('');
                });

                // Form submission
                $("#paynowguideForm").submit(function(event) {
                    event.preventDefault();

                    var paymentAmount = $('#payment_amount_guide').val();
                    var guideId = $('#hidden_guide_id').val();

                    var form = $(this)[0];
                    var data = new FormData(form);

                    data.append('payment_amount_guide', paymentAmount);
                    data.append('hidden_guide_id', guideId);
                    ...
                    $.ajax({
                        ...
                        if (!response.success) {
                            if (response.errors.guide_processed_by_required) {
                                ...
                                $('#payment_amount_guide').focus();
                            }
                        } else {
                            show_ACCOUNTSMANAGER_GUIDE_DATA_NEW('<?= $ID; ?>', '<?= $from_date; ?>', '<?= $to_date; ?>', ' <?= $agent_name; ?>', '<?= $quote_id; ?>');
                            $('#accountmanageraddpaymentguidemodalsection').modal('hide');
                ...
                function show_ACCOUNTSMANAGER_GUIDE_DATA_NEW(ID, fromDate, toDate, agentName, selectedQuoteId) {
                    $.ajax({
                        type: 'post',
