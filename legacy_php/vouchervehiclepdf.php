...
class TCPDFCustom extends TCPDF
{
  ...
  {
    global $logged_user_level;
    global $logged_agent_id;
    ...
    if ($logged_agent_id != '' &&  $logged_agent_id != '0'):
      $company_name = get_AGENT_CONFIG_DETAILS($logged_agent_id, 'company_name');
      $agent_site_logo = get_AGENT_CONFIG_DETAILS($logged_agent_id, 'site_logo');
      $company_address = get_AGENT_CONFIG_DETAILS($logged_agent_id, 'site_address');
      $company_contact_no = getAGENT_details($logged_agent_id, '', 'get_agent_mobile_number');
      $company_email_id = getAGENT_details($logged_agent_id, '', 'get_agent_email_address');
    ...
    $table_company_details = '<table cellspacing="0" cellpadding="8" border="0">
        <tr>
            <td width="70%">
                <img src="' . $company_logo_format . '" width="100px"/>
            </td>
            <td width="30%">
...
if ($total_itinerary_plan_details_count > 0) :
  while ($fetch_itinerary_plan_data = sqlFETCHARRAY_LABEL($select_voucher_plan_details)) :
    $itinerary_booking_ID = $fetch_itinerary_plan_data['itinerary_quote_ID'];
    ...
    <tr>
        ...
        <td width="25%" style="border:4px solid #d3d3d3;">
          <br/>
            ...
            <span style="font-weight:semi-bold; font-size:12px; color:#232323;">' .   $itinerary_booking_ID . '</span> 
      </td>
       <td width="25%" style="border:4px solid #d3d3d3;">
          <br/>
  ...
  while ($fetch_itinerary_plan_data = sqlFETCHARRAY_LABEL($select_voucher_plan_details)) :
    ...
    if ($total_itinerary_plan_details_count > 0) :
      while ($fetch_itinerary_plan_data = sqlFETCHARRAY_LABEL($select_vehiclevoucher_eligible_plan_details)) :
        ...
        $total_extra_kms_charge = $fetch_itinerary_plan_data['total_extra_kms_charge'];
        $get_total_extra_kms_charge = number_format($total_extra_kms_charge, 2);

        $gets_total_rental_charges = $fetch_itinerary_plan_data['total_rental_charges'];
        $gets_total_toll_charges = $fetch_itinerary_plan_data['total_toll_charges'];
        $gets_total_parking_charges = $fetch_itinerary_plan_data['total_parking_charges'];
        $gets_total_driver_charges = $fetch_itinerary_plan_data['total_driver_charges'];
        $gets_total_permit_charges = $fetch_itinerary_plan_data['total_permit_charges'];
        $gets_total_before_6_am_extra_time = $fetch_itinerary_plan_data['total_before_6_am_extra_time'];
        $gets_total_after_8_pm_extra_time = $fetch_itinerary_plan_data['total_after_8_pm_extra_time'];
        $gets_total_before_6_am_charges_for_driver = $fetch_itinerary_plan_data['total_before_6_am_charges_for_driver'];
        $gets_total_before_6_am_charges_for_vehicle = $fetch_itinerary_plan_data['total_before_6_am_charges_for_vehicle'];
        $gets_total_after_8_pm_charges_for_driver = $fetch_itinerary_plan_data['total_after_8_pm_charges_for_driver'];
        $gets_total_after_8_pm_charges_for_vehicle = $fetch_itinerary_plan_data['total_after_8_pm_charges_for_vehicle'];


        $total_cost_of_vehicle =  $gets_total_rental_charges +  $gets_total_toll_charges +  $gets_total_parking_charges +  $gets_total_driver_charges +  $gets_total_permit_charges + $gets_total_before_6_am_charges_for_driver + $gets_total_before_6_am_charges_for_vehicle +  $gets_total_after_8_pm_charges_for_driver + $gets_total_after_8_pm_charges_for_vehicle;

        $get_total_cost_of_vehicle = number_format($total_cost_of_vehicle, 2);
        $customer_salutation = get_CONFIRMED_ITINEARY_CUSTOMER_DETAILS($itinerary_plan_ID, 'customer_salutation');
                    ...
                    <tr>
                        <td width="25%" style="border:4px solid #d3d3d3;">
                          <br/>
                            ...
                            <span style="font-weight:semi-bold; font-size:12px; color:#232323;">' . $customer_salutation . ' ' . $customer_name . '</span> 
                      </td>
                        <td width="25%" style="border:4px solid #d3d3d3;">
                          <br/>
                            <span style="font-weight:regular; font-size:12px; color:#8e8a8a;"s>Guest Age:</span><br/>
                        ...
                        <table cellspacing="0" cellpadding="8" border="0">
                            <tr>
                                <td width="65%" style="border:4px solid #d3d3d3;">
                                    <br/>
                                    <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">Booking ID : </span><span style="font-weight:bold; font-size:12px; color:#232323;">' . $itinerary_booking_ID . '</span>
    ...
    if ($total_itinerary_plan_details_count > 0) :
      ...
      while ($fetch_itinerary_plan_data = sqlFETCHARRAY_LABEL($select_vehicle_daywise)) :
        ...
        $total_pickup_km = (!empty($fetch_itinerary_plan_data['total_pickup_km']) ? number_format($fetch_itinerary_plan_data['total_pickup_km'], 2) : '');
        $total_pickup_duration = formatTimeDuration($fetch_itinerary_plan_data['total_pickup_duration']);
        $total_drop_km = (!empty($fetch_itinerary_plan_data['total_drop_km']) ? number_format($fetch_itinerary_plan_data['total_drop_km'], 2) : '');
        $total_drop_duration = formatTimeDuration($fetch_itinerary_plan_data['total_drop_duration']);
        $vehicle_rental_charges = (!empty($fetch_itinerary_plan_data['vehicle_rental_charges']) ? number_format($fetch_itinerary_plan_data['vehicle_rental_charges'], 2) : '');
        $vehicle_toll_charges = (!empty($fetch_itinerary_plan_data['vehicle_toll_charges']) ? number_format($fetch_itinerary_plan_data['vehicle_toll_charges'], 2) : '');
        $vehicle_parking_charges = (!empty($fetch_itinerary_plan_data['vehicle_parking_charges']) ? number_format($fetch_itinerary_plan_data['vehicle_parking_charges'], 2) : '');
        $vehicle_driver_charges = (!empty($fetch_itinerary_plan_data['vehicle_driver_charges']) ? number_format($fetch_itinerary_plan_data['vehicle_driver_charges'], 2) : '');
        $vehicle_permit_charges = (!empty($fetch_itinerary_plan_data['vehicle_permit_charges']) ? number_format($fetch_itinerary_plan_data['vehicle_permit_charges'], 2) : '');
        $before_6_am_extra_time = (!empty($fetch_itinerary_plan_data['before_6_am_extra_time']) ? number_format($fetch_itinerary_plan_data['before_6_am_extra_time'], 2) : '');
        $after_8_pm_extra_time = (!empty($fetch_itinerary_plan_data['after_8_pm_extra_time']) ? number_format($fetch_itinerary_plan_data['after_8_pm_extra_time'], 2) : '');
        $before_6_am_charges_for_driver = (!empty($fetch_itinerary_plan_data['before_6_am_charges_for_driver']) ? number_format($fetch_itinerary_plan_data['before_6_am_charges_for_driver'], 2) : '');
        $before_6_am_charges_for_vehicle = (!empty($fetch_itinerary_plan_data['before_6_am_charges_for_vehicle']) ? number_format($fetch_itinerary_plan_data['before_6_am_charges_for_vehicle'], 2) : '');
        $after_8_pm_charges_for_driver = (!empty($fetch_itinerary_plan_data['after_8_pm_charges_for_driver']) ? number_format($fetch_itinerary_plan_data['after_8_pm_charges_for_driver'], 2) : '');
        $after_8_pm_charges_for_vehicle = (!empty($fetch_itinerary_plan_data['after_8_pm_charges_for_vehicle']) ? number_format($fetch_itinerary_plan_data['after_8_pm_charges_for_vehicle'], 2) : '');
        $total_vehicle_amount = (!empty($fetch_itinerary_plan_data['total_vehicle_amount']) ? number_format($fetch_itinerary_plan_data['total_vehicle_amount'], 2) : '');
        ...
        $get_total_running_km = (!empty($total_running_km) ? number_format($total_running_km, 2) : '');
        $get_total_running_time = formatTimeDuration($total_running_time);
        $get_total_siteseeing_km = (!empty($total_siteseeing_km) ? number_format($total_siteseeing_km, 2) : '');
        $get_total_siteseeing_time = formatTimeDuration($total_siteseeing_time);
        $get_total_travelled_km = (!empty($total_travelled_km) ? number_format($total_travelled_km, 2) : '');
        ...
        if ($logged_user_level != 4):
          ...
          if ($logged_vendor_id == '' || $logged_vendor_id == '0'):
            $vehicle_details .= '<td width="25%" align="left" style="border:4px solid #d3d3d3;"><br/>
            ...
            $vehicle_details .= '<td width="50%" align="left" style="border:4px solid #d3d3d3;"><br/>
                                       <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">Total KM : </span> <br/>
                                       <span style="font-weight:semi-bold; font-size:12px; color:#232323;">' . $get_total_travelled_km . ' KM</span>
                                        </td>';
          endif;
    ...
    <tr>
         ...
         <tr>
            <td width="20%" style="border:4px solid #d3d3d3;">
              <br/>
                ...
                <span style="font-weight:semi-bold; font-size:12px; color:#232323;"><span style="font-family:DejaVuSans;">' . general_currency_symbol . '</span> ' . number_format($gets_total_rental_charges, 2) . '</span> 
           </td>
            <td width="20%" style="border:4px solid #d3d3d3;">
              <br/>
                <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">Toll Charges</span><br/>
                <span style="font-weight:semi-bold; font-size:12px; color:#232323;"><span style="font-family:DejaVuSans;">' . general_currency_symbol . '</span> ' . number_format($gets_total_toll_charges, 2) . '</span> 
           </td>
            <td width="20%" style="border:4px solid #d3d3d3;">
              <br/>
                <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">Parking Charges</span><br/>
                <span style="font-weight:semi-bold; font-size:12px; color:#232323;"><span style="font-family:DejaVuSans;">' . general_currency_symbol . '</span> ' . number_format($gets_total_parking_charges, 2) . '</span> 
           </td>
            <td width="20%" style="border:4px solid #d3d3d3;">
              <br/>
                <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">Driver Charges</span><br/>
                <span style="font-weight:semi-bold; font-size:12px; color:#232323;"><span style="font-family:DejaVuSans;">' . general_currency_symbol . '</span> ' . number_format($gets_total_driver_charges, 2) . '</span> 
           </td>
            <td width="20%" style="border:4px solid #d3d3d3;">
              <br/>
                <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">Permit Charges</span><br/>
                <span style="font-weight:semi-bold; font-size:12px; color:#232323;"><span style="font-family:DejaVuSans;">' . general_currency_symbol . '</span> ' . $gets_total_permit_charges . '</span> 
           </td>
        </tr>
         <tr>
          <td width="80%"  style="border:4px solid #d3d3d3;">
             <br/>
             <span style="font-weight:bold; font-size:12px; color:#232323;">TOTAL USED KM</span>
          </td>
           
          <td width="20%"  style="border:4px solid #d3d3d3; text-align:right;">
             <br/>
              <span style="font-weight:semi-bold; font-size:12px; color:#232323;">' . $total_kms . ' KM</span> 
          </td>
        </tr>
         <tr>
          <td width="40%"  style="border:4px solid #d3d3d3;">
             <br/>
      ...
      <tr>
            <td width="40%" style="border:4px solid #d3d3d3;">
              <br/>
                <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">GST ' . $vehicle_gst_percentage . '%' . '</span><br/>
            ...
            <td width="30%" style="border:4px solid #d3d3d3;">
              <br/>
                <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">Margin' . ' ' . '(' . $vendor_margin_percentage . '%' . ' ) ' . '</span><br/>
            ...
            <td width="30%" style="border:4px solid #d3d3d3;">
              <br/>
                <span style="font-weight:regular; font-size:12px; color:#8e8a8a;">Margin Service Tax' . ' ' . $vendor_margin_gst_percentage . '%' . '</span><br/>
...
<tr>
  ...
  $pdf->writeHTML($table_terms_condition, true, false, false, false, '');
  $pdf->Output('itinerary-voucher - ' . $itinerary_booking_ID . '.pdf', 'I');
