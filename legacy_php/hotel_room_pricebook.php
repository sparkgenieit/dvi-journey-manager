...
ob_start();
session_start();
$hotelprice_import_session_id = session_id();
...
if ($_GET['regen'] == 'y' && $_GET['route'] == 'import') :
  ...
  $hotelprice_import_session_id = session_id();
  echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import'; </script>";
elseif ($_GET['regen'] == 'y') :
  session_regenerate_id(TRUE);
  $hotelprice_import_session_id = session_id();
  echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php'; </script>";
endif;
...
if (isset($_POST['upload_csv']) && $_POST['upload_csv'] == 'confirm_upload') :
  ...
  if (!$file_temp_loc) : // if not file selected
    ...
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=file_temp_loc'</script>";
    die;
  endif;

  if (!preg_match("/\.(xlsx|csv)$/i", $file_name)) : // check file extension
    //echo 'Error: your file is not CSV.';
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=file_valid'</script>";
  ...
  if ($file_size > 5242880) : // file check size
    ...
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=file_size'</script>";
    die;
  endif;

  if ($file_error_msg == 1) : // 
    //echo "Error: an error occured while processing the file, try agian.";
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=file_error_msg'</script>";
    die;
  endif;
  ...
  if (!empty($hotelpricebookroom_csv_file_titles)) :

    $sample_col_count = count($hotelpricebookroom_csv_file_titles);
    ...
    if ($imported_col_count == $sample_col_count) :
      ...
      for ($i = 0; $i < $sample_col_count; $i++) :
        ...
        if (strtolower($imported_csv_file[$i]) != strtolower($hotelpricebookroom_csv_file_titles[$i])) :
          // echo $imported_csv_file[$i];
          // echo "Error: Invalid File Content, try again.";
          echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=file_mismatch'</script>";
          die;
        endif;
      endfor;
    else :
  ...
  if ($move_file != true) : // if not move to the temp location
    ...
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=move_file'</script>";
    @unlink($file_temp_loc); // remove to the temp folder
    die;
  endif;
  ...
  while (($data = fgetcsv($handle, $csvFileLength, $csvSeparator)) !== FALSE) : // while for each row
    if (!$flag) :
      ...
      if ($data[0] != '') :
        ...
        $hotel_city = trim($validation_globalclass->sanitize($data[1]));
        $hotel_name = trim($validation_globalclass->sanitize($data[2]));
        $hotel_code = trim($validation_globalclass->sanitize($data[3]));
        $getHotelID = getHOTELDETAILS($hotel_code, 'get_hotelid_from_hotelcode');
        $room_name = trim($validation_globalclass->sanitize($data[4]));
        $room_type = trim($validation_globalclass->sanitize($data[5]));
        ...
        if ($date != date('F-Y', strtotime('01-' . $date))) :
          echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?switch=E&type=monthyear_error&month=$month&year=$year&sno=$count'</script>";
          session_regenerate_id(TRUE);
          $hotelprice_import_session_id = session_id();
          exit();
        endif;
        ...
        $day29 = trim($validation_globalclass->sanitize($data[36]));
        $day30 = trim($validation_globalclass->sanitize($data[37]));
        $day31 = trim($validation_globalclass->sanitize($data[38]));
        ...
        // echo $day31;
        // exit;
        //CHECK Hotel Name TITLE IS EMPTY
        if ($hotel_name == "") :
          session_regenerate_id(TRUE);
          $hotelprice_import_session_id = session_id();
          echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=hotel_name_required&sno=$count'</script>";
          die;
        endif;

        // CHECK Hotel Code IS EMPTY
        if ($hotel_code == "") :
          session_regenerate_id(TRUE);
          $hotelprice_import_session_id = session_id();
          echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=hotel_code_required&sno=$count'</script>";
          die;
        endif;

        $arrFields = array('`csvtype`', '`sessionID`', '`field1`', '`field2`', '`field3`', '`field4`', '`field5`', '`field6`', '`field7`', '`field8`', '`field9`', '`field10`', '`field11`', '`field12`', '`field13`', '`field14`', '`field15`', '`field16`', '`field17`', '`field18`', '`field19`', '`field20`', '`field21`', '`field22`', '`field23`', '`field24`', '`field25`', '`field26`', '`field27`', '`field28`', '`field29`', '`field30`', '`field31`', '`field32`', '`field33`', '`field34`', '`field35`', '`field36`', '`field37`', '`field38`', '`field39`', '`field40`', '`status`');

        $arrValues = array("1", "$hotelprice_import_session_id", "$hotel_city", "$hotel_name", "$hotel_code", "$room_name", "$room_type", "$month", "$year", "$day1", "$day2", "$day3", "$day4", "$day5", "$day6", "$day7", "$day8", "$day9", "$day10", "$day11", "$day12", "$day13", "$day14", "$day15", "$day16", "$day17", "$day18", "$day19", "$day20", "$day21", "$day22", "$day23", "$day24", "$day25", "$day26", "$day27", "$day28", "$day29", "$day30", "$day31", "", "", "1");

        // print_r($arrValues);
        // exit;

        // CHECK HOTEL NAME NOT EXIT
        $check_hotel_name_not_exist = sqlQUERY_LABEL("SELECT `hotel_name` FROM `dvi_hotel` WHERE `deleted` = '0' and `hotel_id` = '$getHotelID'") or die("#3-CHECK_HOTEL_NAME_NOT_EXIST: UNABLE_TO_GET_DETAILS: " . sqlERROR_LABEL());
        $get_hotel_name_num_rows_count = sqlNUMOFROW_LABEL($check_hotel_name_not_exist);
        if ($get_hotel_name_num_rows_count == 0) :
          session_regenerate_id(TRUE);
          $hotelprice_import_session_id = session_id();
          echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=check_hotel_name_not_exist&hotel_name=$hotel_name'</script>";
          die;
        endif;


        // CHECK HOTEL CODE NOT EXIT
        $check_hotel_code_not_exist = sqlQUERY_LABEL("SELECT `hotel_code` FROM `dvi_hotel` WHERE `deleted` = '0' and `hotel_id` = '$getHotelID' and `hotel_code` = '$hotel_code'") or die("#3-CHECK_HOTEL_CODE_ALREADY_EXIST: UNABLE_TO_GET_DETAILS: " . sqlERROR_LABEL());
        $get_hotel_name_num_rows_count = sqlNUMOFROW_LABEL($check_hotel_code_not_exist);
        if ($get_hotel_name_num_rows_count == 0) :
          session_regenerate_id(TRUE);
          $hotelprice_import_session_id = session_id();
          echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import&switch=E&type=check_hotel_code_not_exist&hotel_code$hotel_code'</script>";
          die;
        endif;
  ...
  if ($result == 1) :
    ...
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=templist&code=1'
		</script>";
    exit();
  else :
    //Templist Shift details			
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?code=2'
		</script>";
    exit();
  endif;
endif;
...
if (isset($_POST['confirm_import']) && $_POST['confirm_import'] == "Import") :
  ...
  if ($rowCount > 0) :
    for ($i = 0; $i <= $rowCount; $i++) :
      ...
      $select_temp_data_records = sqlQUERY_LABEL("SELECT `temp_id`,`csvtype`,`sessionID`,`field1`,`field2`,`field3`,`field4`,`field5`,`field6`,`field7`,`field8`,`field9`,`field10`,`field11`,`field12`,`field13`,`field14`,`field15`,`field16`,`field17`,`field18`,`field19`,`field20`,`field21`,`field22`,`field23`,`field24`,`field25`,`field26`,`field27`,`field28`,`field29`,`field30`,`field31`,`field32`,`field33`,`field34`,`field35`,`field36`,`field37`,`field38`,`field39`,`status` FROM `dvi_tempcsv` WHERE `temp_id`='$temp_id' and `status`='1' and `csvtype` = '1'") or die("Unable to get records:" . sqlERROR_LABEL());

      while ($fetch_records = sqlFETCHARRAY_LABEL($select_temp_data_records)) :
        $hotel_city = $fetch_records['field1'];
        $hotel_name = $fetch_records['field2'];
        $hotel_code = $fetch_records['field3'];
        $getHotelID = getHOTELDETAILS($hotel_code, 'get_hotelid_from_hotelcode');
        $room_name = $fetch_records['field4'];
        $room_type = $fetch_records['field5'];
        $room_type_id = getROOMTYPE_DETAILS($room_type, 'room_type_id');
        ...
        $selected_rooms_query = sqlQUERY_LABEL("SELECT `room_ID` FROM `dvi_hotel_rooms` where `deleted` = '0' AND `status`='1' and `hotel_id` = '$getHotelID' AND `room_type_id`='$room_type_id' ") or die("#PARENT-LABEL: getHOTEL_ROOM_TYPE_DETAIL: " . sqlERROR_LABEL());
        if (sqlNUMOFROW_LABEL($selected_rooms_query) > 0) :
          while ($fetch_room_data = sqlFETCHARRAY_LABEL($selected_rooms_query)) :
            $getroomID_temp = $fetch_room_data['room_ID'];

            $room_rate_for_the_day_num_rows = getROOM_PRICEBOOK_DETAILS($getHotelID, $getroomID_temp, $year, $month, 'day_1', 'room_rate_for_the_day_num_rows');
            ...
            for ($day_i = 1; $day_i <= 31; $day_i++) {
              ...
              $get_room_rate = getROOM_PRICEBOOK_DETAILS($getHotelID, $getroomID_temp, $year, $month, $day_name, 'room_rate_for_the_day');

              $day_variable_name = "day" . $day_i;

              if ($$day_variable_name) {
                // If the variable already has a value, keep it as is
                $days[$day_variable_name] = $$day_variable_name;
              } elseif (empty($$day_variable_name)) {
                // If the variable is empty, try to get the room rate
                $days[$day_variable_name] = $get_room_rate ? $get_room_rate : 0;
              }
            }
            ...
            if ($room_rate_for_the_day_num_rows == 0) :
              $arrFields = array('`hotel_id`', '`room_type_id`', '`room_id`', '`year`', '`month`', '`day_1`', '`day_2`', '`day_3`', '`day_4`', '`day_5`', '`day_6`', '`day_7`', '`day_8`', '`day_9`', '`day_10`', '`day_11`', '`day_12`', '`day_13`', '`day_14`', '`day_15`', '`day_16`', '`day_17`', '`day_18`', '`day_19`', '`day_20`', '`day_21`', '`day_22`', '`day_23`', '`day_24`', '`day_25`', '`day_26`', '`day_27`', '`day_28`', '`day_29`', '`day_30`', '`day_31`', '`createdby`', '`status`');
              $arrValues = array("$getHotelID", "$room_type_id", "$getroomID_temp", "$year", "$month", "$day1", "$day2", "$day3", "$day4", "$day5", "$day6", "$day7", "$day8", "$day9", "$day10", "$day11", "$day12", "$day13", "$day14", "$day15", "$day16", "$day17", "$day18", "$day19", "$day20", "$day21", "$day22", "$day23", "$day24", "$day25", "$day26", "$day27", "$day28", "$day29", "$day30", "$day31", "$logged_user_id", "1");
              ...
              $arrFields = array('`day_1`', '`day_2`', '`day_3`', '`day_4`', '`day_5`', '`day_6`', '`day_7`', '`day_8`', '`day_9`', '`day_10`', '`day_11`', '`day_12`', '`day_13`', '`day_14`', '`day_15`', '`day_16`', '`day_17`', '`day_18`', '`day_19`', '`day_20`', '`day_21`', '`day_22`', '`day_23`', '`day_24`', '`day_25`', '`day_26`', '`day_27`', '`day_28`', '`day_29`', '`day_30`', '`day_31`', '`createdby`', '`status`');
              $arrValues = array("$day1", "$day2", "$day3", "$day4", "$day5", "$day6", "$day7", "$day8", "$day9", "$day10", "$day11", "$day12", "$day13", "$day14", "$day15", "$day16", "$day17", "$day18", "$day19", "$day20", "$day21", "$day22", "$day23", "$day24", "$day25", "$day26", "$day27", "$day28", "$day29", "$day30", "$day31", "$logged_user_id", "1");

              $sqlwhere = " `hotel_id` = '$getHotelID' and `room_id` = '$getroomID_temp' and `year` = '$year' and `month` = '$month' and `status` = '1' and `deleted` = '0'";
    ...
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=import_response'</script>";
  elseif ($rowCount == 0) :
    echo "<script type='text/javascript'>window.location = 'hotel_room_pricebook.php?route=templist&switch=W&type=import_row'</script>";
    exit();
  endif;
endif;
...
<body>
        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
                      ...
                      <div class="justify-content-center bulk-upload-body">
                        <div class="card-body bulk-import-body text-center p-5">
                          <svg xmlns="http://www.w3.org/2000/svg" height="150" version="1.1"
                            viewBox="-23 0 512 512" width="150">
                            <g id="surface1">
                              <path
                              ...
                              <path
                                ...
                                fill="#f4f4f7" data-original="#000000" />
                              <path
                          ...
                          <div class="mt-5">
                            <input type="file" class="btn btn-light rounded-pill btn-sm p-0"
                              name="csv">
                          </div>
                          <button type="submit" name="upload_csv"
                            class="btn btn-primary rounded-pill btn-sm mt-3"
                            value="confirm_upload">Upload</button>
                          <div class="mt-3">

                            <a href="excel_sampleformat_hotelroom_pricebook.php"
                              class="fs-6">Download Sample CSV </a>
                            <div id="icon-container" class="clickable mt-2"
                              ...
                              <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                ...
                                xml:space="preserve" class="ms-1">
                                <g>
                                  <path
                                    d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"
                                    fill="#3a57e8" data-original="#000000" opacity="1"
                                    class=""></path>
                                  <path
                                    d="M12 7a1 1 0 0 0-1 1v5a1 1 0 1 0 2 0V8a1 1 0 0 0-1-1zM12 15c-.26 0-.52.11-.71.29-.181.19-.29.45-.29.71s.109.52.29.71c.38.37 1.04.37 1.42 0 .18-.19.29-.45.29-.71s-.11-.52-.29-.71c-.19-.18-.45-.29-.71-.29z"
                                    fill="#3a57e8" data-original="#000000" opacity="1"
                                    class=""></path>
                                </g>
                              </svg>
                              ...
                              <div class="row justify-content-center">
                                <div class="col-md-4 text-start bulk-import-container py-4">
                                  <ul class="m-0">
                                    ...
                                    <li>All the contents should be in same order as the
                                      format.</li>
                                  </ul>
                                </div>
                              </div>
                  ...
                  <div class="col-sm-12">
                    <div class="card">
                      ...
                      <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="row">
                          <div class="col">
                            <a href="hotel_room_pricebook.php?regen=y"
                              class="btn btn-sm btn-label-github waves-effect mx-1"> <svg
                                class="icon-20 me-1" width="20" viewBox="0 0 24 24"
                            ...
                            <button type="submit" name="confirm_import" value="Import"
                              class="btn btn-sm bg-primary text-white mx-1">
                              <svg class="icon-18" style="margin-right: 5px;" width="18"
                                ...
                                <path
                                  ...
                                  stroke="currentColor" stroke-width="1.5"
                                  stroke-linecap="round" stroke-linejoin="round"></path>
                                <path d="M12.0215 2.19044V14.2314" stroke="currentColor"
                                  stroke-width="1.5" stroke-linecap="round"
                                  stroke-linejoin="round"></path>
                                <path d="M9.10645 5.1189L12.0214 2.1909L14.9374 5.1189"
                                  stroke="currentColor" stroke-width="1.5"
                                  stroke-linecap="round" stroke-linejoin="round"></path>
                              </svg>Import</button>
                          </div>
                      ...
                      <div class="card-body">
                        <div class="table-responsive import_table_div">
                          <table id="hotelpricebook_LIST"
                            class="table table-flush-spacing border pb-3" role="grid"
                            width="100%">
                            <thead class="table-head">
                              <tr class="ligth">
                                <th><input type="checkbox" class="select-all"
                                    id="select-all" name="select-all"></th>
                                <th>Hotel City</th>
                                <th>Hotel Name</th>
                                <th>Hotel Code</th>
                                <th>Room Name</th>
                            ...
                            <tbody>
                              ...
                              if ($num_row > 0) :
                                while ($row = sqlFETCHARRAY_LABEL($select_temp_hotelpricebook_list)) :
                                  $temp_id = $row['temp_id'];
                                  $field1 = html_entity_decode($row['field1']);
                                  $field1 = htmlspecialchars_decode($field1);
                                  $field2 = $row['field2'];
                                  $field3 = $row['field3'];
                                  $field4 = $row['field4'];
                                  $field5 = $row['field5'];
            ...
            <?php elseif ($_GET['route'] == 'import_response') :
              $query_imported = "SELECT `temp_id` FROM `dvi_tempcsv` WHERE `csvtype`='1' and `sessionID`= '$hotelprice_import_session_id'";
              $result_query_imported = sqlQUERY_LABEL($query_imported);
              $num_of_row_total = sqlNUMOFROW_LABEL($result_query_imported);

              $total_imported_query = "SELECT `temp_id` FROM `dvi_tempcsv` where `csvtype`='1' and `status`='2' AND `sessionID`='$hotelprice_import_session_id' ";
                    ...
                    <div class="card my-3">
                      <div class="card-body">
                        ...
                        <div class="text-center">
                          <a class="btn btn-light btn-sm shadow-none mt-1"
                            href="hotel_room_pricebook.php?route=import">Upload again</a>
                        </div>
                        <h5 class="text-center mt-3">OUT OF <?php echo $num_of_row_total; ?>,
                          <?php echo $num_of_row_imported; ?> IMPORTED</h5>
                      </div>
                    </div>
                  </div>
                  <div class="col-sm-12 p-0">
                    <div class="card mb-4 fullscreen">
                      <div class="card-header d-none">
                        <div class="media">
                          <div class="media-body">
                            <h4 class="content-color-primary mb-0"><?php echo $__list_import; ?>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div class="card-body p-3">
                        <div class="table-responsive import_table_div">
                          <table class="table table-bordered table-responsive" id="import_list"
                            width="100%">
                            <thead>
                              <tr>
                                <th>Reason</th>
                  ...
                  <div class="col-md-12 mt-3">
                    <div class="card">
                      <div class="card-body text-center">
                        <h5>Few Records Only Imported Successfully</h5>
                        <div class="mt-4">
  ...
  if ($_GET['switch'] != '' && $_GET['type'] != '') :
    ...
    elseif ($_GET['type'] == 'hotel_code_required') :
      $sno = $_GET['sno'];
      $customMsg = "Empty Hotel Code on Row - $sno  !!! ";
    elseif ($_GET['type'] == 'hotel_name_required') :
      $sno = $_GET['sno'];
      $customMsg = "Empty Hotel Name on Row - $sno  !!! ";
    elseif ($_GET['type'] == 'room_code_required') :
      $sno = $_GET['sno'];
      $customMsg = "Empty Room Code on Row - $sno  !!! ";
    elseif ($_GET['type'] == 'hotel_name_not_exist') :
      $hotel_name = $_GET['hotel_name'];
      $customMsg = "Invalid data !!! Hotel Name $hotel_name is not exist.";
    elseif ($_GET['type'] == 'hotel_code_not_exist') :
      $hotel_code = $_GET['hotel_code'];
      $customMsg = "Invalid data !!! Hotel Code $hotel_code is not exist.";
    elseif ($_GET['type'] == 'room_code_not_exist') :
      $room_code = $_GET['room_code'];
      $customMsg = "Invalid data !!! Room Code $room_code is not exist.";
    elseif ($_GET['type'] == 'room_code_already_exist') :
      $room_code = $_GET['room_code'];
      $customMsg = "Duplicate data !!! Room Code $room_code is already existing.";
    endif;
  endif;
  ?>
  <!-- / Layout wrapper -->
