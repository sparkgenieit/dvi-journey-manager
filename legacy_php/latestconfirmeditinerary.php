...
$get_cnfi_filter = isset($_GET['type']) ? $_GET['type'] : '';
$cnfi_list_filter = '';
$agent_id = '';
$guide_id = '';
$staff_id = '';
...
if ($get_cnfi_filter != '') {
    if ($get_cnfi_filter == 'cancelled_bookings') {
        $cnfi_list_filter = '1';
        // echo "<script>alert('Cancelled Bookings Filter Applied. List Filter: $cnfi_list_filter');</script>";
    } elseif ($get_cnfi_filter == 'agent_bookings') {
        $agent_id_value = isset($_GET['aid']) ? $_GET['aid'] : '';
        // echo "<script>alert('Agent Bookings Filter Applied. Agent ID: $agent_id');</script>";
    } elseif ($get_cnfi_filter == 'guide_bookings') {
        $guide_id_value = isset($_GET['gid']) ? $_GET['gid'] : '';
        // echo "<script>alert('Guide Bookings Filter Applied. Guide ID: $guide_id');</script>";
    } elseif ($get_cnfi_filter == 'travelexpert_bookings') {
        $staff_id_value = isset($_GET['tid']) ? $_GET['tid'] : '';
        // echo "<script>alert('Travel Expert Bookings Filter Applied. Staff ID: $staff_id');</script>";
    } elseif ($get_cnfi_filter == 'vendor_bookings') {
        $vendor_id_value = isset($_GET['vid']) ? $_GET['vid'] : '';
        // echo "<script>alert('Vendor Bookings Filter Applied. Vendor ID: $vendor_id');</script>";
    }
}
?>
...
<body>
                        ...
                        <div class="loader-overlay" id="show_itineary_loader">
                            <div class="loader" id="loader">
                                <div class="text-center">
                                    <img class="loader-image" src="assets/img/loader-pin.gif" alt="Loading..." width="250px" height="200px">
    ...
    <div class="modal fade" id="MODALINFODATA" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                ...
                <div class="modal-body receiving-distancelimit-restriction-modal-info-form-data">
                    <div class="row">
                        <div class="text-center">
                            ...
                            <!-- <img src="../../assets/img/svg/route-optimize.svg" alt="route optimize"/> -->
                        </div>
    ...
    <script>
        ...
        function showITINERARYLIST() {
            $.ajax({
                type: 'post',
                url: 'engine/ajax/ajax_confirmed_itinerary_list.php?type=show_form&get_cnfi_filter=<?= $cnfi_list_filter; ?>&agent_id=<?= $agent_id_value; ?>&guide_id=<?= $guide_id_value; ?>&staff_id=<?= $staff_id_value; ?>&vendor_id=<?= $vendor_id_value; ?>',
        ...
        function showITINERARYFORMSTEP2(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/ajax_latest_confirmed_itineary_step2_form.php?type=show_form",
                data: {
                    _ID: ID
                },
                success: function(response) {
