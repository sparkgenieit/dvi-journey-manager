...
$filename = "Sample_Excel_Format_Room.Csv";
while (ob_get_level()) {
    ob_end_clean();
}
header_remove();
...
$select_hotel_room_details = sqlQUERY_LABEL("SELECT dvi_hotel.*, dvi_hotel_rooms.*, dvi_hotel.hotel_name AS hotel_name,dvi_hotel.hotel_code As hotel_code,dvi_hotel_rooms.room_title AS room_name,dvi_hotel_rooms.room_ref_code AS room_code FROM dvi_hotel JOIN dvi_hotel_rooms ON dvi_hotel_rooms.hotel_id=dvi_hotel.hotel_id WHERE dvi_hotel.status='1' and dvi_hotel.deleted='0' and dvi_hotel_rooms.deleted='0'") or die("#1-UNABLE_TO_COLLECT_ORDER_DETAILS:" . sqlERROR_LABEL());
...
while ($fetch_hotel_data = sqlFETCHARRAY_LABEL($select_hotel_room_details)) :
    ...
    $room_name = $fetch_hotel_data['room_name'];
    ...
    $sheet->setCellValue('C' . $rowIndex, $hotel_code); // Insert the username here
    $sheet->setCellValue('D' . $rowIndex, $room_ref_code);
    $sheet->setCellValue('E' . $rowIndex, $room_name);
    $sheet->setCellValue('F' . $rowIndex, $month);
    $sheet->setCellValue('G' . $rowIndex, $year);
    $sheet->setCellValue('H' . $rowIndex, ''); // Insert the total product qty here
