...
<body>
                            ...
                            <div class="col-md-12">
                                <div class="card p-0">
                                    <div class="card-header pb-3 d-flex justify-content-between">
                                        <h5 class="card-title mb-3">List of GST Settings</h5>
                                        <a href="javascript:void(0)" class="btn btn-label-primary waves-effect" onclick="showGSTSETTINGMODAL(0);" data-bs-dismiss="modal">+ Add GST Settings</a>
                                    </div>
                                    <div class="card-body dataTable_select text-nowrap">
                                        <div class="text-nowrap table-responsive table-bordered">
                                            <table class="table table-hover " id="gst_setting_LIST">
                                                <thead>
                                                    <tr>
                                                        <th scope="col">S.No</th>
                                                        <th scope="col">Action</th>
                                                        <th scope="col">GST Title</th>
    ...
    <div class="modal fade" id="addGSTSETTINGFORM" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-simple modal-enable-otp modal-dialog-centered">
            <div class="modal-content p-3 p-md-5">
                <div class="receiving-gst-setting-form-data">
    ...
    <script>
        $(document).ready(function() {
            $('#gst_setting_LIST').DataTable({
                ...
                buttons: [{
                        ...
                        exportOptions: {
                            columns: [0, 1, 4], // Only name, email and role
                        }
                    },
                    {
                        extend: 'excel',
                ...
                ajax: {
                    "url": "engine/json/__JSONgstsetting.php",
                    "type": "GET"
                },
                columns: [{
                        data: "count"
        ...
        //ADD & DELETE MODAL
        function showGSTSETTINGMODAL(GST_SETTING_ID) {
            $('.receiving-gst-setting-form-data').load('engine/ajax/__ajax_add_gstsetting.php?type=show_form&GST_SETTING_ID=' + GST_SETTING_ID + '', function() {
                const container = document.getElementById("addGSTSETTINGFORM");
                const modal = new bootstrap.Modal(container);
                modal.show();
                if (GST_SETTING_ID) {
                    $('#GSTSETTINGFORMLabel').html('Update GST Settings');
                } else {
                    $('#GSTSETTINGFORMLabel').html('Add GST Settings');
                }
            });
        }
        //SHOW DELETE POPUP
        function showDELETEGSTSETTINGMODAL(ID) {
            $('.receiving-confirm-delete-form-data').load('engine/ajax/__ajax_manage_gstsetting.php?type=delete&ID=' + ID, function() {
        ...
        function confirmGSTSETTINGDELETE(ID) {
            $.ajax({
                type: "POST",
                url: "engine/ajax/__ajax_manage_gstsetting.php?type=confirmdelete",
                data: {
                    _ID: ID
                },
                dataType: 'json',
        ...
        //STATUS UPDATE
        function togglestatusITEM(STATUS_ID, GST_SETTING_ID) {
            if (GST_SETTING_ID) {
                $.ajax({
                    type: "POST",
                    url: "engine/ajax/__ajax_manage_gstsetting.php?type=updatestatus",
                    data: {
                        GST_SETTING_ID: GST_SETTING_ID,
                        STATUS_ID: STATUS_ID
                    },
                    dataType: 'json',
                    success: function(response) {
