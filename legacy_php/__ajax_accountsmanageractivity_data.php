...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) : //CHECK AJAX REQUEST
    ...
    if ($_GET['type'] == 'show_form_activity') :
        ...
        while ($getstatus_fetch = sqlFETCHARRAY_LABEL($getstatus_query)) :
            ...
            $itinerary_route_IDs = getACCOUNTSMANAGER_ACTIVITYPLAN_IDS($from_date, $to_date, 'itinerary_route_ID');
            ...
            $select_accountsmanagerHotspot_query = sqlQUERY_LABEL("SELECT `itinerary_plan_ID`, SUM(`total_paid`) AS `paid_amount`, SUM(`total_balance`) AS `balance_amount` FROM `dvi_accounts_itinerary_activity_details` WHERE `deleted` = '0' AND `activity_amount` > 0  {$filterbyaccounts_date_format} {$filterbyaccountsmanager} {$acc_itinerary_details_ID} GROUP BY `itinerary_plan_ID`") or die("#1-UNABLE_TO_COLLECT_COURSE_LIST:" . sqlERROR_LABEL());
...
?>
                    ...
                    <div class="card-body p-3">
                        <div class="d-flex align-items-start justify-content-center">
                            <div class="content-left">
                                ...
                                <div class="text-center mt-2">
                                    <h4 class="mb-0 me-2 <?= $profit_label ?>"><?= general_currency_symbol ?> <?= number_format(round($total_profit_activity - $coupon_discount_amount), 2); ?></h4>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card p-3 mb-4 px-4">
            <div class="d-flex justify-content-between mb-3">
                <h4 class="mb-0">List of Activity Detail</h4>
                <div class="d-flex align-items-center">
                    <input type="text" id="searchActivity" class="form-control me-3" placeholder="Search...">
                    <?php if($billed_amount != 0):?>
                    <button id="export-accounts-activity-btn" class="btn btn-sm btn-label-success"><i class="ti ti-download me-2"></i>Export</button>
                    <?php endif; ?>
                </div>
            </div>
            <div class="text-nowrap  table-responsive  table-bordered">
                <table class="table table-hover" id="activity_accountsmanager_list">
                        ...
                        echo '<tbody>';
                            while ($fetch_list_data = sqlFETCHARRAY_LABEL($select_accountsmanagerLIST_query_activity)) :
                                ...
                                if ($ID == 2) {
                                    ...
                                    $paynow_button = '<button type="button" class="btn btn-label-primary pay-now-activity-btn" data-row-id="' . $activity_ID . '" data-acc-activity_detail-id="' . $accounts_itinerary_activity_details_ID . '" data-bs-toggle="modal" data-total-inhandeactivity-paynow="' . $inhand_amount_withoutformat . '" data-total-balanceactivity-paynow="' . $total_balance_withoutformat . '" data-itinerary-plan-id="' . $itinerary_plan_ID . '" data-bs-target=".accountmanageraddpaymentactivitymodalsection">Pay Now</button>';
                                }
                                    ...
                                    $total_profit =  general_currency_symbol . ' ' . number_format(round($total_activity_amount - $total_activity_incidental - $coupon_discount_amount), 2);
        ...
        <div class="modal fade accountmanageraddpaymentactivitymodalsection" id="accountmanageraddpaymentactivitymodalsection" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-simple modal-enable-otp modal-dialog-centered">
                <div class="modal-content p-3 p-md-5">
                    <div class="modal-body">
                        ...
                        <form id="paynowactivityForm" class="row g-3" action="" method="post" data-parsley-validate>
                            <div class="col-12">
                                <div>
                                    ...
                                    <input type="hidden" name="hidden_itinerary_activity_ID" id="hidden_itinerary_activity_ID">
                                </div>
                            </div>
                            <div class="col-12">
                                <div>
                                    <label for="payment_amount_activity" class="form-label">Payment Amount <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" required id="payment_amount_activity" name="payment_amount_activity" autocomplete="off" placeholder="Enter Payment Amount" />
                                    <input type="hidden" name="hidden_activity_id" id="hidden_activity_id">
                                    <input type="hidden" name="hidden_acc_activity_detail_id" id="hidden_acc_activity_detail_id">
        ...
        <script>
            $(document).ready(function() {
                $("#searchActivity").on("keyup", function() {
                    ...
                    $("#activity_accountsmanager_list tbody tr").filter(function() {
                ...
                $('#export-accounts-activity-btn').click(function() {
                    window.location.href = 'excel_export_accounts_manager_activity.php?id=<?= $ID ?>&from_date=<?= $from_date ?>&to_date=<?= $to_date ?>&agent_name=<?= $agent_name ?>&quote_id=<?= $quote_id ?>';
                });

                $(document).on('click', '.pay-now-activity-btn', function() {
                    const generalCurrencySymbol = '₹';
                    const activityId = $(this).data('row-id');
                    const acc_activity_detail_id = $(this).data('acc-activity_detail-id');
                    const totalBalance = $(this).data('total-balanceactivity-paynow');
                    const totalInhand = $(this).data('total-inhandeactivity-paynow');
                    ...
                    $('#paynowactivityForm')[0].reset();
                    $('#hidden_activity_id').val(activityId);
                    $('#hidden_acc_activity_detail_id').val(acc_activity_detail_id);
                    $('#hidden_itinerary_activity_ID').val(itineraryid);
                    $('#payment_amount_activity').data('total-balanceactivity-paynow', totalBalance).data('total-inhandeactivity-paynow', totalInhand);
                ...
                $('#payment_amount_activity').on('input', function() {
                    var paymentAmount = parseFloat($(this).val()) || 0;
                    var totalInhand = parseFloat($(this).data('total-inhandeactivity-paynow')) || 0;
                    var totalBalance = parseFloat($(this).data('total-balanceactivity-paynow')) || 0;
                    ...
                    if (paymentAmount > totalInhand) {
                        ...
                        $('#paynowactivityForm button[type="submit"]').attr('disabled', true);
                        ...
                        $('#paynowactivityForm button[type="submit"]').attr('disabled', false);
                    }
                });

                // Clear modal on close
                $('.accountmanageraddpaymentactivitymodalsection').on('hidden.bs.modal', function() {
                    $('#paynowactivityForm')[0].reset();
                    $('#hidden_activity_id').val('');
                });

                // Form submission
                $("#paynowactivityForm").submit(function(event) {
                    event.preventDefault();

                    var paymentAmount = $('#payment_amount_activity').val();
                    var activityId = $('#hidden_activity_id').val();

                    var form = $(this)[0];
                    var data = new FormData(form);

                    data.append('payment_amount_activity', paymentAmount);
                    data.append('hidden_activity_id', activityId);
                    ...
                    $.ajax({
                        ...
                        if (!response.success) {
                            if (response.errors.hotspot_processed_by_required) {
                                ...
                                $('#payment_amount_activity').focus();
                            }
                        } else {
                            // $('#activity_accountsmanager_list').DataTable().ajax.reload();
                            show_ACCOUNTSMANAGER_ACTIVITY_DATA_NEW('<?= $ID; ?>', '<?= $from_date; ?>', '<?= $to_date; ?>', ' <?= $agent_name; ?>', '<?= $quote_id; ?>');
                            $('#accountmanageraddpaymentactivitymodalsection').modal('hide');
                ...
                function show_ACCOUNTSMANAGER_ACTIVITY_DATA_NEW(ID, fromDate, toDate, agentName, selectedQuoteId) {
                    $.ajax({
                        type: 'post',
