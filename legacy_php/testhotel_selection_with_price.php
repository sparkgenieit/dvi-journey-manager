...
SELECT
  ...
  IRD.itinerary_route_ID AS itinerary_route_id,  -- stable lowercase key
...
if (!function_exists('hobse_pick_price')) {
    // BEFORE: function hobse_pick_price(array $resp): array
    function hobse_pick_price(array $resp, ?string $mustRatePlanCode = null): array
    {
        ...
        foreach ($hotels as $h) {
            ...
            foreach ($optList as $opt) {
                // Some APIs attach rate plan at the option level
                ...
                foreach ($rates as $rate) {
                    ...
                    // If filtering, keep only exact matches (option-level OR rate-level)
                    if ($needle) {
                    ...
                    if ($price !== null && ($min === null || $price < $min)) {
                        $min = $price;
                        // try to capture room ids (field names vary across HOBSE tenants)
                        $bestMeta = [
        ...
        return ['ok' => ($min !== null), 'price' => $min, 'currency' => $currency, 'meta' => $bestMeta];
    }
}
...
function check_hotel_one_night(array $row, string $from, string $to, array $roomsForHobse, array $paxRooms, string $nationality, array $plan, bool $debug = false): array
{
    ...
    if (!empty($row['hobse_hotel_code'])) {
        try {
            ...
            // 4) Pick only rates that match the meal plan rate code (if we have one)
            ...
            $attempts[] = [
                ...
                'request'  => $debug ? clip_debug($payload) : null,
                'response' => $debug ? clip_debug($resp)    : null,
    ...
    if (!empty($row['tbo_hotel_code'])) {
        try {
            ...
            if ($pick['ok']) {
                ...
                // Always record both attempts for trace/debug
                $attempts[] = [
                    'vendor'   => 'TBO',
                    'ok'       => true,
                    'price'    => $pick['price'],
                ...
                if (!empty($dvi['ok'])) {
                    // Prefer DVI if pricebook has a price
                    $rate = $dvi['rate'] ?? [];
                    return [
                        'ok'       => true,
...
function build_picks(int $planId, float $radiusKm, int $showPerGroup, bool $debugVendor): array
{
    ...
    // candidates per date & group (sorted by distance)
    $candidates = [];
    $sqlEcho    = [];
    $today = date('Y-m-d');
    ...
    // date iteration in order
    $prevDate = null;
    foreach ($dates as $d) {
        $usedByDate[$d] = [];
        $isConsecutive = false;
        if ($prevDate && (strtotime($d) === strtotime($prevDate . ' +1 day'))) $isConsecutive = true;

        foreach (['group1', 'group2', 'group3', 'group4'] as $g) {
            if ((microtime(true) - $t0) > $budget) {
                $traces[$d][$g] = ['candidates' => []];
                if ($d < $today) { // don't book past dates
            ...
            if ($isConsecutive && !empty($lastPickedByGroup[$g])) {
                ...
                // ensure not already used by another group today
                if (empty($usedByDate[$d][$keepHotelId])) {
                    // ensure it's present in today's candidates (within radius/category/facilities today)
                    foreach ($list as $row) {
                        if (time_left() <= 0.0) {
                            break;
                        }
            ...
            // 2) Otherwise walk candidates by distance asc, skip duplicates within the date
            $found = false;
            $attemptsCount = 0;
            foreach ($list as $row) {
                if (time_left() <= 0.0) {
                ...
                if (isset($triedHotelIds[$hid])) continue;   // already tried (e.g., keep-same failed)
                $attemptsCount++;
    ...
    return [
        ...
        'radiusKm'     => $radiusKm,
        'facilities'   => $facArr,
        'candidates'   => $candidates, // for SQL echo + context
        'sqlEcho'      => array_map(fn($byG) => array_map(fn($x) => $x, $byG), []), // not used but kept for parity
        'picks'        => $picks,
        'traces'       => $traces,
    ];
}
                                            ...
                                            <tbody>
                                                ...
                                                ?>
                                                    ...
                                                    <?php if (!empty($cand['attempts'])): ?>
                                                        <!-- Debug row: collapsible vendor request/response JSON -->
                                                        <tr>
...
$radius = isset($_GETs['radius']) ? (float)$_GET['radius'] : DEFAULT_RADIUS_KM;
...
// ======= REVIEW SECTION =======
// Example: peek at one mapped row before writing
// (Pick a date+group you know has a selection)
...
if ($someDate) {
    ...
    if ($pick && !empty($pick['ok'])) {
        ...
        //echo '<pre style="padding:12px;background:#111;color:#eee;">PREVIEW hotel_details (selected): ' . htmlspecialchars(json_encode($preview, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)) . '</pre>';
    }
}
...
echo '<div class="card">Persist summary: '
    . htmlspecialchars(json_encode($summary, JSON_UNESCAPED_SLASHES)) . '</div>';
